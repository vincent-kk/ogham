...
### [`@ogham/cennad`](./plugins/cennad/) — Codex / Gemini / Antigravity CLI 위임

MCP 도구, 스킬, 라이프사이클 훅을 통해 Claude가 **OpenAI Codex CLI** 또는 **Google Gemini / Antigravity CLI**에 작업을 위임할 수 있게 해주는 Claude Code 플러그인입니다.

모델마다 강점이 다릅니다. Codex는 샌드박스 셸에서의 대규모 코드 생성에 강하고, Gemini와 Antigravity는 실시간 웹 기반 리서치와 초대형 컨텍스트 종합에 강합니다. cennad는 이 위임을 명시적이고, 비율 인식적이며, 세션 간 재현 가능하게 만듭니다.
...
| MCP 도구 | 4    | `start_conversation`, `continue_conversation`, `list_antigravity_models`, `open_settings`       |
| Agents   | 0    | (스킬이 외부 CLI에 직접 위임)                                                                   |
| Hooks    | 2    | SessionStart, UserPromptSubmit                                                                  |

> `gemini`와 `antigravity`는 상호 배타적인 Google 엔진입니다(Gemini CLI 서비스는 **2026-06-18** 종료). `/cennad:setup`에서 하나를 활성화하세요.

**주요 기능:**

- **다중 모델 위임** — 코드 중심 작업은 Codex(샌드박스 셸, 리팩토링)로, 리서치 중심 작업은 Gemini 또는 Antigravity(실시간 웹 검색, 대형 컨텍스트)로 키워드 기반 라우팅
- **교차 검증** — `/cennad:crosscheck`가 동일 프롬프트를 두 프로바이더에 병렬로 보내고 응답을 종합
...
# Gemini CLI에 위임 (레거시 — 서비스 2026-06-18 종료)
/cennad:gemini
...
| **[`cennad`](./plugins/cennad/)**             | Claude plugin | Claude Code에서 OpenAI Codex CLI / Google Gemini / Antigravity CLI로 위임 |
