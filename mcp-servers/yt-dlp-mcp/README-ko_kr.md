...
| 변수                         | 기본값           | 용도                                                                                                                                                                                 |
| ---------------------------- | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `YTDLP_PLAYER_CLIENT`        | `ios,tv,default` | yt-dlp가 위장할 YouTube player client 목록(콤마, 순서대로 시도). 전 호출 공통. `ios`/`tv`가 429를 가장 잘 피하고, `default`(web 포함)가 폴백이라 댓글·포맷이 정상 동작. 빈값=비활성. |
...
YouTube는 IP 주소 단위로 레이트 리밋을 걸며, 그 제한은 시간이 지날수록 누적됩니다. 자막·스크립트 엔드포인트가 가장 엄격하고, 한 번 제한에 걸리면 몇 분에서 길게는 약 하루까지 지속될 수 있습니다. 서버는 기본적으로 429에 강한 `ios`/`tv` player client를 위장하므로(`YTDLP_PLAYER_CLIENT`) 별도 설정 없이도 대부분의 제한을 피합니다. 그래도 도구 응답에 `[RATE_LIMITED]`나 `[BLOCKED]`가 보이면, 효과가 큰 순서대로 다음을 시도하세요:
