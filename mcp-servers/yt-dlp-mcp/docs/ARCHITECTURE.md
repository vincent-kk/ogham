...
설계 철학 두 줄:
1. **포괄 지원, 최소 기본** — yt-dlp가 하는 일을 도구로 폭넓게 노출하되, 기본 활성은 4개뿐. 나머지는 **flag를 켠 경우에만 도구로 등록**(미사용 시 LLM context 절약).
...
**비목표 (Non-goals)**: yt-dlp 로직의 JS 재구현(ADR-7), 인증 필요한 비공개 영상(쿠키 opt-in으로만). *(영상/오디오 다운로드는 이제 opt-in 기능으로 포함됨 — ADR-8.)*
...
- **결정**: 도구를 **레지스트리**로 정의하고, **기본 4개만 always-on**, 나머지는 **각 기능별 환경변수 flag가 켜졌을 때만 `registerTool` 호출**. 꺼두면 `tools/list`에 아예 안 나타남.
...
flowchart TD
  subgraph Transport["MCP Transport (mcp/)"]
    SRV["McpServer"]
    REG["Tool Registry<br/>(조건부 등록: default | flag)"]
...
flowchart TD
  ...
  Loop -->|enabledBy = default| Reg["registerTool"]
  Loop -->|enabledBy = flag & flag ON| Reg
...
### 6-1. 도메인 타입(발췌)
```ts
interface TranscriptSegment { text: string; startMs: number; durationMs: number; }
interface VideoMetadata { videoId: string; title: string; channel: string; viewCount?: number; durationSec?: number; uploadDate?: string; }
interface Chapter { title: string; startMs: number; endMs?: number; }
interface HeatmapSpan { startMs: number; endMs: number; score: number; }   // most-replayed
interface CommentNode { id: string; text: string; author: string; likeCount?: number; parent?: string; depth: number; replies?: CommentNode[]; }
interface DownloadResult { path: string; bytes: number; format: string; }   // 파일 산출물
```

### 6-2. 포트(인터페이스)
```ts
interface BinaryManager { ensureBinary(signal?: AbortSignal): Promise<string>; }       // 동시 호출 안전(lock)
interface VersionResolver { resolveSafeVersion(cooldownDays: number): Promise<{tag:string; assetUrl:string; sumsUrl:string}>; }
interface Runner { run(binPath: string, args: string[], opts?: {timeoutMs?: number; signal?: AbortSignal}): Promise<{stdout: string}>; }

// 도구 레지스트리 계약 (ADR-8의 핵심)
type EnableKey = 'default'
  | 'subtitles' | 'metadataSummary' | 'comments' | 'chapters' | 'heatmap'
  | 'thumbnail' | 'download' | 'playlist';
interface ToolDefinition {
  name: string;                 // 예: 'ytdlp_get_comments'
  enabledBy: EnableKey;         // 'default' 또는 특정 flag
  register(server: McpServer, deps: Deps): void;   // zod 스키마·핸들러 등록
}
// 부팅: for (t of TOOL_REGISTRY) if (t.enabledBy==='default' || config.enable[t.enabledBy]) t.register(server, deps);
```
...
> **기본 4개는 항상 등록.** 나머지는 해당 `YTDLP_ENABLE_*=1`일 때만 등록되어 `tools/list`에 노출.

| 도구 | 기능 | 등록 조건 | 부작용/주석 |
|------|------|-----------|------------|
| `ytdlp_search_videos` | 키워드 검색(페이지네이션·날짜필터) | **기본** | readOnly |
| `ytdlp_list_subtitle_languages` | 가용 자막 언어/형식 | **기본** | readOnly |
| `ytdlp_download_transcript` | 깨끗한 자막 텍스트(타임스탬프 옵션) | **기본** | readOnly |
| `ytdlp_get_video_metadata` | 메타데이터(JSON, 필드 선택) | **기본** | readOnly |
| `ytdlp_get_video_subtitles` | 원본 자막(VTT/json3, 타임스탬프 보존) | `YTDLP_ENABLE_SUBTITLES` | readOnly |
| `ytdlp_get_video_metadata_summary` | 사람이 읽는 메타 요약 | `YTDLP_ENABLE_METADATA_SUMMARY` | readOnly |
| `ytdlp_get_comments` | 댓글(flat/threaded/markdown_tree) | `YTDLP_ENABLE_COMMENTS` | readOnly |
| `ytdlp_get_comments_summary` | 댓글 요약 | `YTDLP_ENABLE_COMMENTS` | readOnly |
| `ytdlp_get_chapters` | 챕터(구간 목차) | `YTDLP_ENABLE_CHAPTERS` | readOnly |
| `ytdlp_get_heatmap` | most-replayed 구간(시청 히트맵) | `YTDLP_ENABLE_HEATMAP` | readOnly |
| `ytdlp_get_thumbnail` | 썸네일 **다운로드** | `YTDLP_ENABLE_THUMBNAIL` | 파일 생성(¬readOnly) |
| `ytdlp_download_video` | 영상 다운로드(해상도·트리밍) | `YTDLP_ENABLE_DOWNLOAD` | 파일 생성·¬idempotent |
| `ytdlp_download_audio` | 오디오 추출 다운로드 | `YTDLP_ENABLE_DOWNLOAD` | 파일 생성 |
| `ytdlp_get_playlist` | 플레이리스트 항목(+채널 영상목록) | `YTDLP_ENABLE_PLAYLIST` | readOnly |
...
- **확장**: 새 yt-dlp 기능(예: `ytdlp_list_formats`, `ytdlp_get_channel`)은 `ToolDefinition` 엔트리 + `EnableKey` 추가만으로 편입(G6).
- **편의 묶음 옵션**: `YTDLP_ENABLE_ALL=1`로 전체 on(편의), 또는 `YTDLP_DISABLE=<csv>`로 일부 off — (선택 구현).
