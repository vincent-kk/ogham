...
function ctxWith(runner: Parameters<typeof downloadOperation>[0]['runner']): OpContext {
...
describe('download/thumbnail/subtitles operations', () => {
  it('returns the moved file path and size for downloads', async () => {
    ...
    const runner = makeFakeRunner({ stdout: filePath, files: { 'video.mp4': 'binarydata' } });
    const result = await downloadOperation(ctxWith(runner), { url: SAMPLE_URL, kind: 'video' });
  ...
  it('passes audio extraction flags', async () => {
    ...
    const runner = makeFakeRunner({ stdout: filePath, files: { 'audio.m4a': 'aaa' } });
    await downloadOperation(ctxWith(runner), { url: SAMPLE_URL, kind: 'audio' });
  ...
  it('detects the new thumbnail file', async () => {
    ...
    const result = await thumbnailOperation(ctxWith(runner), { url: SAMPLE_URL });
  ...
  it('returns raw subtitle content + parsed segments', async () => {
    const runner = makeFakeRunner({ stdout: 'abc', files: { 'abc.en.json3': SAMPLE_JSON3 } });
    const ctx: OpContext = { runner, config: env.config, paths: env.paths, logger: silentLogger };
    const result = await subtitlesOperation(ctx, { url: SAMPLE_URL, language: 'en' });
