...
import { isToolEnabled } from '../mcp/registry.js';
import type { EnableKey, ToolDefinition } from '../mcp/tool-definition.js';
...
describe('isToolEnabled', () => {
  it('always enables default tools', () => {
    expect(isToolEnabled(fakeTool('default'), loadConfig({}).enable)).toBe(true);
  });

  it('keeps flagged tools off by default', () => {
    expect(isToolEnabled(fakeTool('comments'), loadConfig({}).enable)).toBe(false);
  });
