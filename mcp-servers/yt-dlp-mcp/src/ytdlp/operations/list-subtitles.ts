...
import { asRecord, asRecordArray, asString } from '../../utils/coerce.js';
...
function toTracks(group: Record<string, unknown> | undefined, isAutomatic: boolean): SubtitleTrack[] {
  ...
  for (const [language, value] of Object.entries(group)) {
    ...
    const ext = variants.map((v) => asString(v.ext)).find((e): e is string => Boolean(e)) ?? 'vtt';
    const name = variants.map((v) => asString(v.name)).find((n): n is string => Boolean(n));
