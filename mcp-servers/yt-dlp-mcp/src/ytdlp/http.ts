...
async function fetchOrThrow(url: string, signal?: AbortSignal): Promise<Response> {
  ...
  try {
    ...
    throw new YtDlpMcpError(ErrorCode.NETWORK, `Network request failed: ${url}`, { cause: error });
  }
  if (!res.ok) {
    const code = res.status === 429 ? ErrorCode.RATE_LIMITED : ErrorCode.NETWORK;
...
export async function downloadToFile(url: string, destPath: string, signal?: AbortSignal): Promise<void> {
  const res = await fetchOrThrow(url, signal);
