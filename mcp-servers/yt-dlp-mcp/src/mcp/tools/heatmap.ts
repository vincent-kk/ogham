...
import { formatTimestamp } from '../../postprocess/formatter.js';
...
export const heatmapTool: ToolDefinition = {
  ...
  register(server, { service, config }) {
    server.registerTool(
      'ytdlp_get_heatmap',
      { title: 'Get heatmap', description, inputSchema, annotations: READ_ONLY },
      async (args, extra) =>
        handleToolExecution(
          async () => {
            const result = await service.execute(
              { cacheKey: cacheKey('heatmap', args.url), cacheable: true, signal: extra.signal },
              (ctx) => heatmapOperation(ctx, { url: args.url }),
            );
            return { text: render(result), structuredContent: { ...result } };
          },
          { errorPrefix: 'Error fetching heatmap', characterLimit: config.extraction.characterLimit },
        ),
    );
  },
};
