...
const inputSchema = {
  ...
  fields: z
    .array(z.string())
    .optional()
    .describe("Subset of metadata keys to return, e.g. ['title','viewCount','uploadDate']. Omit for all."),
};
...
export const metadataTool: ToolDefinition = {
  ...
  register(server, { service, config }) {
    server.registerTool(
      ...
      { title: 'Get video metadata', description, inputSchema, annotations: READ_ONLY },
      async (args, extra) =>
        handleToolExecution(
          async () => {
          ...
          { errorPrefix: 'Error extracting video metadata', characterLimit: config.extraction.characterLimit },
        ),
    );
  },
};
