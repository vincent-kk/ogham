...
const inputSchema = {
  ...
  resolution: z.enum(['480p', '720p', '1080p', 'best']).optional().describe('Preferred quality (default 720p).'),
  startTime: z.string().regex(TIME_RE).optional().describe('Trim start HH:MM:SS[.ms] (requires ffmpeg).'),
  endTime: z.string().regex(TIME_RE).optional().describe('Trim end HH:MM:SS[.ms] (requires ffmpeg).'),
};
...
export const downloadVideoTool: ToolDefinition = {
  ...
  register(server, { service, config }) {
    server.registerTool(
      ...
      { title: 'Download video', description, inputSchema, annotations: WRITES_FILE },
      async (args, extra) =>
        handleToolExecution(
          async () => {
          ...
          { errorPrefix: 'Error downloading video', characterLimit: config.extraction.characterLimit },
        ),
    );
  },
};
