...
- **Codex 스킬 변이**(`buildCodexSkills`) — filid·entrez·r-statistics·imbas 의 `subagent_type` 스폰을 self-load(`.shared/personas/<id>.md` 채택)로 재작성해 `.codex-plugin/skills/` 방출. prawf 는 이미 self-load(무변환). **Claude `skills/` 바이트 불변.**
