...
> ## ✅ 실행 완료 — 코드 레벨 (2026-07-15)
>
> `instructionsFile()` · `ruleDocsTarget()` 가 `hostPaths` 에 들어가고, filid 의 **쓰기(채널 분기) + 읽기(훅 합집합 판독)**, maencof 의 **3 핸들러 + 등록부 + SessionStart 훅**이 전부 반영됐다. 마커 구간 병합은 순수 모듈 `@ogham/cross-platform/instructions` 가 소유한다. 전체 테스트 4267 초록.
>
> **이 문서가 몰랐던 것 두 가지** (착수 후 드러남):
>
> 1. **훅엔 `OGHAM_HOST` 가 없다.** 어댑터는 MCP 선언에만 주입한다 → 훅은 `detectHost()` 를 **쓸 수 없다**. 그래서 읽기 채널은 호스트 분기가 아니라 **모든 채널 합집합 판독**으로 풀었다 (§2 의 "읽기 쪽도 같이 봐야 한다" 를 이 방식으로 충족).
> 2. **maencof 도 훅 읽기·쓰기 함정이 있었다** (§2 는 filid 만 지적). SessionStart 훅이 `CLAUDE.md` 에 직접 병합하므로, MCP 가 `AGENTS.md` 에 쓴 뒤 훅이 두 번째 사본을 만든다 → 훅이 **섹션이 이미 있는 파일을 따라가도록** 수정.
>
> **남은 것: 호스트 실측** (TODO §"유예된 실측(M2)" — M2-5 · M2-6). `codex debug prompt-input` 에 병합분이 실리는지, agy 의 지침 파일이 무엇인지.
>
> 아래는 **당시 근거 기록**이며 재실행 대상이 아니다.

---

> **원 상태: 미착수 — 수정 방향 기록 (2026-07-15).** [stage4-host-paths.md](./stage4-host-paths.md)(경로 좌표 — 코드는 main `77825966` 에 반영됨)의 **후속 델타**다. 이 문서는 그 작업에 **없던 항목** 하나만 다룬다: G8 게이트를 그 뒤에 닫았기 때문이다.
>
...
경로 좌표(`pluginRoot()`·`projectRoot()`)는 `@ogham/cross-platform` 의 `hostPaths` 로 **코드상** 정리됐다(호스트 실측은 TODO V1–V4 에서 별도로 닫는다). **이 문서가 다루는 것은 "쓰기 대상 파일의 이름·위치가 호스트마다 다르다" 는 축이다.**
...
> ⚠ **`hostPaths` 에 손대기 전에 확인할 것 (TODO V3 와 같은 지점)**: 현재 `pluginRoot()` 는
> `detectHost() === "codex" ? process.cwd() : null` 이라 **agy 에서 `null` 을 반환한다.**
> 이 문서의 채널 분기를 얹기 전에 그 분기를 먼저 정리하는 편이 안전하다 — 안 그러면
> agy 분기를 두 번 손대게 된다.
