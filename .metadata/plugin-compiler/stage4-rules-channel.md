...
> **작성 2026-07-15.** [stage4-host-paths.md](./stage4-host-paths.md)(경로 좌표 — 코드는 main `77825966` 에 반영됨)의 **후속 델타**다. 이 문서는 그 작업에 **없던 항목** 하나만 다룬다: G8 게이트를 그 뒤에 닫았기 때문이다.
>
> **자립 문서** — 실측 근거를 여기에 다 담았으므로 다른 문서를 읽지 않아도 착수할 수 있다.
>
> ⚠ **작업 위치: 이 브랜치(`feature/issue-78-1`)** — 같은 `hostPaths` 헬퍼를 건드리고, 그 헬퍼의 호스트 실측 재검증(TODO 의 V1–V4)이 선행되기 때문이다. 검증 하네스가 이 브랜치에 있다.
> **선행 조건: TODO 의 M1(main 머지 + 어댑터 재생성) → V1–V4(경로 재검증).**
