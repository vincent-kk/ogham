...
| 플러그인 자기 파일 해석 (r-statistics `contract.R`)            | 🟡 `CLAUDE_PLUGIN_ROOT` 부재 → 폴백 경로 미존재로 `run_r` 파손. **`process.cwd()` 로 분기하면 해결**(§9-A) | 🟡 미실측 (G4)                                         |
| 사용자 프로젝트 경로 (r-statistics data-root · deilen 프로젝트 식별) | 🔴 **복구 불가 — 모델이 인자로 넘기는 수밖에 없다** (env·roots·cwd 전부 없음, §9-B) | 🟡 미실측 (agy MCP 스키마에 `cwd` 없음 — G4)           |
