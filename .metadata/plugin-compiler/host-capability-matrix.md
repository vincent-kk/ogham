...
| --- | --- | --- | --- |
| **L1. agy PreInvocation injectSteps 미렌더** (agy 1.1.2, F4) | 어댑터는 완성·검증됨(`01d1ca98`). **게이팅 훅(D1b, PreToolUse `decision`)** 은 별도 채널이라 동작 가능 — 이걸로 filid 구조가드·maencof 레이어가드 강제를 우회 확보 시도. | **컨텍스트 주입 훅(SessionStart·UserPromptSubmit)은 agy 에서 무동작** — agy 가 injectSteps 를 렌더할 때까지. agy 업스트림 이슈로 추적. |
