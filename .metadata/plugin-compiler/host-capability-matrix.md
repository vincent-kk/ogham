...
- 플러그인 MCP 서버 프로세스의 env 는 어댑터가 넣은 `OGHAM_HOST` **단 하나**다(실측: `ps eww`). Codex 는 세션 cwd·워크스페이스 경로를 **어떤 형태로도 주지 않는다**(훅 프로세스와 달리 `CLAUDE_PLUGIN_ROOT`·`PLUGIN_DATA` 도 없음) → `process.cwd()` 로 사용자 상대 경로를 푸는 런타임(deilen)은 Codex 에서 **플러그인 루트 기준으로 오해석한다**(§6 G7 · §7).
...
7. ~~**G7 — `process.cwd()` 의존 런타임 감사**~~ → **닫힘 (2026-07-15) — 제약 확정**. `cwd:"."` 고정으로 MCP 서버 cwd = **플러그인 설치 루트**(lsof 확인). 그리고 Codex 는 MCP 프로세스에 **세션 cwd 를 알려줄 어떤 env 도 주지 않는다**(env = `OGHAM_HOST` 뿐). ⇒ deilen `preview`/`render_viewer` 처럼 사용자 상대 경로를 `process.cwd()` 로 푸는 도구는 Codex 에서 **플러그인 루트 기준으로 오해석**한다. 대응(모델에 절대경로 강제 / MCP roots 프로토콜 활용 / 도구 인자로 cwd 수신)은 **미결정 — Stage 4 결정 대상**.
...
| 세션 상대 경로 해석 (deilen `preview`)                        | 🔴 **오해석 확정** — MCP cwd=플러그인 루트, 세션 cwd 를 알 방법 없음 (G7)   | 🟡 미실측 (agy MCP 스키마에 `cwd` 없음 — G4)           |
