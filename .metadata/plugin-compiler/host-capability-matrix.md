...
이벤트 매핑(프로파일 번역표, `fallback` 기반):

- `SessionStart(컨텍스트 주입)` → `PreInvocation` + **세션당 1회 가드(런타임 러너 어댑터, Stage D)**. 가드 없이는 매 모델 호출마다 재실행되어 컨텍스트 반복 주입 — 구조는 emit 하되 once-guard 는 필수 잔여.
- `UserPromptSubmit` → `PreInvocation`(매 모델 호출마다 — 의미 확장, 경량 페이로드 전제).
- **`SessionEnd` → drop(재배선 아님)**. agy `Stop` 은 **대화 종료가 아니라 매 실행 루프(턴) 종료마다** 발화하므로 SessionEnd 로 매핑하면 무거운 정리/커밋/recap 을 **매 턴 실행**하게 된다 — `Stop` 으로는 "세션당 1회" 의미를 만들 수 없다. 따라서 agy 훅에서 제거하고 **MCP 서버 기동 시 stale-sweep**(다음 세션 시점)으로 보상. recap/commit 형(maencof)은 등가 보상 불가 = 손실. `fallback: stop` 은 진짜 경량 턴-종료 작업에 한해 opt-in 으로만 `Stop` 사용.
- `PreToolUse`/`PostToolUse` → 동명 agy 이벤트(grouped), matcher 를 agy 도구명으로 번역(`Read|Write|Edit` → `view_file|write_to_file|replace_file_content` — 정확 어휘는 Stage 1 실측).
- `SubagentStart` → 대응 없음(drop).

emitter 는 이 드롭/재배선을 **경고(hook-loss)로 표면화**해 조용한 손실을 막는다.
...
| SessionEnd 정리/recap (filid·maencof·imbas)             | 🟡 MCP 서버 기동 시 sweep (다음 세션 시점으로 지연)       | 🟡 **agy 훅에서 드롭 + MCP-기동 sweep** (Stop 은 매 턴 발화라 무거운 SessionEnd 부적합). recap/commit(maencof)은 손실 |
