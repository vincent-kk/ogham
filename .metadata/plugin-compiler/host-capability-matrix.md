...
2. ~~**G2 — Codex 훅 exec 헤드리스**~~ → **닫힘 (2026-07-15)**. **신뢰된 훅은 헤드리스 `codex exec` 에서 발화한다**(ponytail 실측 — stderr `hook: SessionStart` / `hook: … Completed`). **미신뢰 훅은 조용히 건너뛴다** — exit 0 완주, 경고 없음, 프로세스 미스폰. ⇒ **cennad 의 codex 위임 경로는 trust 승인 전까지 ogham 훅이 무동작이고 아무도 알려주지 않는다.** 우회로: `codex exec --dangerously-bypass-hook-trust`.
...
4. **G4 — agy MCP 상대 args 해석 기준** (§4.4). ⚠ 이 문서의 agy 사실은 **1.1.1**(2026-07-11) 실측인데 로컬 agy 는 **1.1.2** 다 — 승계하지 말고 1.1.2 로 재실측한다(§8).
5. **G5 — agy 의 `hooks/hooks.json` 자동 로드 오독** (§4.3) — 오독 시 훅 파일 개명으로 회피. G4 와 같은 이유로 1.1.2 재실측 대상.
...
| 플러그인 자기 파일 해석 — `CLAUDE_PLUGIN_ROOT` (6 플러그인 / 7 지점) | 🟡 env 부재 → 폴백. r-statistics `contract.R` 은 폴백 경로가 없어 **`run_r` 파손**, filid `syncRuleDocs` 는 **조용히 skip**. `process.cwd()` 분기로 해결 (§9-A) | 🟡 미실측 (G4)                                         |
| 사용자 프로젝트 경로 — `process.cwd()` (**8 플러그인 / 31 지점**)    | 🔴 **전부 플러그인 루트를 본다.** imbas(15) 사실상 전 기능·cennad 위임 spawn·r-statistics allow-root·deilen 프로젝트 격리. 서버 안에서 복구 불가 — **모델이 인자로 넘기는 수밖에 없다** (§9-B) | 🟡 미실측 (agy MCP 스키마에 `cwd` 없음 — G4)           |
...
- ✅ **실측 확정 (2026-07-11, 로컬 — agy 축 유지)**: agy plugin CLI 5종/컴포넌트 validate/설치 위치/hooks.json named-group 5이벤트/`--print` MCP 미기동, Codex 0.144.1 의 `mcp__<server>.<tool>` 도구명·args 미전개. ⚠ **이 agy 사실은 agy 1.1.1 기준** — 2026-07-14 로컬 agy 는 **1.1.2** 로 마이너 드리프트가 있다. agy 축 게이트(G4·G5)는 1.1.2 로 재실측한다.
...
- ⚠️ **미실측**: §6 게이트 **G2·G3**(Codex 훅 — 훅 보유 플러그인 미설치), **G4·G5**(agy 축 — 1.1.2 로 재실측), **G8**(Codex 규칙 채널). agy 인터랙티브 MCP·도구명·agents 스폰.
