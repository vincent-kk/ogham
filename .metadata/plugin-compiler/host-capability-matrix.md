...
| ------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- |
| **L1. agy PreInvocation injectSteps 미렌더** (agy 1.1.2, F4)       | **게이팅(D1b) 우회 확보 — 실측 완료(`85fea062`)**: agy 는 PreToolUse `{decision:deny}` 를 **강제**(probe view_file deny 시 모델 우회). maencof Layer1·filid 문서계약 **차단**이 agy 에서 강제(번역 완료, emitter 대기). | **주입 훅·권고 가드(구조경고·vault redirect)는 agy 무동작** — PreToolUse 엔 주입 채널 없음(차단만 이식). **--print 는 `workspacePaths` 미주입 → 가드 no-op**, 대화형 주입 미검증(stage5 F6). |
...
| **L3. Codex `agents` 컴포넌트 부재** (Codex 매니페스트 스키마, E4) | subagent 위원회 의존 스킬(filid Phase D·prawf·atlassian 미디어)에 **단일 에이전트 폴백** 정의 또는 별도 설치 안내.                                                                                                      | Codex 에서 **다중 subagent 위원회는 미이식** — 위원회 스킬은 단일 에이전트로 격하되거나 별도 도구 필요.                                                                                      |
