...
| #      | 결과                                                                                                                                                                                                                                                                                                                                                                                       |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **G2** | ✅ **닫힘**. **신뢰된 훅은 `codex exec` 헤드리스에서 발화한다** (ponytail 의 SessionStart·UserPromptSubmit 실측 — stderr 에 `hook: <Event>` / `hook: <Event> Completed`). **미신뢰 훅은 조용히 건너뛴다** — 세션은 정상 완주(exit 0)하고 **경고 한 줄 없으며** 훅 프로세스도 안 뜬다. ⇒ **cennad 의 codex 위임 경로는 훅 trust 승인 전까지 ogham 훅이 돌지 않고, 사용자는 그 사실을 알 수 없다.** 자동화용 우회로로 `codex exec --dangerously-bypass-hook-trust` 가 있다. |
