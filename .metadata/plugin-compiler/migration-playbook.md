...
- **언제 실행하나**: `.claude-plugin/plugin.json`·`.mcp.json`·`hooks/hooks.json`·`skills/` 존재 여부가 바뀌는 모든 변경 후, 그리고 릴리즈 전. 버전 필드는 `version:sync`(`scripts/injectVersion.mjs`)가 `.claude-plugin` 과 `.codex-plugin` 을 함께 동기화한다.
...
1. ~~injectVersion.mjs 컷오버 파손~~ → in-place 라 컷오버 없음. 대신 `.codex-plugin/plugin.json` version 동기화가 injectVersion.mjs 에 추가됨 — 플러그인에 `.codex-plugin` 이 없으면 조용히 skip 하는지 확인.
