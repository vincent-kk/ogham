...
| **Codex 완성도 심화**                | 🟡 **진행 중** — §1 read추적 ✅·§3 cwd감사 ✅. **§2 페르소나 이식**: 검증 3종 ✅(MCP-subagent·스킬발견=**replace**·self-load 파일읽기 → 변이 아키텍처 전면 검증, `18d7fc7a`·`df70b4e3`·`69783f5e`) · **컴파일러 구현 남음** → [handoff-codex-skill-variants.md](./handoff-codex-skill-variants.md). agy 업스트림 대기 |
| **C4-3 상태 디렉터리** (호스트 인지) | ✅ **완료**(`055aafdf`) — `pluginCache()` host-aware(claude 바이트 동일·codex=`CODEX_HOME ?? ~/.codex`), deilen·r-statistics 통합, errorLog·imbas 훅=claude(구조적). fail-first + 전체 4441 통과. 정본 [c4-3-state-directory.md](./c4-3-state-directory.md)                                                           |
...
- **검증 3종 완료 ✅** ([stage6](./stage6-codex-multiagent.md)): (a) MCP-subagent 접근, (b) 스킬발견=**replace**(매니페스트 `skills` 만 로드→변이 dir 로 Claude `./skills/` 충돌없이 shadow), (c) **self-load 파일읽기**(subagent 가 별도 `agents/<id>.md` 읽고 채택 — 마커로 증명). ⇒ **변이 아키텍처 전면 검증**.
- **남은 구현 (다음 세션)**: (1) 컴파일러 `buildCodexSkills` — `subagent_type` 스폰을 self-load 스폰(`Read agents/<id>.md`)으로 재작성, `.codex-plugin/skills/`(전 스킬 copy-all + 변이) 방출, Codex 매니페스트 `skills`→`.codex-plugin/skills/` 재지정. (2) PluginFacts 확장(agents/skills 읽기) + `package.json:files` + adapters:check baseline + 플레이북. (3) 파일럿 filid 크로스리뷰(순수추론) → prawf(무변환 확인) → entrez(하드 MCP). **Claude `skills/` 바이트 불변** 필수.
