...
브랜치 `feature/issue-78-1`, 커밋 5개:

- `a3a74d78` 문서 재편(in-place 채택) · `f3ba81f2` 도구 재작성 · `2d87f58d` 배선(스크립트·inject-version·files) · `3abb3bbe` 어댑터 21파일 · `23d251e9` CLI 분리+스펙 97개.
- 어댑터 4종 생성·커밋됨: 플러그인별 `.codex-plugin/plugin.json`(×10)·`mcp_config.json`(×9), 루트 `.agents/plugins/marketplace.json`·`.agents/plugins.json`.
- 게이트 확인: `yarn plugin:adapters:check` 통과(21 unchanged), 루트 `yarn typecheck`·`yarn test:run` 에 도구 편입 완료.
...
```bash
codex plugin marketplace add /Users/Vincent/Workspace/ogham
...
- [ ] **G4** agy 인터랙티브에서 `mcp_config.json` 상대 args 기동 (실패 시 emitter 를 절대경로 주입으로 교체)
...
- [ ] CI 에 `yarn plugin:adapters:check` 편입 + `ci.yml` paths 에 `.codex-plugin/**`·`**/mcp_config.json`·`.agents/**` 추가
...
- Codex 는 훅 파일 해시로 trust 를 고정(config.toml `[hooks.state]`) — 훅 파일이 바뀌면 `/hooks` 재승인 필요할 수 있음(G3 에서 확정).
