...
| atlassian    | —                                                          | tools      | 6      | 3      | ★★                                        |
| maencof-lens | SessionStart                                               | tools      | 3      | 1      | ★★★ (훅 1)                                |
...
| maencof      | SessionStart, UPS, PreToolUse, **PostToolUse**, SessionEnd | tools      | 28     | 5      | ★★★★★ (훅 5 + 최대 스킬)                  |
| filid        | SessionStart, PreToolUse, SubagentStart, UPS, SessionEnd   | tools      | 20     | 14     | ★★★★★ (L2 PoC 대상)                       |

- **서버명 전역 충돌 (Codex)**: MCP 보유 9개 플러그인이 모두 서버명 `tools` — 스코프 없는 Codex 도구명(`mcp__tools.*`)에서 **동시 설치 즉시 충돌**. 서버명=플러그인명 오버라이드 결정의 수치 근거.
...
구성: MCP(server **`tools`**) + skills 19 + **agents 14** + **hooks 5**. 모든 구조 차이를 동시에 노출.
...
- server `tools` (전 플러그인 공통) — 스코프 없는 Codex 도구명 충돌에 대한 서버명=플러그인명 오버라이드 검증.
...
- **maencof-lens**(최소 L2: 1 hook·1 agent) — Claude 등가 통과. Codex: 훅 미생성 + `researcher.toml`(`sandbox_mode` 를 tools 에서 유도). agy: `hooks.json` named-group(SessionStart→PreInvocation, cwd-상대 커맨드), agent tool 그랜트 `mcp_tools_*` 재매핑.
...
- 잔여 캐비엇: filid `cross-review/reference.md` 가 Claude 도구명 규약 자체(bare `mcp__plugin_filid_tools__`)를 문서화 — 실제 호출 아니라 tool 이름이 없어 토큰화 대상 밖, codex/agy 에서 문서 텍스트로 잔존(기능 무해).
