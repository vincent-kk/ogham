...
ISSUE-1~5 수정 완료(ISSUE-6은 ISSUE-1의 증상이라 자동 해소):

- **ISSUE-1**: agent grant를 full-form으로 교정 + **MCP 도구명을 kebab-case로 전면 rename**
  (`run-r`/`get-r-job`/`cancel-r-job`/`assert-analysis-plan`). createServer 등록 + buildMcpServer
  이름 가드 + agent grant + 전 문서 동기화. (위 부록의 snake 도구명은 수정 전 관측값)
- **ISSUE-2**: 미등록 기법 → `unregistered_technique` soft 경고(과거 `ok` 무검증 통과 제거).
- **ISSUE-4**: `gam`/`spline_regression`/`ancova`/`cmh` 룰셋+meta.yaml 등록, ANCOVA용 신규
  AssumptionId `homogeneity_of_slopes` 추가, statistician 결정트리 갱신.
- **ISSUE-3**: contract.R가 jsonlite 부재 시 명확히 `stop`, CSV는 `utils::read.csv`(선언 encoding
  존중), "Unknown data ref" 메시지 개선.
- **ISSUE-5**: r-setup에 R 패키지 점검/설치 단계 + `--packages` 플래그 추가.

검증: typecheck · test(102 passed) · build(kebab 이름 가드 통과). 런타임 반영은 플러그인 캐시
재동기 + 세션 reload 필요.
