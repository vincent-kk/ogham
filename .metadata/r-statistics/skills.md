...
- `meta.yaml`: `{ technique, family, outcome_types, required_assumptions:[{id, severity:hard|soft}], required_artifacts, packages }` → `assert-analysis-plan`이 소비.
...
- ⚠️ 설치 실행은 **동의 게이트** 필수(시스템 변경=비가역) + 알려진 패키지매니저 명령만. `run-r`과 분리 채널.
