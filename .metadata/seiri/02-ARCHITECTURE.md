...
| 계층      | seiri                                   | 비고                                        |
| --------- | --------------------------------------- | ------------------------------------------- |
| **Hook**  | 4개 — 등록 3(SessionStart · PostToolUse · SubagentStart) + dormant 1(InstructionsLoaded) | filid도 4개. **차단 훅 없음**(P2) |
...
│
├── hooks/
│   ├── hooks.json               # SessionStart · PostToolUse · SubagentStart — InstructionsLoaded는 dormant(미등록)
│   └── INTENT.md
│
├── scripts/
...
| **훅**                    | **4개** — 등록 3 + dormant 1 | 2→4 개정(2026-07-24). 차단 훅 없음 불변 |
...
| **PostToolUse** (`Bash`) | `bridge/post-tool-use.mjs`       | 같은 명령 연쇄 실패 → trace-cause 신호 1줄             | **standard↑** (advisory 침묵) | 등록                                  |
...
**문구 강도 조절 폐지와의 경계**: 폐지된 것(헌법 P4)은 규칙 *어휘*(MUST/SHOULD)의 강도 조절 — 배포 문서와 주입의 이중 강도 문제입니다. 다이얼 축은 규칙 본문을 재진술하지 않고 **블록·라우팅·신호의 유무**만 바꾸므로 그 폐지와 충돌하지 않습니다.
...
**v1은 전부 무조건 로드**로 시작했고, **2026-07-24 조건부 로드 도입을 확정**했습니다(S3 `test-validity` 1순위 — 10이슈 A/B의 강모델 순비용 2.8× 실측이 트리거). frontmatter는 배포 시점 주입이 아니라 **템플릿 자체에 포함**합니다 — templateHash가 raw 바이트 해시라 배포 시 주입하면 드리프트 감지가 깨집니다. 키는 `paths:`(위 자체 실측) vs `globs:`(커뮤니티는 paths 불량 보고 — anthropics/claude-code#17204·#22170) 중 배포 전 스모크 1회로 확정합니다.
