...
이유: 이 표의 나머지 항목은 전부 **상시 컨텍스트 비용**을 재는데, `src/` 줄 수만은 그렇지 않습니다. 도구 스키마·훅 렌더·스킬 description 은 매 세션 모델이 지불하지만, `src/` 는 esbuild 번들로만 존재하고 모델 컨텍스트에 들어가지 않습니다. 줄 수가 실제로 위험한 지점은 이미 별도 게이트가 지킵니다 — 훅 번들 바이트 캡, 스킬 2KB, 규칙 200줄. 대형화(R4)의 진짜 신호는 코드량이 아니라 **도구·훅·에이전트 개수와 역할 밖 목록 위반**이고, 그 셋은 위 표에 그대로 남습니다.
