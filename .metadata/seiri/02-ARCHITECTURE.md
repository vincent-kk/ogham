...
filid `src/hooks/userPromptSubmit`가 같은 원칙을 명시합니다 — *"배포된 규칙 문서는 하니스가 자동 주입하므로, 이 모듈은 그 내용을 복제해서는 안 되고 위치만 가리킨다."*
...
| **Skill** | 4개 — setup · brainstorm · interview · debug | |
...
│   │   ├── INTENT.md
│   │   ├── infra/
│   │   │   ├── configLoader/    # 설정 읽기·쓰기 (활성 규칙·다이얼)
│   │   │   ├── cacheManager/    # 세션 플래그
│   │   │   └── projectHash/     # 저장소+워크트리 키
│   │   └── rules/
│   │       ├── manifestLoader/  # manifest.json 파싱·검증
│   │       ├── ruleSync/        # 배포·제거·diff 생성
│   │       └── hashCompare/     # templateHash 대조 (드리프트)
│   ├── mcp/
│   │   ├── INTENT.md
│   │   ├── server/              # MCP 서버 조립
│   │   ├── serverEntry/         # 진입점
...
│
├── skills/
│   ├── setup/SKILL.md           # 사용자 전용 — 배포·해제
│   ├── brainstorm/SKILL.md      # 사용자 전용
│   ├── interview/SKILL.md       # 사용자 전용
│   └── debug/SKILL.md           # 자동
│
...
```json
{
  ...
  "license": "MIT",
  "keywords": ["claude-code", "plugin", "code-quality", "agent-legible", "rules"],
  "skills": "./skills/",
  "mcpServers": "./.mcp.json"
}
```

**`agents` 필드를 넣지 않습니다** — seiri는 에이전트를 갖지 않습니다.

> ⚠️ `package.json`의 `repository.directory`가 현재 `plugins/filid`입니다 — 초기화 시 복사된 값. **`plugins/seiri`로 수정 필요.**
...
| 규칙 | 5~9개, 각 **200줄 미만** | 공식: 초과 시 *"reduce adherence"* |
| 스킬 | 각 ≤2KB | 선례의 1,214줄 스킬이 반면교사 |
| `src/` | **≤1,500줄** (테스트 제외) | filid는 43파일·119심볼. seiri는 AST·메트릭 계층 없음 |
| 에이전트 | **0개** | |
...
| 상태 | 계약 | 강제 수단 |
|---|---|---|
| 자동 (3·4) | *"자동 발동되었다. 질문하지 않는다. 판단이 필요하면 보수적 기본값을 택하고 한 줄로 알린다."* | `disallowed-tools: AskUserQuestion` (턴 단위) |
| 호출 (5) | *"필요한 만큼 질문한다. 사용자가 그만 물으라 하면 산출물만 남기고 멈춘다."* | 거부권 조항 |
...
| **문서 트림 제안** | `/doctor` — *"코드에서 유도 가능한 것은 자르고, 함정·근거·관례는 남긴다"* |
