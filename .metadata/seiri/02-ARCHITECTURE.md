...
`advisory`(기본) / `standard` / `strict`. **주입 렌더와 세션 훅 신호만 바꾸고 배포된 규칙 문서는 바꾸지 않습니다** — 문서를 바꾸면 templateHash 드리프트 감지와 충돌합니다(P4).
...
유효 다이얼 = `runtime ?? config ?? advisory`. 훅은 매 실행 시 재계산하므로 변경 즉시 반영됩니다. 런타임 값이 살아 있으면 렌더가 출처를 명시합니다 — 예: `Intervention: advisory (runtime; baseline: standard)`. **조용한 오버라이드는 없습니다.** 워크트리마다 프로젝트 디렉토리가 다르므로 런타임 값은 자연히 워크트리 단위로 분리됩니다.
...
**다이얼 = 조건부 주입 전체의 총 밸브**(선제 배선 원칙). advisory는 조건부 주입 전부가 침묵하는 기준선입니다 — 디스패치 프록시 실측(2026-07-23, 발화 33/35)이 검증한 상태와 동일합니다. 발동 폭 축은 발화를 "만들지" 않습니다 — 실측이 기준선 발화를 확인했으므로, 축은 그 기준선에서 경계 순간을 넓히고 좁힐 뿐입니다.
