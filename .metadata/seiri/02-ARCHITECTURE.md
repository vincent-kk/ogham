...
| 항목                      | 목표                                      | 근거                                                                                 |
| ------------------------- | ----------------------------------------- | ------------------------------------------------------------------------------------ |
| **상시 주입**             | 훅 렌더 기본 **~5줄** · strict ≤8줄       | 규칙 본문은 하니스 몫                                                                |
| **MCP 도구**              | **3개 이하** — 현 2, config는 action 흡수 | 스키마가 상시 컨텍스트 비용                                                          |
| **훅**                    | **번들 5** — 등록 4 + dormant 1           | 2→5 개정(2026-07-24, UserPromptSubmit 추가). 차단 훅 없음 불변                       |
...
확장(2026-07-23)의 근거이고, 여기에 `trace-structure`가 더해져 현재 **12종**입니다.
...
**UserPromptSubmit은 상태가 아니라 _주의_를 나릅니다** (2026-07-24 추가). 원안은 이 훅을 뺐으나(_"상태는 세션 중 거의 안 바뀐다"_) 그 근거는 상태 재주입 용도에만 성립합니다 — 매 턴 리마인더가 나르는 것은 상태가 아니라 스킬 발동·규칙 준수 상기라, 긴 세션·컴팩션으로 흐려지는 주의를 되살립니다. SessionStart(세션당 1회)가 못 잡는 세션 중 드리프트를 매 턴 커버하며, 정적 한 줄이라 지연은 무시 가능합니다. 여전히 비차단·다이얼 게이팅(advisory 침묵)이라 아래 선제 배선 원칙 안에 있습니다. 세션 중 다이얼 조작은 MCP `config` action 경유입니다.

**선제 배선 원칙(2026-07-24, 사용자 결정)**: 디스패치 프록시 실측·10이슈 A/B가 검증한 것은 소컨텍스트 정합까지이고, 세션 신호 훅이 겨냥하는 대컨텍스트·다유혹 조건(긴 세션·컴팩션 후·연쇄 실패·위임)의 실측은 긴 메인 세션이 필요해 비용이 금지적입니다. 실측을 기다리지 않고 선제 배선하되 전부 다이얼로 게이팅합니다 — advisory = 조건부 주입 전부 침묵(프록시 실측이 검증한 기준선), 훅 지연 비용은 수용 판정.
...
2026-07-24 등록 해제 — 관측 로그가 소비처 없는 상시 부작용이었습니다(정본 `src/hooks/instructionsLoaded/INTENT.md`). 코드·번들은 존치하며, 재측정이 필요할 때(서브에이전트 규칙 도달·컴팩션 생존·조건부 로드 검증) `hooks.json` 블록을 복원합니다.
...
**3번째 도구는 만들지 않습니다** — 다이얼 조회·변경은 absorb-first 규칙(tools INTENT)에 따라 `rule_docs_sync`의 action으로 흡수합니다(2026-07-24, 구 스케치의 `config_get` 대체). 기준선 쓰기는 여전히 setup 표면 전용입니다.

**코드 분석·검색 도구는 두지 않습니다** — 하니스가 이미 갖고 있습니다. 도구 추가는 규모 목표를 넘기는 결정입니다.

## 다이얼

`advisory`(opt-out floor) / `standard`(기본) / `strict`. **주입 렌더와 세션 훅 신호만 바꾸고 배포된 규칙 문서는 바꾸지 않습니다** — 문서를 바꾸면 templateHash 드리프트 감지와 충돌합니다(P4). **기본값은 `standard`**(2026-07-24) — seiri를 켜면 두 번째 결정 없이 workflow chain이 켜지고, 불용 시 `advisory`로 내려 opt-out합니다.
...
**다이얼 = 조건부 주입 전체의 총 밸브**(선제 배선 원칙). **기본값은 `standard`**이고, advisory는 조건부 주입 전부가 침묵하는 **바닥(opt-out floor)** — 불용 시 여기로 내립니다. 이 침묵 상태가 디스패치 프록시 실측(2026-07-23, 발화 33/35)이 검증한 무비용 기준선과 동일합니다. 발동 폭 축은 발화를 "만들지" 않습니다 — 실측이 기준선 발화를 확인했으므로, 축은 그 기준선에서 경계 순간을 넓히고 좁힐 뿐입니다.

**문구 강도 조절 폐지와의 경계**: 폐지된 것(헌법 P4)은 규칙 _어휘_(MUST/SHOULD)의 강도 조절 — 배포 문서와 주입의 이중 강도 문제입니다. 다이얼 축은 규칙 본문을 재진술하지 않고 **블록·라우팅·신호의 유무**만 바꾸므로 그 폐지와 충돌하지 않습니다.
...
### 조건부 로드 — 파서 실측 (2026-07-24, Claude Code 2.1.218)
...
**v1은 전부 무조건 로드**로 시작했고, **2026-07-24 조건부 로드 도입을 확정**했습니다(S3 `test-validity` 1순위 — 10이슈 A/B의 강모델 순비용 2.8× 실측이 트리거). frontmatter는 배포 시점 주입이 아니라 **템플릿 자체에 포함**합니다 — templateHash가 raw 바이트 해시라 배포 시 주입하면 드리프트 감지가 깨집니다.
