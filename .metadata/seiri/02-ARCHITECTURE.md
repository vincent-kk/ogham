...
| 계층      | seiri                                                       | 비고                                        |
| --------- | ----------------------------------------------------------- | ------------------------------------------- |
| **Hook**  | 번들 4 — 등록 3(4개 이벤트) + dormant 1(InstructionsLoaded) | filid도 4개. **차단 훅 없음**(P2)           |
...
│
├── hooks/
│   ├── hooks.json               # SessionStart · PostToolUse(+Failure) · SubagentStart — InstructionsLoaded는 dormant(미등록)
│   └── INTENT.md
│
├── scripts/
...
| **훅**                    | **번들 4** — 등록 3 + dormant 1           | 2→4 개정(2026-07-24). 차단 훅 없음 불변                                              |
...
**UserPromptSubmit도 없습니다.** 매 프롬프트 실행은 지연을 낳고, 상태는 세션 중 거의 바뀌지 않습니다 — 세션 중 다이얼 조작은 MCP `config` action 경유입니다.
...
| 세션 신호 훅         | **침묵**           | 활성                 | 활성                         | PostToolUse · SubagentStart |
