...
- [ ] G1: <관찰 가능한 결과 — 낯선 사람이 판정할 수 있게>
      CHECK: `<셸 명령, 그대로 실행됨>`
      EXPECT: `<조건 충족 시에만 출력되는 고정 리터럴 문자열>`
      EVIDENCE: pending
...
- **CHECK와 EXPECT는 한 쌍으로.** CHECK가 실제 결과 조건을 검사하고, 충족된 경우에만 EXPECT의 고정 문자열을 출력한다. 양쪽에서 나타나는 줄(`done`)은 증명이 아니다. EXPECT는 양끝 공백을 정리한 대소문자 구분 리터럴 부분문자열이며, 한 출력 줄 안에서만 일치한다. 슬래시·메타문자·플래그는 일반 문자이고 정규식 모드나 구형 원장 이관은 없다.
...
- **조건 검사를 CHECK 안에.** 데이터를 출력하기만 하는 명령은 결과 조건의 assertion을 덧붙인다. 실패를 기대하는 게이트도 CHECK 안에서 그 실패를 확인한 뒤 성공 마커를 출력한다.
...
**성공은 출력에 남아야 한다.** 표준형은 `<결과 조건을 검사하는 명령> && echo <MARKER>` + `EXPECT: <MARKER>`다. CHECK가 결과 조건까지 검사해 실패 시 마커를 막으므로, exit code를 못 보는 호스트에서도 판정이 같다. 조회 명령의 정상 종료만으로 내용 조건을 대신하지 않는다. CHECK가 이미 출력하는 고정 성공 문자열(`TYPECHECK_OK`)은 재사용할 수 있다. EXPECT는 그 문자열과 관측된 증거를 확인하며, 가변 값의 비교·패턴 검사는 CHECK가 소유한다.
