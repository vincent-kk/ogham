...
| **판정 관측·기록·보고** | PostToolUse / PostToolUseFailure 훅  | 명령이 어느 작업 원장의 CHECK와 일치하면 EXPECT 대조 → 원장에 박스·EVIDENCE 기록 → **판정 한 줄** 주입. 기계 출처.           |
...
| 스킬             | 추가되는 것 (요지)                                                                                                                                                                                            | 현재  | 여유  |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- | ----- |
| `write-plan`     | 작업 이름을 정하고(kebab-case, 한 줄로 밝힘) `.seiri/tasks/<name>/plan.md`에 저장 — 저장소 명시 지침이 다른 위치를 정하면 그곳에. 3단계 "명령과 기대 출력"을 **게이트로 산출** — `gates.md`, 태스크당 1~4개, Final은 지정 검증. 게이트 없는 태스크는 계획 실패. | 2,393 | 1,703 |
| `review-plan`    | 2단계에 게이트 품질 — 결과인가 활동인가, EXPECT가 결정적인가, exit 0 형인가, 숫자에 CHECK가 있는가. 게이트를 쓴 눈과 다른 눈이 본다(challenge 위임 시 원장도 인계).                                                | 3,085 | 1,011 |
| `execute`        | 1단계 `status`로 시작·재개(세션이 바뀌었어도 원장이 어디까지 왔는지 말한다). 2단계 진행 원장을 **게이트 원장**으로(회고형 → 전망형). 태스크 닫기 = 그 태스크의 게이트 met. 위임 반환 시 CHECK 재실행. "모든 태스크 완료" = `status` `all_met`. 이탈은 `abandon`. | 2,479 | 1,617 |
| `verify`         | 1단계 "증명할 명령"은 게이트의 CHECK다. 4단계 뒤: `status`가 UNMET이면 그 완료 주장은 `/seiri:execute`의 순간이다(뒷가장자리). 보고할 숫자는 재측정 또는 "unverified".                                              | 1,734 | 2,362 |
| `request-review` | 2단계 인계물에 원장 N of N과 ABANDON 목록.                                                                                                                                                                    | 1,767 | 2,329 |
| `finish`         | 1단계 검증 전에 `status`(전체) — 미충족 원장이 있으면 보고하고 멈춘다: 끝낼 것이 없다. 작업 디렉토리 정리는 권하되 하지 않는다.                                                                                     | 1,790 | 2,306 |
