...
## 다음 작업 — 실하니스 트랙 (2026-07-23 갱신)

> **완료**: 트랙 B · A-6 rule-lint(`ruleInvariants.test.ts` — 임계·러너·D8 관용구·hash + **B1·B5·B6 골격**) · 구현 평가·트랙 1 수정(커밋 `97f54dd4`) · ① 근본 재발방지 (c) 정본 단일화(커밋 `b40a648d`). 남은 건 **전부 실하니스/외부 의존** — 아래.

### ★ 실하니스 테스트 — 시나리오·방법론·관측·실행 체크리스트: **[phase0/d7-dispatch.md](./phase0/d7-dispatch.md)** (2026-07-23 신설)

> **D7 프록시 실측 완료 (2026-07-23) → [phase0/d7-results.md](./phase0/d7-results.md).** 서브에이전트 프록시로 7 시나리오×5회 실행: 자율 발화 **33/35 (6/7 시나리오 5/5)**, implement **3/5**(미발화 2건은 유능 모델의 기저 fail-first — 실패 아님, D9 관련), 체인 **implement→verify** 확인(plan-선행·request-review-후미는 규모 민감으로 미발화). 부수: 호출형 비노출 ✅ · 질문금지 0/52(단 서브에이전트 AskUserQuestion 부재 가능성으로 집행 증명은 약함). **하니스 결함 발견·수정**: 서브에이전트가 ogham cwd 상속 → 일부가 `d7-dispatch.md`를 읽고 자기-오염(9/35, plan-r1이 §6.2 인용하며 스스로 거부) → 워크스페이스 펜스로 재실행(v2 오염 **0/15**). **잔여 = 메인세션 골드스탠다드(§8)**: implement/plan 재확인 · `/context` 실로드 · 컴팩션 생존 · 질문금지 집행. 프록시는 메인세션의 충실한 근사이나 동일물은 아니므로 §8은 그대로 유효.

Vincent 로컬 설치 후 실행. 나오 몫: 시나리오 설계(완료)·판독·원장 갱신. **핵심 방법론**: 디스패치는 발화율이 지표(규칙 micro-test의 행동차이와 다름) · **발화(로드)와 행동(절차)을 분리 관측** · 무개입 처치군(프롬프트에 "스킬 써라" 금지) · 관측 1급은 세션 transcript(`*.jsonl`).

> **세션 2026-07-24 완료 — 1~4 실측 끝. 전부 [phase0/d7-results.md](./phase0/d7-results.md).** 프록시 하니스(신선 서브에이전트 + 정확 프롬프트 + transcript grep + cwd 펜스).

1. ✅ **D7 디스패치 발화율** (관문) — 6/7 시나리오 5/5, 33/35(94%). 강압 부트스트랩 없이 발화 확인, SessionStart 안내 불요. 체인 implement→verify.
2. ✅ **부수 측정** — 호출형 비노출·질문금지·규칙 `/context` 실로드(albatrion 실세션) 확인. ⏸ **컴팩션 생존만 잔여**(긴 메인 세션 필요).
3. ✅ **통합 검증(머지 게이트)** — 0 모순·무누락·경계정확(임계 filid만)·공존·orphan 은퇴·배포 바이트일치·filid 회귀無(1182 passed). **구조적 머지 안전.** 네이밍 교차-도구 seam은 수용(seiri advisory / filid enforces).
4. ✅ **10이슈 A/B (효능)** — 5이슈×as-is/to-be×3, shown+hidden 오라클. **opus 리프트 0 (양쪽 100%) / haiku +20pp (80%→100%, 전부 reuse 차원)**. seiri = **약한 모델용 보험**, 강한 모델엔 순비용(2.8× 토큰). 프로세스 규칙(verify·fail-first) 겨냥 다단계 상황은 미검증.
5. ⏸ **S9 대조군(3표본)**: decision-trail 편입 자격(cennad).
6. ✅ **D9 규칙 분량 슬리밍 완료 (2026-07-24)** — [phase0/d9-results.md](./phase0/d9-results.md): 구조 4종(S1·S2·S4·S5) lite 재단. **2-1 성능가드**(sonnet 30런) full 15/15 = lite 15/15·**롤백 0**(4종 채택, 상시비용 -13%). **2-2 프로세스 A/B**(6런) **lift 없음**(sonnet 기본역량이 이미 원인수정·fail-first 수행). 긴 세션·컴팩션 프로세스 값은 메인세션 잔여.
