...
**Phase 0 진행 중**: S1 대조군 완료 — 유효 표본 5/5 전원 F1 실패 후보, **사용자 판독 대기** ([phase0/s1-agent-legible.md](./phase0/s1-agent-legible.md)). 판정 후 처치군부터는 이 작업환경의 에이전트가 실행합니다 — 인계는 [phase0/HANDOFF-PROMPT.md](./phase0/HANDOFF-PROMPT.md).
...
**미커밋**: `plugins/seiri/bridge/` · `plugins/seiri/public/` (빌드 산출물 — **사용자가 직접 커밋**하는 저장소 규약. AI 는 `src/`·`skills/`·문서만 커밋). **+ 07-23 스킬 확장분** — 신설 스킬 7 · 수정 스킬 3 · `budgets.ts` · 문서 3(02-ARCHITECTURE·TODO·README). 바로 아래 "지금 사용자가 할 일" 참조.

## 지금 사용자가 할 일 (07-23 S1 대조군 완료 시점)

1. **V1~V5 판독** — [phase0/s1-agent-legible.md](./phase0/s1-agent-legible.md)의 샘플 원문을 읽고
   "대조군 실패 맞다/아니다"를 판정합니다. micro-test 5단계의 "매칭된 것 전부 사람이 읽기"는
   위임이 불가능한 유일한 구간입니다(템플릿 반향과 실제 행동 변화를 사람이 가려야 함).
2. **판정 후 인계** — [phase0/HANDOFF-PROMPT.md](./phase0/HANDOFF-PROMPT.md)를 이 작업환경의
   에이전트에게 전달합니다. 처치군부터는 그 에이전트가 실행합니다(격리 cwd 규약 포함).
3. **결정 대기 4건** — Phase 0 결과가 나오면: D2(S9 `decision-trail` 편입 여부) ·
   D7 처방(발화율 미달 시 SessionStart 비강압 안내 1줄) · D5(규칙 총량 초과 시 S3 `paths:` 조건부 전환) ·
   **D8(S7·S8 ↔ 스킬 중복 해소 — 조항 단위, Phase 4b 대조표 참조)**.
4. **`agy` CLI 점검** — antigravity가 2연속 "no output" 오류로 실험에서 제외됐습니다.
   CLI 업데이트가 필요해 보입니다.
5. `bridge/`·`public/` 커밋은 여전히 사용자 몫입니다(빌드 산출물 규약). **이번 확장으로 재빌드는
   불필요** — `SHIPPED_SKILLS`는 테스트 전용이고 스킬은 번들에 들어가지 않습니다.
...
- [ ] **`filid:setup`이 이관 안내를 표시**: 이관된 4종이 목록에서 사라질 때 _"이 규칙들은 seiri로 이관되었습니다. `/seiri:setup`을 실행하세요"_ — 제거 확인 화면에 노출
...
| **A**   | 5b    | **스킬 확장 v2** — sp 절차 7종 이식 + 체이닝 (Phase 4b)                | ✅ 07-23 (미커밋)                   | A-5      |
| **A**   | **0** | **규칙 검증 + 스킬 디스패치 검증 (micro-test)**                        | ⏭ **다음 — 코드 0줄**              | —        |
