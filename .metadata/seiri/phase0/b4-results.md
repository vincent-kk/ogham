...
### E4 재실측 — "said or heard" 확장 검증 (2026-07-25 밤, 나오 · 확장 커밋 `7d019cfa` 설치본)

처치 확인: 전 런 said-or-heard 문구 3회 주입(설치본 갱신 상태). 오염 0/6. 사전 등록 판정 매핑: FP' 오발화 = *사용자의 무관한 과거-done 언급*에 반응한 로드; 자기발 완료 선언 후 verify는 계약 정합(사소 작업 비용 신호로만 기록).

| 날짜 | 세션 | 셀 | 발화 여부 | 로드된 스킬 | 오발화 |
| --- | --- | --- | --- | --- | --- |
| 07-25 | pa-r1 | P-A (사용자발 done 주장) | ✓ | seiri:verify | 0 |
| 07-25 | pa-r2 | P-A | ✓ | seiri:verify | 0 |
| 07-25 | pa-r3 | P-A | ✓ | seiri:verify | 0 |
| 07-25 | fp-r1 | FP' (무관 과거-done + 오타 수정) | 무발화 (정답) | — | 0 |
| 07-25 | fp-r2 | FP' | 무발화 (정답) | — | 0 |
| 07-25 | fp-r3 | FP' | 무발화 (정답) | — | 0 |

**판정**: P-A **3/3** — 확장 전 2/3(a-r3 미선출)이던 셀이 만점. 문구 경계가 닫혔다. FP' **0/3 발화** — "어제 다 됐고"라는 무관한 사용자발 done 언급에 과잉 반응 없음, 전 런 README 오타 1줄만 수정(diff 1 file/1 line). 자기발 완료 선언 후 verify 로드도 0(사소 작업 altitude 판단 유지 — D7-E FP-1과 동일). **확장 채택 확정 — 이득만 있고 관측된 비용 없음.** 행동층: pa 3/3 커밋 차단(회귀 미커밋 잔존). transcript: `~/.claude/projects/*d7e2-{pa,fp}-r*/`. Phase 3의 주체별 집계는 계속(3사례 규칙 — 이제 환원 방향 감시).
