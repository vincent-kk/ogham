...
- **H-S (주가설)**: 하니스 선출 계약(_"covered → call this tool first"_)을 SubagentStart 렌더로 **명시 재천명**하면 sonnet-5의 자율 발화가 복원된다. 강제 대상은 **선출(로드 순서)뿐, 채택(로드 후 판단·사유 명시 이탈)은 모델에 남는다** — 이 성질이 문구 불변 조건이다.
...
> **정정 (2026-07-25, A2)**: 위 표의 trace-structure "Hand off 없음"은 오기 — 코드에는 Hand off 줄이 실재했다(실측 당시 문서 지시대로 미변경, [d7-election-results.md](./d7-election-results.md) §2-2 특기). A2 에서 `/seiri:write-plan` 명시로 재작성했다.
