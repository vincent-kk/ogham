...
## `configScope` — user/project 설정 레이어 (2026-07-29)

플러그인 설정을 두 네임스페이스로 나눠 읽고 쓰는 계층. user 는
`pluginCache(<plugin>)/config.json`, project 는
`<projectRoot>/.<plugin>/config.json` 이며 project 가 user 를 재정의한다.
두 위치 모두 이 변경 **이전부터 저장소에 존재하던 자리**라 데이터
마이그레이션이 없다 — 플러그인마다 둘 중 하나만 쓰고 있었을 뿐이다.

```
configScope/
├── merge/    순수 문서 연산 — node import 0개
└── layers/   좌표 해석 · 파일 I/O · 상태 조립
```

`merge/` 가 순수한 것은 계약이다. 브라우저 설정 페이지 번들과 훅 번들이
파일 I/O·`env-paths` 그래프 없이 병합 규칙만 가져가야 하고,
`merge/__tests__/pureImports.test.ts` 가 이를 강제한다. 실측으로 deilen 설정
