...
| 컴포넌트           | 역할                                                                | 위치                    |
| ------------------ | ------------------------------------------------------------------- | ----------------------- |
| Claude             | 문서 생성 주체. `present` 스킬로 렌더 위임, 수거된 피드백 반영    | —                       |
| Skill `present`    | `render_viewer` 호출 → `collect_feedback` poll 루프 → 피드백 정리   | `skills/present/`       |
...
2. `present` 스킬이 `render_viewer({ content | path, title?, options? })` 호출.
