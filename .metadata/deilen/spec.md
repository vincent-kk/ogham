...
2. `preview` 스킬이 `render_viewer({ content | path, title?, options? })` 호출.
...
5. `preview` 가 `collect_feedback({ session_id, wait_seconds })` 를 호출 → 서버가 요청을 연 채 대기.
...
- Hook 기반 자동 개입 (v1 — 명시적 `preview` 호출만).
