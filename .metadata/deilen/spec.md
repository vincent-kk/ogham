...
| 컴포넌트           | 역할                                                                | 위치                    |
| ------------------ | ------------------------------------------------------------------- | ----------------------- |
| Claude             | 문서 생성 주체. `display` 스킬로 렌더 위임, 수거된 피드백 반영    | —                       |
| Skill `display`    | `render_viewer` 호출 → `collect_feedback` poll 루프 → 피드백 정리   | `skills/display/`       |
...
1. Claude 가 문서 markdown 을 생성.
2. `display` 스킬이 `render_viewer({ content | path, title?, options? })` 호출.
3. MCP 가 렌더 세션 생성 → markdown 을 source-line 매핑 HTML 로 변환 → HTTP 서버 기동(없으면) → 브라우저 오픈.
4. `render_viewer` 는 `{ session_id, url, status: "serving" }` 를 **즉시 반환**(논블로킹).
5. `display` 가 `collect_feedback({ session_id, wait_seconds })` 를 호출 → 서버가 요청을 연 채 대기.
...
9. 미제출 상태로 `wait_seconds` 경과 시 `{ status: "pending" }` 반환 → `display` 가 자동 재호출(루프).
...
- `display` SKILL.md: `status:"complete"` 일 때까지 `collect_feedback` 재호출. M 라운드 초과 시 "준비되면 알려달라"로 수동 폴백.
