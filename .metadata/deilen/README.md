...
- Agent 없음. Hook 없음 (v1). 스킬 2개: `setup`, `preview` (플러그인 prefix 미사용)
...
- raw HTML 직접 렌더(GitHub 패리티): markdown-it `html:true` + allowlist sanitizer 단일 방어층, raw 유래는 내부 class/data-\* 불허 프로파일로 정제([rendering.md](./rendering.md))
...
- 렌더러 적재: **전부 동봉 + lazy 서빙**(`public/assets/`) — MCP 런타임·브라우저 메모리엔 미포함, 비용은 설치 용량뿐
