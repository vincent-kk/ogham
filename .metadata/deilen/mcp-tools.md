...
- `status:"pending"`: 텍스트 블록 `{ status:"pending", draft_count }` — `display` 가 재호출.

알 수 없는 `session_id` → `unknown` 에러. `closed` 세션 → `closed` 에러.

settle 결과 매핑(상세 [mcp-runtime.md](./mcp-runtime.md)): `superseded`(동일 세션 collect 재등록 — `display` 직렬이라 정상 미발생, 발생 시 `pending` 처럼 재호출) / `server_closing`(서버 종료 중 → `closed` 로 반환). 호출 취소(`extra.signal` abort) 시 resolver 정리 후 반환 중단.
...
렌더 세션을 종료한다 (선택적 — `display` 가 수거 후 정리에 사용).

입력: `{ session_id: string }`.
동작: 세션을 `closed` 처리 + 미해결 resolver settle + 서버 refcount `release`. 마지막 serving 세션이면 grace 후 서버 종료(reap).
반환: `{ status: "closed" }`.
...
- HTTP 서버는 활성 세션이 있는 동안 유지. 마지막 serving 세션이 명시적으로 닫히면(submit/`close_viewer`) grace reap; read-only 탭 닫기는 idle 폴백(기본 1분) 종료.
