...
보고서를 렌더 세션으로 등록하고 뷰어를 띄운다. **즉시 반환(논블로킹).**
...
`path` 정책: 호출자(Claude)는 신뢰 fs 주체 → **임의 readable 파일 허용**. canonicalize 후 일반 파일·utf8·크기 상한(`config.max_report_mb`=5MB) 검증, 위반 시 `read_error`. `content` 도 동일 크기 상한.
...
| 필드           | 타입      | 설명                                                                                                                               |
| -------------- | --------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| `session_id`   | `string`  | `render_report` 가 반환한 ID.                                                                                                      |
...
settle 결과 매핑(상세 [mcp-runtime.md](./mcp-runtime.md)): `superseded`(동일 세션 collect 재등록 — `present` 직렬이라 정상 미발생, 발생 시 `pending` 처럼 재호출) / `server_closing`(서버 종료 중 → `closed` 로 반환). 호출 취소(`extra.signal` abort) 시 resolver 정리 후 반환 중단.
...
렌더 세션을 종료한다 (선택적 — `present` 가 수거 후 정리에 사용).

입력: `{ session_id: string }`.
동작: 세션의 뷰어 비활성화 + 미해결 resolver 를 `closed` 로 정리. 활성 세션이 0 이고 설정 UI 미사용이면 HTTP 서버 idle 종료 스케줄.
반환: `{ status: "closed" }`.
...
- 보관: `config.session_ttl_hours`(기본 72). 만료 세션 디렉토리는 다음 MCP 기동 시 정리.
- HTTP 서버는 활성 세션이 있는 동안 유지, 마지막 세션 종료 + idle 초과 시 자동 종료.
