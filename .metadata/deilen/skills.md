...
트리거(예): "present this", "이 문서 페이지로 보여줘", "브라우저로 검토하게 해줘".
...
- **채팅에 본문 재출력 금지**: present 할 문서를 길게 출력한 뒤 또 렌더하면 2배 소비. 생성→바로 `render_viewer`(또는 파일) 경로로.
...
- **plan 모드 권한 주의**: plan 모드는 승인 전 도구 사용을 제한한다. `render_viewer` 는 저장소를 수정하지 않지만(페이지 서빙만) 클라이언트 정책에 막힐 수 있으므로, 보통 **plan 확정/exit 후** present 하거나 plan 을 파일로 두고 present.
