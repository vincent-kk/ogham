...
트리거(예): "display this", "이 문서 페이지로 보여줘", "브라우저로 검토하게 해줘".
...
3. **자동 수거 루프** — `collect_feedback({ session_id, wait_seconds })` 반복:
   ...
   - `status:"pending"` → 즉시 재호출(자동). `display` 가 M 라운드(기본 5) 초과하면 "검토 끝나면 알려주세요"로 **수동 폴백** 후 대기.
...
- **채팅에 본문 재출력 금지**: display 할 문서를 길게 출력한 뒤 또 렌더하면 2배 소비. 생성→바로 `render_viewer`(또는 파일) 경로로.
...
- **plan 모드 권한 주의**: plan 모드는 승인 전 도구 사용을 제한한다. `render_viewer` 는 저장소를 수정하지 않지만(페이지 서빙만) 클라이언트 정책에 막힐 수 있으므로, 보통 **plan 확정/exit 후** display 하거나 plan 을 파일로 두고 display.
- 역할 분리: deilen 은 "표시+피드백"만. plan/기획 문서의 분해·이슈화는 imbas 영역([../imbas/BLUEPRINT.md](../imbas/BLUEPRINT.md)) — read-only plan 취급 관례 참조.

## 비채택

- 렌더 전용·수거 전용 스킬 분리(둘은 한 흐름이라 `display` 로 통합).
- Hook 기반 자동 `display` 제안(v1 — 명시 호출만; 추후 검토).
