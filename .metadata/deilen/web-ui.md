...
- 생애: 첫 `render_viewer`/`open_settings` 에서 1회 기동. serving 세션이나 collect 대기가 남아 있는 동안 유지하고, 모두 닫힌 뒤 `idle_shutdown_minutes`(기본 1분) 초과 시 종료한다. heartbeat 는 페이지 쪽 세션 상태 확인용이며, MCP 종료 시 서버도 동반 종료한다.
...
└── renderers/               # *.entry.ts (buildRenderers.mjs → bridge/assets/)
                            # + 다이어그램 확대 라이트박스(mermaid chunk 동봉)
```
...
- 다이어그램 확대: 렌더된 mermaid 블록 우상단의 확대 버튼 → 전문 라이트박스(휠·버튼 줌, 드래그 팬, Fit 복귀, `Esc`/✕ 닫기). 원본 SVG 는 문서에 남고 **복제본**을 띄우되 `id` 를 보존한다 — mermaid 가 SVG 안에 넣는 `#deilen-mermaid-N …` 스코프 스타일이 복제본에도 걸려야 하기 때문. 오버레이 `z-index` 는 제출·확인 오버레이(100) 아래(90) — 다이어그램이 결정을 가리지 않는다. 렌더 실패 블록(SVG 없음)에는 버튼을 붙이지 않는다.
- 코멘트가 달린 블록 하이라이트는 앰버(`--mark`), 작성 중(composer 열림) 블록은 더스티 로즈(`--pending`) — 크롬이 모노크롬 잉크(`--accent` = 글자색)라 두 wash 는 회색 선택과만 구분되면 된다. 둘은 박스 모델·중첩 진해짐 방지를 공유하고 같은 `<li>`/블록에 플래그를 얹어 불릿·체크박스 거터를 침범하지 않는다. 토큰·타이포 정본은 [pages/DESIGN.md](../../plugins/deilen/src/mcp/pages/DESIGN.md).
...
- 편집을 열면 열린 composer 가 그 항목을 대신하므로 **원본 카드는 목록에서 빠진다**(같은 코멘트가 두 번 보이지 않게) — 저장·취소로 composer 가 닫히면 되돌아온다. 다른 카드는 그대로 보인다.
...
`buildViewerHtml.mjs` / `buildSettingsHtml.mjs` 가 각 페이지 html+css+js 를 esbuild 로 minify·inline → `public/viewer.html`, `public/settings.html`(런타임 `loadViewerHtml`/`loadSettingsHtml` 로 `fs` 로드, 번들 미포함). 뷰어의 무거운 렌더러는 `buildRenderers.mjs` 가 독립 브라우저 엔트리로 `public/assets/` 에 빌드, `handleGetAsset` 가 서빙(번들 비포함).
...
- `/assets/<chunk>`: 빌드 매니페스트 allowlist 만 서빙 — 경로 구분자·`..`·인코딩 traversal·`public/assets` 외부 심링크 거부, 알려진 확장자만.
