...
| 메서드 | 경로                          | 동작                                                                                              |
| ------ | ----------------------------- | ------------------------------------------------------------------------------------------------- |
| GET    | `/r/<session>?token=`         | 뷰어 HTML(`reportHtml`). `__DEILEN_STATE__` 에 session_id·token·title·렌더 HTML·raw markdown 주입. |
...
서버는 sanitize+line-map HTML 을 렌더하되 **raw markdown 도 함께 노출**한다(세션 `report.md` 기반: `__DEILEN_STATE__` 동봉 또는 `GET /api/report?session=&format=md`). 파싱을 클라이언트로 옮기지 않는다.
