...
- **liveness = heartbeat**: 각 뷰어 탭이 `POST /api/ping`(주기 ~30s)으로 세션 생존을 갱신. 서버는 max(마지막 도구 호출, 마지막 ping) 기준으로 idle 판정 — 둘 다 `idle_shutdown_minutes` 초과 시 `server.close()` + 싱글톤 null(세션 active 여부 무관 → Claude 크래시·`close_viewer` 누락에도 종료). 활동 발생 시 타이머 재설정.
...
- 세션 Map: `session_ttl_hours` TTL + **상한(LRU evict)** 으로 무한 증식 차단. `close_viewer`/idle/TTL 정리.
