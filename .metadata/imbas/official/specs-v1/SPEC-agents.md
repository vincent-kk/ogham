...
> Status: Draft v1.2 (2026-07-10) — Implementation sync (bare names, maxTurns)
...
| Agent               | 역할                                              | 사용 Phase             | Model  | maxTurns |
| ------------------- | ------------------------------------------------- | ---------------------- | ------ | -------- |
| **analyst**         | 문서 정합성 검증, 역추론 검증                     | Phase 1, Phase 2(검증) | sonnet | 50       |
| **planner**         | Story 분할, 사용자 가치 기반 서술                 | Phase 2(분할)          | sonnet | 60       |
| **engineer**        | 코드 탐색, Subtask/Task 생성                      | Phase 3                | opus   | 80       |
| ~~**imbas-media**~~ | _migrated to `@ogham/atlassian` as `media` agent_ | —                      | —      | —        |

에이전트 이름은 접두사 없는 bare name (plugin 네임스페이스가 `imbas:` 를 제공).
스킬은 `subagent_type: "imbas:<agent>"` 형태로 spawn 하며, SubagentStart 훅
(agentEnforcer)이 `imbas:` 접두사를 벗겨 역할 제약을 주입한다.
...
name: planner
description: >
  Decomposes planning documents into implementation-ready issues from a product/business perspective.
  Selects the issue type per item content (Story for user value, Task for technical/chore
  work, Bug for defects — from config-available types; no blind inheritance from the source).
...
| Level 0 | **Task**  | 기술/chore 작업 — planner 는 원문 유래 기술 항목에 Task 타입 부여, engineer 는 크로스-Story 공통 Task 추출 | [컴포넌트] + 기술 작업 내용   |
...
tools:
  ...
  - Glob # 파일 패턴 탐색
  - Bash # 빌드/테스트 확인
  # imbas MCP (AST 코드 분석) — full-form grant (short-form은 서브에이전트 미해석)
...
[imbas:split skill]
    ├── spawns planner (Story 분할)
    │    └── returns: stories-manifest.json
    └── spawns analyst (역추론 검증)
