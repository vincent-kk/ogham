...
1. **Plan-then-Execute** — P1-P3은 매니페스트만 생성. 실제 provider 쓰기(Jira/GitHub/local)는 `imbas:imbas-manifest` 스킬이 수행.
...
**호출 시점:** `imbas:imbas-devplan` 스킬 또는 imbas-engineer가 AST 도구 호출 시 napi 에러 감지.
...
| digest | — | (`imbas:imbas-read-issue` 경유) getJiraIssue, addCommentToJiraIssue |
| `imbas:imbas-read-issue` | — | getJiraIssue (본문+코멘트 구조화) |
