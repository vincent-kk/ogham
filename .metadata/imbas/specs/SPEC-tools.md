...
> Status: v1.4 (2026-04-06) — Multi-provider alignment
...
| 9 | Config | `config_get` | true | Read config.json |
| 10 | Config | `config_set` | false | Update config.json fields |
| 11 | Cache | `cache_get` | true | Read issue tracker metadata cache |
| 12 | Cache | `cache_set` | false | Write issue tracker metadata cache |
| 13 | AST | `ast_search` | true | AST pattern search via @ast-grep/napi |
| 14 | AST | `ast_analyze` | true | Dependency graph / cyclomatic complexity |
| 15 | Utility | `ping` | true | Health check |
...
**Side Effects:**
- `.imbas/<KEY>/imbas:cache/<type>.json` 저장
...
**호출 시점:** `imbas:devplan` 스킬 또는 imbas-engineer가 AST 도구 호출 시 napi 에러 감지.
...
| `imbas:fetch-media` | — | getConfluencePage, fetchAtlassian |
| digest | — | (`imbas:read-issue` 경유) getJiraIssue, addCommentToJiraIssue; (`imbas:fetch-media` 경유) fetchAtlassian |
| `imbas:read-issue` | — | getJiraIssue (본문+코멘트 구조화) |
