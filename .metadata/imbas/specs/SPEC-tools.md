...
| 11 | Cache | `imbas_cache_get` | true | Read Jira metadata cache |
| 12 | Cache | `imbas_cache_set` | false | Write Jira metadata cache |
...
|-------|------|----------|-------------|
| `project_key` | string | yes | Jira 프로젝트 키 |
...
{
  "run_id": "20260404-001",
  "run_dir": ".imbas/PROJ/runs/20260404-001",
  "state": { /* RunState 전체 */ }
}
```

**Side Effects:**
- `.imbas/<KEY>/runs/<YYYYMMDD-NNN>/` 디렉토리 생성
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `project_key` | string | no | 미지정 시 config.defaults.project_key 사용 |
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `updates` | Record<string, unknown> | yes | dot-path 키-값 (예: `{ "defaults.project_key": "PROJ" }`) |
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `project_key` | string | no | 미지정 시 config default |
...
```typescript
Input: { project_key, run_id, type, item_id, updates: { status?, jira_key? } }
```
...
- [SPEC-atlassian-tools.md](./SPEC-atlassian-tools.md) — 외부 Atlassian MCP
