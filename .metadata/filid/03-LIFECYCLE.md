...
```
┌──────────┐    ┌───────────┐    ┌───────────┐    ┌──────────────────┐    ┌─────────┐    ┌───────────────┐
│ /init │───→│ /scan │───→│ /sync │───→│ /review       │───→│ /promote │───→│ /fca-query    │
│          │    │           │    │           │    │                  │    │             │    │               │
...
```
┌────────────────┐    ┌───────────────┐    ┌──────────────────┐    ┌──────────────────┐
│ /pipeline   │───→│ /review   │───→│ /resolve      │───→│ /revalidate   │
│                │    │               │    │                  │    │                  │
...
```
┌───────────┐    ┌──────────────┐    ┌─────────────┐    ┌─────────────────┐    ┌─────────────────┐    ┌──────────────┐
│ /guide     │    │ /restructure  │    │ /migrate │    │ /pull-request│    │ /update      │    │ /revalidate│
│           │    │              │    │             │    │                 │    │                 │    │ (단독 실행)   │
...
`/pipeline`으로 전체 흐름을 오케스트레이션하거나 각 단계를 개별 실행 가능.
...
│   ├── justifications.md     # /resolve 출력
│   └── re-validate.md        # /revalidate 출력
└── debt/                  # 기술 부채 파일 (전체 공유)
    └── <debt-id>.md          # 개별 부채 항목
```
