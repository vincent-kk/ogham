...
4. **무분별한 성장**: INTENT.md, DETAIL.md가 끝없이 비대해짐
...
2. **부속품(Organ) 격리**: 공유 유틸리티를 리프 레벨로 격리 (INTENT.md 금지)
...
| INTENT.md 50줄 제한      | `validateIntentMd()`                      | `core/document-validator.ts`                               |
| 3-tier 경계 시스템       | `ThreeTierBoundary` 검증                  | `core/document-validator.ts`                               |
| DETAIL.md append-only 금지 | `detectAppendOnly()` + `validateDetailMd()` | `core/document-validator.ts`                             |
| Organ INTENT.md 금지     | `guardStructure()`                        | `hooks/structure-guard.ts`                                 |
...
│                                                                    │
│  ┌─────────────┐    ┌────────────────┐    ┌───────────────────┐  │
│  │ Write/Edit   │───→│  PreToolUse    │───→│ pre-tool-use      │  │
│  │ Read Tool    │    │    Hook         │    │ intent-injector   │  │
│  └─────────────┘    └────────────────┘    └───────┬───────────┘  │
│        │                                          │ pass/block    │
│        ▼                                          ▼               │
│  ┌─────────────┐    ┌────────────────┐                           │
...
│  │  qa-reviewer (RO) │ code-surgeon │ drift-analyzer │ restructurer│ │
│  └──────────────────────────────────────────────────────────────┘ │
│                                                                    │
│  ┌──────────────────────────────────────────────────────────────┐ │
│  │                     Skills (16 명령)                          │ │
│  │  /fca-init │ /fca-scan │ /fca-sync │ /fca-review │ /fca-promote│ │
│  │  /fca-guide │ /fca-restructure │ /fca-resolve │ /fca-revalidate│ │
│  │  /fca-migrate │ /fca-pipeline │ /fca-pull-request │ /fca-update│ │
│  │  /fca-context-query │ /fca-structure-review │ /fca-ast-fallback│ │
│  └──────────────────────────────────────────────────────────────┘ │
└──────────────────────────────────────────────────────────────────┘
```
...
├── .mcp.json                    # MCP 서버 등록 (command: node)
├── agents/                      # 에이전트 정의 (7개)
│   ├── fractal-architect.md     # 읽기 전용 설계자
│   ├── implementer.md           # DETAIL 범위 구현자
│   ├── context-manager.md       # 문서 관리자
│   ├── qa-reviewer.md           # 읽기 전용 QA
│   ├── code-surgeon.md          # 코드 수술자
│   ├── drift-analyzer.md        # 드리프트 분석자
│   └── restructurer.md          # 재구조화 에이전트
├── hooks/
│   └── hooks.json               # Hook 이벤트 매핑
├── libs/
│   ├── find-node.sh             # Node.js 바이너리 탐색 (백업 유틸)
│   └── run.cjs                  # 크로스 플랫폼 Hook 러너
├── bridge/
│   ├── mcp-server.cjs           # MCP 서버 번들 (~726KB)
│   ├── pre-tool-use.mjs         # INTENT.md/DETAIL.md 검증
│   ├── agent-enforcer.mjs       # 역할 제한
│   ├── context-injector.mjs     # 규칙 주입
│   ├── plan-gate.mjs            # Plan 모드 게이트
│   ├── session-cleanup.mjs      # 세션 정리
│   └── setup.mjs                # 세션 초기화 및 캐시 관리
├── skills/                      # 스킬 정의 (16개)
│   ├── fca-init/SKILL.md
│   ├── fca-scan/SKILL.md
│   ├── fca-sync/SKILL.md
│   ├── fca-structure-review/SKILL.md
│   ├── fca-promote/SKILL.md
│   ├── fca-context-query/SKILL.md
│   ├── fca-guide/SKILL.md
│   ├── fca-restructure/SKILL.md
│   ├── fca-review/              # 거버넌스: phases/, personas/, state-machine.md
│   ├── fca-resolve/             # 거버넌스: 수정 해결
│   ├── fca-revalidate/          # 거버넌스: Delta 재검증
│   ├── fca-migrate/SKILL.md
│   ├── fca-pipeline/SKILL.md
│   ├── fca-pull-request/SKILL.md
│   ├── fca-update/SKILL.md
│   └── fca-ast-fallback/SKILL.md
├── src/                         # TypeScript 소스
│   ├── index.ts                 # 라이브러리 export (94+ 함수 + 30+ 타입)
│   ├── types/                   # 타입 정의 (6 파일)
│   ├── core/                    # 순수 비즈니스 로직 (12 모듈, boundary-detector, cache-manager 포함)
│   ├── ast/                     # AST 분석 (5 모듈)
│   ├── metrics/                 # 메트릭 분석 (4 모듈)
│   ├── compress/                # 압축 (2 모듈)
│   ├── hooks/                   # Hook 로직 + 엔트리
│   │   ├── entries/             # Hook 엔트리포인트
│   │   ├── shared.ts            # 공유 유틸리티
│   │   ├── intent-injector.ts   # PreToolUse Read|Write|Edit INTENT.md 주입
│   │   └── ...                  # 기타 Hook 로직
│   ├── mcp/                     # MCP 서버 + 도구 (2+6)
...
| `@ast-grep/napi`            | ^0.31.0       | 구조적 AST 패턴 검색    |
...
- `bridge/*.mjs`: ESM 형식 → `#!/usr/bin/env node` + `for await` 지원
- MCP 서버와 Hook 스크립트를 별도 빌드 스크립트로 분리 → 독립적인 번들 최적화 가능
- `libs/run.cjs`: 크로스 플랫폼 Hook 러너 → `find-node.sh` 대체 (`.mcp.json`에서 `node` 직접 호출)
- `typescript`를 `external`로 처리 → 사용자가 `npm install` 시 자동 설치
- 단일 파일 번들 → `dist/` 디렉토리 없이 동작

**트레이드오프**:

- `mcp-server.cjs`가 ~726KB로 커짐 (minify 미적용 — 디버깅 우선, `@ast-grep/napi` 추가로 증가)
...
- Layer 4 스킬 수가 8→16으로 증가 — 관리 범위 확대
