...
| `run-c` | `seeded-change.md` | `REQUEST_CHANGES` | 의도적으로 심은 organ·external-boundary error |
...
| `run-g` | `out-of-scope-certainty.md` | `APPROVED` | 범위 밖 certainty가 verdict에 닿지 않음 |
...
`run-g`는 위 2단계에서 fixture 파일을 `main`에 추가한 뒤 base commit을 만들며 branch에는 clean change만 둔다. materialize하기 전에 `out-of-scope-certainty.md`를 읽는다. v6 Step 2 `scope`의 범위 밖 위반 수인 `outOfScopeCount`는 4여야 하며 changed-scope candidate에는 없어야 한다.
