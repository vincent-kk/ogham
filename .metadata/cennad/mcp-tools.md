...
z.object({
  ...
  prompt: z.string().min(1),
  model: z.enum(["high", "mid", "low"]).optional(),
});
```
...
`model` 은 생략 시 `mid` 로 dispatcher 에 적용되며, 매 turn 재적용된다 (원 세션 tier 는 복원하지 않는다 — `SessionMeta` 는 구체 모델명을 저장하지 않으므로). `start_conversation` 의 `model` 과 대칭이나 start 는 required, continue 는 optional 이다.
...
- `continue_conversation`: session_id 로 재개, 동일 cwd(project-scoped) 세션만. optional model(tier) 로 해당 turn 모델 지정.
