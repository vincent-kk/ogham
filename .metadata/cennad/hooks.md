...
`+2` 는 세 줄의 구성 속성을 갖는다: provider 가 이 workspace 에서 직접 편집을
적용한다는 사실 명시, 혼합 작업의 분할 지시(판단은 여기 · 구현분은 위임 — 분할은
keep 이 아니라 dispatch), 그리고 닫힌 예외 4개. 예외 (1) 은 사용자가 이 세션을
**지명**한 경우만, (2) 는 프롬프트로 건넬 수 없는 상태(이 대화·실시간 사용자 입력·
이 세션만 가진 도구)만 인정하며 workspace 파일은 근거가 되지 않는다.
