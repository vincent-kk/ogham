...
1. `packages/cennad/` 디렉토리 생성.
2. `package.json`: name `@ogham/cennad`, version `0.1.0`, type `module`, `exports`/`main`/`types` dist 기반.
...
4. `.claude-plugin/plugin.json`: `{ name: "cennad", skills: "./skills/", mcpServers: "./.mcp.json" }`.
...
9. 루트 `yarn build:all` 이 cennad 까지 통과.
...
9. `yarn cennad build:plugin` 으로 `bridge/mcp-server.cjs` 생성 확인.
...
4. `scripts/buildHooks.mjs` (filid 복제 + cennad `hookEntries`).
5. `hooks/INTENT.md`, `hooks/hooks.json`.
6. `yarn cennad build` 가 `bridge/injectStatic.mjs`, `bridge/injectDynamic.mjs` 생성 + 가드 통과.
...
3. 루트 `CLAUDE.md` 의 패키지 목록에 cennad 추가.
