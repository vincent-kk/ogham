...
| `--config`                                              | `-c`         | `key=value`        | 로컬 설정 파일의 특정 설정을 런타임에서 임시로 오버라이드합니다. cennad 는 `-c model_reasoning_effort=<effort>` 로 tier 별 effort 를 주입합니다.               |
| `--image`                                               | `-i`         | `path` (콤마 구분) | 초기 프롬프트와 함께 목업 이미지나 UI 스크린샷 파일을 첨부합니다.                                                                                              |
| `--model`                                               | `-m`         | `string`           | 기본 설정 파일(`.toml`)에 지정된 모델 대신 임시로 다른 모델을 사용합니다. `model_map.codex` 의 tier 해석 결과가 주입되며 `exec` / `exec resume` 모두 받습니다. |
...
### 📦 모델 카탈로그와 reasoning effort (`core/codexModels` 내부 캐시)

`codex debug models` 가 모델 카탈로그를 JSON 으로 반환하며, 각 항목의 `supported_reasoning_levels[].effort` 가 그 모델이 받는 effort 집합입니다. `core/codexModels` 가 이를 1시간 TTL 로 캐시하고(`<CENNAD_HOME>/runtime/codex-models.json`), settings UI 의 `/provider-status` 가 직접 호출합니다. codex 미설치·조회 실패 시 정적 fallback 으로 degrade 하며 절대 throw 하지 않습니다.

effort 스케일은 `low < medium < high < xhigh < max < ultra` 이고, **모델마다 지원 집합이 다릅니다**.

| 모델                                   | 지원 effort                               | 성격                   |
| :------------------------------------- | :---------------------------------------- | :--------------------- |
| `gpt-5.6-sol`                          | low / medium / high / xhigh / max / ultra | frontier               |
| `gpt-5.6-terra`                        | low / medium / high / xhigh / max / ultra | balanced everyday work |
| `gpt-5.6-luna`                         | low / medium / high / xhigh / max         | fast and affordable    |
| `gpt-5.5` / `gpt-5.4` / `gpt-5.4-mini` | low / medium / high / xhigh               | 이전 세대              |

claude-code 는 상한을 넘는 effort 를 조용히 낮추지만 **codex 는 그렇지 않습니다** — 모델이 광고하지 않은 effort 를 보내면 `invalid_request_error` 로 호출 자체가 실패합니다. 따라서 cennad 는 model 과 effort 를 항상 짝으로 해석하고, settings UI 가 선택된 모델의 집합으로 effort 선택지를 제한합니다.
