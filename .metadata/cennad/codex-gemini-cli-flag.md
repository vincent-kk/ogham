...
> **중요**: Gemini CLI 서비스는 **2026-06-18 종료** 예정입니다. cennad는 Google 엔진 슬롯을 `gemini`와 `antigravity` 중 하나로만 활성화할 수 있으며 두 값은 **상호 배타적**입니다. 신규 설치는 `antigravity`(`agy`)를 사용하는 것을 권장합니다.
...
`agy`는 Gemini CLI 서비스 종료 이후 Google 엔진 슬롯을 대체하는 CLI 어댑터입니다. cennad는 `agy -p` 형태로 비대화형 호출하며(plain text 출력 — `--output-format` 플래그 없음), 세션 재개는 `--continue` 플래그로 수행합니다.
...
| cennad `option_flags` 키       | Gemini CLI       | Antigravity CLI (`agy`)          | Codex CLI                             |
| :----------------------------- | :--------------- | :------------------------------- | :------------------------------------ |
| `gemini.yolo`                  | `--yolo`         | —                                | —                                     |
