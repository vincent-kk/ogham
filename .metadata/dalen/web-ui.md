# Web UI — Report Viewer & Settings Server

`render_report`/`open_settings` 가 공유하는 단일 로컬 HTTP 서버(127.0.0.1). cennad `openSettings/webServer/` 의 보안·기동 패턴을 일반화하고, 세션 내내 유지되는 점이 다르다.

## 서버 사양

- 바인딩: `127.0.0.1` 전용. 포트: `config.preferred_port`(기본 0=동적, `server.listen(0,'127.0.0.1')`).
- 생애: 첫 `render_report`/`open_settings` 에서 1회 기동. **세션 생존 = 뷰어 탭 heartbeat**(`POST /api/ping`); 마지막 도구 호출·heartbeat 가 모두 `idle_shutdown_minutes` 동안 단절되면 세션 상태 무관 **폴백 종료**(Claude 비정상 종료·`close_report` 누락 누수 방지). MCP 종료 시 동반 종료.
...
| 메서드 | 경로                          | 동작                                                                                              |
| ------ | ----------------------------- | ------------------------------------------------------------------------------------------------- |
| GET    | `/r/<session>?token=`         | 뷰어 HTML(`reportHtml`). `__DALEN_STATE__` 에 session_id·token·title·렌더 HTML·raw markdown 주입. |
| GET    | `/api/report?session=&token=` | 렌더 HTML+메타 재조회. `&format=md` → raw markdown(원본 복사용).                                  |
...
| GET    | `/settings?token=`            | 설정 HTML(`settingsHtml`). `__DALEN_STATE__` 에 현재 Config 주입.                                 |
...
├── scripts/
│   ├── app.js               # 상태 주입 해석, 본문 마운트, /api/report 재조회
...
서버는 sanitize+line-map HTML 을 렌더하되 **raw markdown 도 함께 노출**한다(세션 `report.md` 기반: `__DALEN_STATE__` 동봉 또는 `GET /api/report?session=&format=md`). 파싱을 클라이언트로 옮기지 않는다.
...
`buildReportHtml.mjs` / `buildSettingsHtml.mjs` 가 각 페이지 html+css+js 를 esbuild 로 minify·inline → `__generated__/reportHtml.ts`, `settingsHtml.ts`. 뷰어의 무거운 렌더러는 `buildRenderers.mjs` 가 독립 브라우저 엔트리로 `bridge/assets/` 에 빌드, `handleGetAsset` 가 서빙(번들 비포함).

## 보안

- token 미검증 → 401 `{ ok:false, message:'Invalid token' }`.
- `__DALEN_STATE__` 주입은 `escapeJsonForHtml`(`<`,`>`,`&`,U+2028,U+2029).
