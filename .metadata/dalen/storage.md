...
루트: `~/.claude/plugins/dalen/`. 경로 상수는 `src/constants/paths.ts`.

## 레이아웃

```
~/.claude/plugins/dalen/
...
- 세션 디렉토리는 `session_ttl_hours` 만료 후 다음 MCP 기동 시 정리.
...
| `idle_shutdown_minutes`   | `number`                  | `10`   | 1–120, 활성 세션 0 후 종료        |
