...
| 렌더러 자산 | `browser` | `src/mcp/pages/report/renderers/*.entry.ts` | `bridge/assets/*.js`(+css) | 각 chunk      |
...
- **싱글톤**: 모듈 레벨 1 인스턴스. `render_report`/`open_settings` 가 기동 요청 시 이미 있으면 재사용 — **중복 `listen` 금지**.
- **liveness = heartbeat**: 각 뷰어 탭이 `POST /api/ping`(주기 ~30s)으로 세션 생존을 갱신. 서버는 max(마지막 도구 호출, 마지막 ping) 기준으로 idle 판정 — 둘 다 `idle_shutdown_minutes` 초과 시 `server.close()` + 싱글톤 null(세션 active 여부 무관 → Claude 크래시·`close_report` 누락에도 종료). 활동 발생 시 타이머 재설정.
...
- **`extra` 전달형 wrapHandler 필요**: cennad 의 현 `wrapHandler` 는 `(args) => …` 로 `extra` 를 버린다. dalen 은 `collect_feedback` 가 `extra.signal` 에 접근해야 하므로 `(args, extra) => …` 를 전달하는 변형이 필요.
- `collect_feedback`:
...
- 세션 Map: `session_ttl_hours` TTL + **상한(LRU evict)** 으로 무한 증식 차단. `close_report`/idle/TTL 정리.
- 이미지: busboy **스트리밍 → 디스크**(`runtime/sessions/<sid>/images/`). 메모리 전체 버퍼 상주 금지. 크기 초과 시 스트림 abort + 부분 파일 삭제.
