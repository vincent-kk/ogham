...
> 데이터로 주입한다. 외부 서버·훅에 의존하지 않는 순수 마크다운 플러그인으로, 평가 동작은 전부 세션 내부에서 일어난다.
...
1. **유형·분야 판별** → [분야 프로파일](./field-profiles.md) 로드(우선순위: `--profile` override > 자동 판정(기본) > 보편 폴백 > (선택) 커스텀 yaml).
