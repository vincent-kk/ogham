...
### 대량 = async (`paper_search_start` / `_status` / `_results`)
...
`setup` 상세(web UI 필드·credentials 분리·동의 흐름)는 [setup.md](./setup.md). 인증 실패·미구성은 `auth_check` 가 선제 감지하여 `setup` 안내.
...
- **다운로드 경로**: `fetch_fulltext` 는 선언된 `outDir` 안에서만 기록(경로 traversal·symlink 탈출 거부), sha256 무결성.
- **license 게이트**: OA·license 미확인 본문은 저장 보류(재배포 위험 차단).

## 캐싱

- `mesh_lookup`: **versioned**(연간 MeSH 갱신 — 버전 키로 무효화).
