...
- **아키텍처**: Dispatcher(search 스킬 겸·상태머신) · 에이전트 1(`paper-search-expert`, 내부 2모드) · 노출 스킬 4(search·query·download·setup) · MCP 5(`paper_search`·`mesh_lookup`·`fetch_fulltext`·`setup`·`auth_check`) · atlassian httpClient(retry·429 backoff·auto-POST·SSRF allowlist).
...
| `skills/.shared`      | `query-strategy.md`·`rerank.md`                                       | 검색식 방법론(QueryRole·ESpell·recall 게이트)·재랭킹 기준 SSoT. lazy. (`agents/`는 서브디렉토리 불가)                                                                                                                                                                                                  |
| `skills`              | `search`·`query`·`download`·`setup` + `.shared/{mcp-tools,eutils}.md` | search=Dispatcher 겸(`references/{state-machine,intent,modes}.md`). `mesh`는 MCP로만.                                                                                                                                                                                                                  |
