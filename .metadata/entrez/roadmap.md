...
- **아키텍처**: Dispatcher(search 스킬 겸·상태머신) · 에이전트 1(`paper-search-expert`, 내부 2모드) · 노출 스킬 4(search·query·download·setup) · MCP 5(`paper-search`·`mesh-lookup`·`fetch-fulltext`·`setup`·`auth-check`) · atlassian httpClient(retry·429 backoff·auto-POST·SSRF allowlist).
...
| `core`                | 캐싱                                                                  | `mesh-lookup`=versioned(연간 MeSH 갱신), ESearch count=short TTL, PMID metadata=긴 TTL(ahead-of-print 갱신 표시).                                                                                                                                                                                      |
