...
핵심 원칙: **물리 1 에이전트 + 내부 2모드 논리 분리**. 한 에이전트가 검색 생성(WHAT)과 재랭킹(RANK)을 모두 맡되, 모드별로 prompt·출력 schema·평가기준을 분리한다. 검색식 방법론과 재랭킹 기준의 SSoT는 에이전트가 아니라 `references/{query-strategy,rerank}.md`이며(중복 방지 — [spec.md](./spec.md) SSoT 표), 도구 I/O 계약은 [mcp-tools.md](./mcp-tools.md)가 단독 소유한다.
...
| `tools` | `Read`, `mcp_mesh_lookup`, `mcp_paper_search` | `Read`=참조 문서(`query-strategy.md`·`rerank.md`)·전달 컨텍스트 로드. MCP 2종만 보유 — `fetch_fulltext`는 download 스킬 소관이라 미부여(검색·랭킹 책임 격리). `Bash`/`Grep`/`Glob` 미부여(파일시스템 작업 없음). |
...
|------|------|------|------|-------------|------------------|
| **생성**(generation) | recall | WHAT을 어떻게 빠짐없이 검색할까 | topic + meshHints | `{queries[]{term, role, breadth, rationale}}` | `references/query-strategy.md` |
| **재랭킹**(rerank) | precision | 후보 중 무엇이 정보요구에 부합하나 | pre-score된 후보 records[] | `{ranked[]{pmid, score, reason}}` | `references/rerank.md` |
...
interface GenerationOutput { queries: GeneratedQuery[] }
```
...
interface RerankOutput { ranked: RankedRecord[] }
```
...
- **참조 로드**: `Read`로 `references/query-strategy.md`(생성)·`references/rerank.md`(재랭킹)를 로드해 방법론을 적용한다. 방법론 본문은 에이전트 정의가 아니라 reference가 SSoT.
...
interface SearchHandoff {
  ...
  context: {
    ...
    operationBudget: { maxRequests: number; maxRecords: number; maxWallMs: number };
