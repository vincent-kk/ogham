...
- **Stop**: changelog gate + session recap (세션당 1회, [13 §5](./13-memory-lifecycle.md)) + lifecycle-dispatcher
- **SessionEnd**:
  - 세션 정리 + 영속화
  - **Personal-context prune**: 만료 state 제거·due 경과 자동 resolve·resolved topic 정리 ([27 §5.3](./27-personal-context.md))
...
- 지식관리 5 + 지식그래프 4 + 인지작업 4 + 세션운영 4 + 운영 6 + 환경설정 7 = **사용자 호출 30개**
- **+ 비호출 meta-skill 1개** (`using-maencof`): SessionStart 훅이 매 세션 system context에 자동 주입. 본문은 `src/hooks/sessionStart/helpers/bootstrap/metaSkillBody.md`에 있으며 `skills/` 디렉토리 밖. [스킬 명세 §7](./18-skills.md)
...
| Session recap         | Stop Hook → recap 주입 (세션당 1회)                                                 | [13 §5](./13-memory-lifecycle.md) |
| InsightCategoryFilter | `capture_insight` MCP 도구 + `insight-injector` 훅                                  | [17 §3, §7](./17-mcp-tools.md)    |
| Socratic Elenchus     | `refine` Phase 2.5                                                                  | [18 §3](./18-skills.md)           |
| Personal-context awareness  | SessionStart Hook → `<personal-context>` 블록 (캡처 지침 내장) + `capture_personal_context` MCP | [27](./27-personal-context.md)          |
...
| Personal-context awareness  | (없음)                              | `personal-context.json::config.enabled=false`             |
