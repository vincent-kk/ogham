...
- CRUD 5 + 검색 6 + 캡처 2 + CLAUDE.md 3 + 동반자 1 + 운영 3 = 총 **20개** ([MCP 도구 명세](./17-mcp-tools.md))
- 신규 카테고리: 캡처(`capture_insight`, `boundary_create`), CLAUDE.md(`claudemd_merge/read/remove`), 동반자(`companion_edit`), 운영(`activity_read`, `work_history`, `context_cache_manage`)
