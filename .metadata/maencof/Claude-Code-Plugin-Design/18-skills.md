...
# 스킬 명세 — 사용자 호출 30개 + 비호출 meta-skill 1개

## 개요

사용자가 `/maencof:명령` 형태로 호출하는 확장 기능(30개). 지식 트리 + 검색 엔진 컨텍스트를
...
| explore       | `/maencof:explore`                     | 대화형 SA 기반 지식 탐색. `--for-brainstorm` 옵션으로 5-8개 seed 생성 후 `think --mode divergent` 핸드오프 | 1     |
| (구 rebuild)  | `/maencof:build --force --reset-cache` | 인덱스 강제 전체 재구축 — `build`에 흡수                                                                   | 1     |
| (구 diagnose) | `/maencof:checkup --quick`             | 인덱스 건강도 진단 (고아, 깨진 링크) — `checkup`에 흡수                                                    | 1     |
...
| 스킬    | 호출               | 역할                      | 핵심 기능                                                                                                                                                                                    |
| ------- | ------------------ | ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| think   | `/maencof:think`   | 다중 해석 / ToT 탐색      | 3-mode: **default**(표준 해석, 5축 복잡성·커버리지·UX·유지보수·팀 역량), **divergent**(참신성 우선), **review**(위험 역전)                                                                   |
| refine  | `/maencof:refine`  | Spec 정제                 | Phase 1(수렴)-2(분해)-**2.5 Socratic Elenchus**-3(검증)-4(확정). Phase 2.5는 가정 표면화(2.5.a) + 반례 탐색(2.5.b) + Immutable Objects 모순 검사(2.5.c). 5-8턴 예산, 최대 1 라운드 back-edge |
| insight | `/maencof:insight` | 인사이트 뷰어 + 필터 관리 | `--category <principle\|refuted\|ephemeral> --accept\|--reject`. 캡처 실행 자체는 MCP `capture_insight` 도구 전담                                                                            |
| suggest | `/maencof:suggest` | 링크/전이 후보 제안       | `kg_suggest_links` 경유, `knowledge-connector` 에이전트와 협업                                                                                                                               |
...
거부된다(`InsightCategoryFilter`). think의 review 모드와 차이: think-review는 산출물
비판, refine 2.5는 대화 수렴 게이트. 상세: `skills/refine/knowledge/socratic-elenchus.md`.
...
| 스킬            | 호출                       | 역할                                                    |
| --------------- | -------------------------- | ------------------------------------------------------- |
| checkup         | `/maencof:checkup`         | 6개 진단 + 자동 수정 (checkup 에이전트 orchestrator)    |
...
| personal-status | `/maencof:personal-status` | personal-context 열람·해소·토글 ([27 §7](./27-personal-context.md)) |
...
| 스킬        | 호출                   | 역할                                              |
| ----------- | ---------------------- | ------------------------------------------------- |
| configure   | `/maencof:configure`   | 통합 환경 설정 진입점 (router)                    |
| bridge      | `/maencof:bridge`      | MCP 설치+등록+워크플로우 스킬 생성                |
| craft-skill | `/maencof:craft-skill` | 커스텀 스킬 생성기                                |
| craft-agent | `/maencof:craft-agent` | 커스텀 에이전트 생성기                            |
| instruct    | `/maencof:instruct`    | CLAUDE.md 섹션 관리 (MCP `claudemd_*` 경유)       |
| rule        | `/maencof:rule`        | 규칙 관리                                         |
| lifecycle   | `/maencof:lifecycle`   | 라이프사이클(`.maencof-meta/lifecycle.json`) 관리 |
...
- 본문: `plugins/maencof/src/hooks/sessionStart/helpers/bootstrap/metaSkillBody.md`
...
4. 본문 예산 `META_SKILL_MAX_CHARS` = 4096 Unicode 코드포인트. 초과 시 런타임은 스킵을
   로그에 남기고, `build-hooks.mjs` 빌드 가드가 빌드 자체를 실패시킨다 (silent skip 금지)
...
| User-state awareness  | (자동 — `<personal-context>` 블록 지침)   | MCP `capture_personal_context`; 관리는 `personal-status`    |
...
| Retrospective         | (사용자 트리거 없음, Stop 자동 1회) | Session recap → [13 §5](./13-memory-lifecycle.md)     |
...
`reflect`는 볼트 관점 저지먼트 리포터이며, Stop recap과 **직교**(혼동 금지).
...
├── 지식 관리 (5)
│   ├── setup, remember, recall, organize, reflect
├── 지식 그래프 (2, 구 rebuild/diagnose는 build/checkup에 흡수)
│   ├── build, explore
├── 인지 작업 (4)
│   ├── think, refine, insight, suggest
├── 세션 운영 (3, 구 dailynote는 hook으로 이전)
│   ├── cleanup, changelog, migrate
├── 운영 (5)
│   ├── checkup, ingest, connect, mcp-setup, manage
└── 환경 설정 (7)
    └── configure, bridge, craft-skill, craft-agent,
        instruct, rule, lifecycle
```

각 스킬 디렉토리에 선택적 `reference.md`, `examples.md`, `knowledge/` 포함 가능.
특히 `refine/knowledge/socratic-elenchus.md`, `think/knowledge/evaluation-criteria.md`,
`tot-methodology.md`가 Phase 2 추가되었다.

**비호출 메타스킬**의 본문은 `skills/` 밖에 있다:

- `plugins/maencof/src/hooks/sessionStart/helpers/bootstrap/metaSkillBody.md`
