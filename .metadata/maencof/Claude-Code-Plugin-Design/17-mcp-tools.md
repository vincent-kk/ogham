...
created: 2026-02-28
updated: 2026-07-09
tags: [mcp, tools, crud, search, knowledge-graph, insight, claudemd, personal-context]
layer: design-area-4
---
...
동반자 페르소나, 세션 캐시 운영을 통합한다. 모든 도구는 단일 요청-응답 사이클 내에서 완결된다.
...
| layer_filter | number[] | —        | Layer 필터 (1–5)                        |
| sub_layer    | enum     | —        | 서브레이어 필터 (kg_search와 동일)      |
| scope        | enum     | balanced | 탐색 폭 프리셋 (focused/balanced/broad) |

모든 선별(`layer_filter`/`sub_layer`/`scope`)은 토큰 예산 조립 **전에** 적용되어 제외
레이어가 예산을 소비하지 않는다. `scope`는 SA 내부 파라미터(threshold/maxHops) 프리셋 —
`focused`(0.1/3) · `balanced`(0.05/5, 기본값이자 scope 도입 이전과 동일) · `broad`(0.02/7).
역할 분담: `kg_search`는 참조 랭킹 반환(본문 없음), `kg_context`는 다문서 콘텐츠 조립.
...
`.maencof-meta/personal-context.json`만 다루며 KG 캐시와 무관하다(companion_edit처럼 read wrapper 등록).
응답은 `{ success, id, merged, message }` 최소형 — 캡처 배너·통지 없음. 상세 계약은 [27 §5.1](./27-personal-context.md).

| 파라미터  | 타입   | 기본값    | 설명                                                      |
| --------- | ------ | --------- | --------------------------------------------------------- |
| target    | enum   | —         | `state`(일시 상태) / `topic`(최근 동향)                   |
| action    | enum   | `capture` | `capture`(upsert — slug 일치 시 재강화/touch) / `resolve` |
| label     | string | —         | 항목 라벨 (≤ 40자; 정규화 슬러그가 dedup 키)              |
| kind      | string | —         | 자유 어휘 (소문자 kebab ≤ 24자, capture 시 필수; [27 §3]) |
| intensity | enum   | —         | `low`/`medium`/`high` (state capture 시 필수)             |
| note      | string | —         | 짧은 묘사 (state ≤ 80자 / topic ≤ 100자, 선택)            |
| evidence  | string | —         | 근거 — 날짜+대화 단서 (≤ 100자, state capture 시 필수)    |
| ttlDays   | number | 14        | state 유효기간 1–60일 (서버가 expiresAt 계산)             |
| due       | string | —         | YYYY-MM-DD (topic 전용, 약속/계획 기한)                   |
...
| 도구                   | 설명                                                                 |
| ---------------------- | -------------------------------------------------------------------- |
| `activity_read`        | 활동 로그 조회 (`.maencof-meta/activity/events/` 날짜별 NDJSON)      |
| `work_history`         | 작업 이력 조회 (기간 요약 / 토픽·레이어 작업일자)                    |
...
| 도구             | 설명                                                                                                                               |
| ---------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| `companion_edit` | `.maencof-meta/companion-identity.json`(동반자 페르소나 정본)의 유일한 편집 채널 — commit 없이 diff 미리보기, `commit:true`로 기록 |

2단계 설계: commit 없는 호출은 diff와 검증 결과만 반환하고 파일을 바꾸지 않으며,
`commit:true`는 백업 후 기록한다. 스키마 위반, 턴당 주입 예산(500자) 초과, brief가
detail보다 짧지 않은 변경은 거부된다. 파일 직접 편집은 금지 — 반드시 이 도구를 경유한다.
...
`/maencof:insight --category <principle|refuted|ephemeral> --accept`로 필터를 열 수 있다.
