...
트리와 그래프 원리를 적용하여 한 개인을 표상하는 지식을 5개의 층위로 구분한다. 아키텍처 버전 2.0.0에서 Layer 3의 3차원 방향성 분할을 도입해 외부 지식의 인지적 병목을 구조적으로 해소했고, 버전 3.0.0에서 Layer 5를 미분류 정보의 임시 수용소로 좁히고 교차 연결 허브를 레이어 직교 속성으로 분리하였다.

각 레이어의 **SA 감쇠 인자**는 확산식 `A[j] = A[i] · W[i,j] · d` 의 `d` 이며, 노드가 이웃으로 내보내는 활성량에 **곱해진다**. 따라서 값이 클수록 넓게 퍼진다 — 감쇠되는 양이 아니다([§8.4](./08-l5-redefinition.md#84-감쇠-인자의-방향)).
...
- **SA 감쇠 인자:** 0.50 (최저 — 코어가 모든 질의에 딸려 나오지 않도록 한다)
...
- **SA 감쇠 인자:** 0.90 (실행 맥락은 넓게 연결된다)

### **Layer 5: 미분류 정보의 임시 수용소 (Unclassified Buffer)**

아키텍처 버전 3.0.0에서 Layer 5는 아직 어느 레이어에도 속하지 않은 정보의 임시 수용소라는 **하나의 역할**을 갖는다. 버전 2.0.0의 Buffer/Boundary 이중 역할은 성질이 정반대인 두 가지를 한 레이어에 묶은 배치였고, 교차 연결 허브는 레이어 직교 속성으로 분리되었다. 상세한 논증은 [§8 Layer 5 재정의](./08-l5-redefinition.md)를 참조한다.

- **트리 구조:** `/05_Context/` 디렉토리. 서브레이어 없이 평면 배치한다.
- **그래프 특성:** **Temporary 노드**. 다른 레이어와의 연결이 미확정 상태이며, 사용자의 분류 또는 시스템의 내용 분석을 거쳐 L3A/B/C 또는 L2로 승격(Promote)되거나, 설정 가능한 만료 기간(기본 30일) 경과 후 삭제 후보로 전환된다. 다른 레이어가 이 노드를 참조하는 것은 금지된다.
- **Frontmatter:** `layer: 5`, `buffer_type`, `promotion_target`, `source_context`, `expires`
- **SA 감쇠 인자:** 0.45

### **허브: 레이어 직교 속성**

교차 연결 허브(MOC, 프로젝트 대시보드)는 레이어가 아니라 역할이다. 어느 레이어의 문서든 `hub: true` · `hub_kind` · `purpose`를 선언해 허브가 되며, `CROSS_LAYER` 엣지를 통해 태그가 겹치는 노드들을 잇는다. 자리를 옮기지 않고 자신이 속한 레이어에 그대로 있는다.

- **그래프 특성:** **Bridge 노드 (높은 fan-out)**
- **SA 감쇠 인자:** 0.95 (레이어·서브레이어 값을 덮어쓴다)
