...
**4. Layer 감쇠 인자 유지 (5-Layer 의미론 보존)**

```
α(i) = α_base · d(layer_i, subLayer_i, hub_i)
```

`d`는 `getLayerDecay`가 반환하는 감쇠 인자이며, 확산 활성화 문헌의 표준 정의(`A[j] = A[i] · W[i,j] · d`)를 따른다. 노드 `i`가 이웃으로 내보내는 활성량(`outMass = a(i) · α(i) / |N(i)|`)에 **곱해지므로 값이 클수록 넓게 퍼진다** — 감쇠되는 양이 아니다. 이 방향을 오독하면 계수 표가 의도의 역방향으로 채워진다([§8.4](../../Tree-Graph-Hybrid-Knowledge-Architecture-Research-Proposal/08-l5-redefinition.md#84-감쇠-인자의-방향)에 실제 사례가 있다).

우선순위는 `hub` > `subLayer` > `layer`다. 축은 레이어 번호에 단조롭지 않고 U자형이다 — 정체성 코어(L1 0.5)와 미분류 임시 수용소(L5 0.45)라는 양 극단이 좁게, 중간의 실질 지식(L3 0.8 · L4 0.9)이 넓게 퍼진다. 허브(`hub: true`)는 0.95로 레이어·서브레이어 값을 덮어쓴다. `α_base`는 QA-SA의 고정 감쇠(0.7)에 대응하는 전역 스케일이며 기본 1.0(layer 감쇠에 위임)이다.

#### 감쇠 인자의 계측 근거

v3 초기값은 설계 근거만으로 정해졌고, 당시 골든셋에는 L5·허브 문서가 하나도 없어 값을 바꿔도 지표가 움직이지 않았다. 픽스처에 허브 2종(`graph-search-hub`·`security-review-hub`)과 평면 L5 4종을 넣어 발화 경로를 만든 뒤, `decaySweep.eval.test.ts` 가 세 계수를 480 조합으로 전수 측정했다. 응답면은 세 축 모두 평탄구간을 가진다:

| 계수          | 평탄구간    | 벗어날 때                                                                      | 채택                 |
| :------------ | :---------- | :----------------------------------------------------------------------------- | :------------------- |
| L5            | 0.25 – 0.6  | 0.7 이상에서 하락 — 미분류 조각이 형제 버퍼로 활성을 흘려 실제 지식을 밀어낸다 | **0.45** (구간 중앙) |
| 허브          | 0.75 – 1.0  | 0.6 이하에서 하락 — 다리가 활성을 전달하지 못한다                              | **0.95**             |
| `CROSS_LAYER` | 0.44 – 0.48 | 0.38 미만에서 브릿지 붕괴, 0.5 이상에서 허브가 본문을 밀어냄                   | **0.45**             |

L5 와 허브는 설계 근거로 고른 값이 계측 평탄구간 안에 있었다. `CROSS_LAYER` 만 근거가 틀렸다 — "허브가 만드는 다리이므로 `LINK` 다음가야 한다(0.85)"는 추론은 계측에서 최저 구간(ndcg10 0.9596)으로 나왔고, 0.45 에서 0.9727 로 올라간다. 올바른 축은 강도가 아니라 **출처**다: `CROSS_LAYER` 는 태그 겹침에서 기계가 추론한 엣지이므로 저자가 쓴 `LINK`·디렉토리 계층·명시된 `RELATIONSHIP` 아래, 가장 약한 추론 엣지인 `DOMAIN`(0.3) 위에 온다. 허브가 자기가 색인한 본문 문서를 밀어내면 허브의 값이 사라진다.

측정 기준선: 검색 골든셋 24 쿼리 ndcg10 0.9727 · recall10 0.9643 · mrr 1.0000 · precisionR 0.8150.
