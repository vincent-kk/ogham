...
- **T6 (후속: tirnanog archival 원인 추적 + 결정 확정)** — 하네스에 쿼리별 분해(`perQuery`)·현행 계수 top10 진단(`diagnoseAtDefault`, `[A]` 마킹)·그리드 0.35 추가(커밋 `4f66490b`). **원인 확정**: recall 0.3333 은 엔진 결함이 아니라 골든 2쿼리(notable·cwe-502)의 relevance 임의성 — 균일 대량 스텁 후보군(수백 건, tie-break 는 pagerank→id 로 updated 무관)에서 특정 4건 지정은 큐레이션 운을 측정. agentic-title 은 recall 1.0(활성 1위 > 스텁 2·3위 — 침강 질서 정상), working-security 는 top10 스텁 0건(침강력 증명). **수정(rev.2)**: 두 쿼리를 스텁 전용 소규모 태그 `crawl4ai`(활성 0/스텁 4)·`citrix`(활성 0/스텁 3)로 교체, 스텁 전원을 relevance 로 실어 임의성 제거 → 재실행에서 archival recall 0.3333→**1.0**(전 계수, fix-prove). **0.3 확정 근거**: ① 0.3/0.35/0.4 의 3면 쿼리 전수 비교에서 차이는 falias 2쿼리뿐(0.35: discovery-watch +0.016 단일 쿼리 노이즈 — PROMOTION 임계 0.005 를 +0.0003 차로 걸침; 0.4: working-dca −0.027 침식 시작) ② suggest_links 경계(`kgSuggestLinks.ts:125` — `score < minScore` 컷이므로 0.2 접촉이면 통과): 스텁 tag-exact 0.5×m 이 0.3→0.15(마진 0.05), 0.35→0.175(마진 절반), 0.4→0.20(경계 접촉, 제안 재진입) ③ 티어 질서: 0.15 는 활성 tag-prefix(0.3)의 절반 — 역전은 m=0.6 ④ archival 하방은 m=0 에서만 소실(0.05 부터 recall 1.0, 3면 공통). **결정: `ARCHIVED_SEED_MULTIPLIER = 0.3` 고정** — 상수 주석을 확정 서사("운영 실측 후 조정 여지 있음" 제거, 검증 러너·경계값 명시)로 갱신(커밋 `52c7b24f`). 검증: typecheck exit 0 + `yarn eval` 7 passed/1 skipped. 리포트: scratchpad `tirnanog-sweep3.json`·`falias-sweep2.json`·`fixture-sweep2.json`.
