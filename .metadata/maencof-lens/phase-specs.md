...
- [ ] `read` rejects documents in excluded layers
- [ ] `navigate` filters neighbor lists by effective layers
...
- `src/tools/` — 5 MCP tool wrappers (search/context/navigate/read/status)
...
4. 상위 결과에 대해 `read(path: top_result.path)` 호출
...
3. `read` — 상위 결과 문서 읽기
4. `navigate` — 그래프 이웃 탐색 (inbound/outbound/hierarchy)
