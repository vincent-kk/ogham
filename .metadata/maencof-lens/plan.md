...
- **Skills (3)**: `setup-lens` (config), `lookup` (search→read→summarize), `brief` (token-budgeted assembly)
...
**Note**: `brief` skill uses `context` only (NOT `search`).
...
```
T1 (lookup SKILL.md)  ─┐
T2 (brief SKILL.md)    ─┼─► T4 (session-start.ts) ─► T6 (docs) ─► T7 (build + verify)
T3 (researcher.md)     ─┘
```
...
| T2  | Create brief skill      | `/skill-constructor` CREATE    | `skills/brief/SKILL.md`                       |
...
- /maencof-lens:brief <query>: token-budgeted context assembly
...
- T2 brief skill — removed redundant `search` step. `context` handles search internally.
...
- [ ] `skills/brief/SKILL.md` — complete workflow (`context` only, no redundant search), `user_invocable: true`
...
- [ ] `phase-specs.md` 7.2 — brief skill workflow synced (`search` removed)
...
Phase 7    Skills (lookup, brief) + Agent (researcher)    ⬜ Next
