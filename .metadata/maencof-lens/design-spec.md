...
| **MCP 도구 prefix** | `lens_` | `lens_search`, `lens_read` |
...
| 도구 | 원본 | 설명 |
|------|------|------|
| `lens_search` | kg_search | Spreading Activation 기반 지식 탐색 |
| `lens_context` | kg_context | 토큰 예산 기반 컨텍스트 조립 |
| `lens_navigate` | kg_navigate | 그래프 이웃 탐색 |
| `lens_read` | maencof_read | 단일 문서 읽기 |
| `lens_status` | kg_status | vault 상태 조회 |
...
Claude → lens_search(seed: ["NER", "모델 최적화"], vault: "fionn")
→ vault에서 "ink-veil NER Architecture Pivot" 문서 발견
→ lens_read(path: "04_Action/ink-veil/ner-model-optimization-request.md")
...
Claude → lens_context(query: "FCA 아키텍처", vault: "fionn", token_budget: 3000)
...
Claude → lens_search(seed: ["기술 레퍼런스"], vault: "fionn")
       → lens_search(seed: ["프로젝트 스팩"], vault: "work")
...
| # | 질문 | 결정 | 근거 |
|---|------|------|------|
| Q1 | MCP 도구 prefix | **`lens_`** | 이름 통일 원칙. `lens_search`, `lens_read` — 짧고 일관됨 |
