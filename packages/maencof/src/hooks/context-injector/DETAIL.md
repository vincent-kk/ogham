...
- 내부 organ: `./turn-context/` — `buildTurnContext`, `readIndexMetadata`, `readCompanionIdentity`, `compressMarkdown`, `readL1Summary`, `buildCompanionIdentityTag` 등.
- 캐시 의존: `cache-manager` (`readTurnContext` / `writeTurnContext` / `writePromptContext` / `isFirstInSession` / `markSessionInjected`).
