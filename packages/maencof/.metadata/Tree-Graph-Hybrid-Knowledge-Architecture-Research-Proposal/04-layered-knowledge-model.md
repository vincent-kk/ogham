...
트리와 그래프 원리를 적용하여 한 개인을 표상하는 지식을 5개의 층위로 구분한다.
...
### **Layer 5: 맥락 메타데이터 (Context & Domain Metadata)**

* **트리 구조:** 인물, 도메인, 환경 정보를 분류하는 **단일 또는 2-뎁스 디렉토리**(예: /05\_Context/persons/, /05\_Context/domains/).
* **그래프 특성:** 다른 레이어 문서들이 참조하는 **'메타데이터 노드'**. 직접적인 지식을 담지 않고, 지식의 맥락(누구와 관련된 지식인가, 어떤 도메인인가)을 제공하는 보조 역할을 한다. 여러 레이어의 문서들로부터 인바운드 참조를 받는다.
