...
```
/maencof:maencof-bridge (this skill — orchestrator)
  1. /maencof:maencof-mcp-setup     → MCP server installation
  2. /maencof:maencof-connect        → Data source registration
  3. /maencof:maencof-craft-skill    → Workflow skill generation
```
...
| Skill | Scope |
|-------|-------|
| `/maencof:maencof-mcp-setup` | MCP server installation only |
| `/maencof:maencof-connect` | Data source registration only |
| `/maencof:maencof-bridge` | **Install + register + workflow skill — all in one** |
...
Delegate to `/maencof:maencof-mcp-setup`:

```
Installing Slack MCP server...
  → Delegating to /maencof:maencof-mcp-setup
...
Delegate to `/maencof:maencof-craft-skill`:
...
3. Save to L4 via /maencof:maencof-remember (tags: slack, dev-updates)
4. Confirm save
```

### Step 5 — Register Data Source

Delegate to `/maencof:maencof-connect`:
...
```
/maencof:maencof-bridge
/maencof:maencof-bridge slack
/maencof:maencof-bridge github
```
...
|-----------|------------|
| MCP installation failure | Refer to `/maencof:maencof-mcp-setup` error handling; offer manual install |
...
| MCP connection failure | Verify token/permissions, suggest `/maencof:maencof-mcp-setup --verify` |
