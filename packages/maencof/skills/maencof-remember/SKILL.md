---
name: maencof-remember
user_invocable: true
description: "[maencof:maencof-remember] Records a concept, insight, or reference into the vault by recommending the right layer, extracting tags, deduplicating, and saving a properly structured markdown document with frontmatter."
argument-hint: "[content] [--layer 2-5] [--sub-layer NAME] [--title TITLE] [--tags TAGS] [--source URL] [--expires DATE]"
version: "1.0.0"
complexity: medium
context_layers: [2, 3, 4, 5]
orchestrator: maencof-remember skill
plugin: maencof
---
...
- L1 documents can only be created via `/maencof:maencof-setup` — not via remember.
...
- Document type (e.g., `tutorial`, `reference`, `maencof-insight`)
...
To explore related documents: /maencof:maencof-explore {tag}
```
...
```
/maencof:maencof-remember [content] [--layer <2-5>] [--sub-layer <name>] [--title <title>] [--tags <tag1,tag2>] [--source <url>] [--expires <YYYY-MM-DD>]
```
...
```
/maencof:maencof-remember If you leave the dependency array empty in a React hook, it only runs once on mount
/maencof:maencof-remember https://example.com/paper paper on spreading activation --layer 3
/maencof:maencof-remember PR review due tomorrow --layer 4 --expires 2026-12-31
/maencof:maencof-remember --title "TypeScript Generic Patterns" --tags typescript,generic,pattern
```

## Error Handling

- **Attempt to create L1**: "Layer 1 can only be created via `/maencof:maencof-setup`."
