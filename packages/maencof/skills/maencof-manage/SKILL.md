...
name: maencof-manage
user_invocable: true
description: "[maencof:maencof-manage] Audits, enables, disables, or deletes maencof skills and agents. Reports usage frequency to identify obsolete entries and supports bulk lifecycle operations across the registry."
argument-hint: "<list|report|disable|enable|delete|create> [name] [--skills|--agents|--all] [--days N] [--force] [--type skill|agent]"
version: "1.0.0"
complexity: medium
context_layers: []
orchestrator: maencof-manage skill
plugin: maencof
---

# maencof-manage — Skill and Agent Management
...
### list mode
```
/maencof:maencof-manage list [--skills|--agents|--all]
```
Display a list of all installed skills/agents and their activation status.

### report mode
```
/maencof:maencof-manage report [--days <N>]
```
...
### disable mode
```
/maencof:maencof-manage disable <name>
```
...
### enable mode
```
/maencof:maencof-manage enable <name>
```
Re-enable a disabled skill/agent.

### delete mode
```
/maencof:maencof-manage delete <name> [--force]
```
...
### create mode
```
/maencof:maencof-manage create <name> --type <skill|agent>
```
...
> **vs `/maencof:maencof-craft-skill` and `/maencof:maencof-craft-agent`**: `manage create` generates a minimal template for quick scaffolding. Use `/maencof:maencof-craft-skill` or `/maencof:maencof-craft-agent` for a full guided workflow with progressive disclosure, complexity tuning, and MCP integration.
...
>
> Note: `mcp_t_create` is intentionally excluded here. It creates vault knowledge documents
