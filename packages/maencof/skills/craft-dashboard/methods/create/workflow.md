...
0. Copy templates verbatim (recursive copies MUST include dotfiles):
   ...
   - templates/scripts/        -> <target>/scripts/   (read-api-port.mjs — aligns the Vite dev proxy with the backend's bound port)
   (The vault run-skill is NOT a template file — CREATE Phase 5 authors it into the vault from the pattern in "Run-skill generation".)
...
Turn 2 — Spec + config
   ...
   - backend/src/config.ts -> {
     ...
     ensureSpec() MUST:
       ...
       e. Resolve port: env PORT > 5174 (the PREFERRED port — the shipped
          server.ts binds the first FREE port at/after it, so a busy 5174
          auto-steps to 5175, …; config.ts only resolves the preferred value)
...
4. Generate the vault run-skill (see "Run-skill generation" below) unless
   --no-run-skill was passed.
5. Print hand-off recipe
6. Do NOT start a long-running server yourself. The generated run-skill (or
   `make serve`) is how the user launches it — both bind a free port and open
   the browser; craft-dashboard must not hold a server open during scaffold.
```
...
1. Resolve <vault> exactly as config.ts does:
   env VAULT_ROOT > spec.vaultPath > parent of <target>.
2. slug = kebab-case(basename(<target>))   (e.g. ./Dashboard -> "dashboard")
   skillDir = <vault>/.claude/skills/run-<slug>/
3. Confirm the write (it is outside <target>):
   - skillDir/SKILL.md already exists -> "Overwrite run-<slug> skill? [y/N]"
     (headless: skip with a logged note — never clobber a skill unprompted).
   - else -> "Generate run-<slug> skill in the vault? [Y/n]"
     (headless: auto-y + log the path).
4. mkdir -p skillDir.
5. Author skillDir/SKILL.md from the "Run-skill template" below, substituting:
     {{SKILL_SLUG}}     -> <slug>
     {{DASHBOARD_NAME}} -> the dashboard's human label (spec title, else <slug>)
     {{DASHBOARD_DIR}}  -> the ABSOLUTE <target> path
...
Launches the personal vault dashboard generated by `maencof:craft-dashboard`
at `{{DASHBOARD_DIR}}`. Production mode: the backend serves the built frontend
on a single port, so no proxy or second process is involved.

This skill was auto-generated at dashboard create time. The path above is baked
in; if you move the dashboard, re-run `/maencof:craft-dashboard` or edit it here.

## Steps

1. `DIR="{{DASHBOARD_DIR}}"`. If `$DIR` does not exist, tell the user the
   dashboard appears to have been moved or deleted, and stop.
2. If `$DIR/node_modules` OR `$DIR/backend/node_modules` is missing, run
   `cd "$DIR" && make install` as a background job and wait for it to finish.
3. Start the server as a BACKGROUND job (it is long-lived — never block the
   session on it):
   ...
   `make serve` rebuilds the frontend, binds the first free port at or after
   5174 (auto-stepping past any in-use port), writes the chosen port to
   `$DIR/.dashboard-runtime.json`, and opens the browser.

4. Wait until `$DIR/.dashboard-runtime.json` exists and `GET /api/health` on
   its port returns `{"ok":true}`, then report the final URL
   (`http://127.0.0.1:<port>/`) to the user.
5. The browser opens automatically. If it did not (headless / no desktop
   environment), give the user the URL to open manually.

## Stopping

The server keeps running in the background. To stop it, read the `pid` from
`$DIR/.dashboard-runtime.json` and `kill` it, or stop the background job.

## Notes

- Port conflicts are handled automatically; the dashboard never fails to start
  because 5174 is taken — it steps to the next free port and the browser opens
  to whatever port it bound.
- The dashboard reads the vault read-only. It never writes to the vault or to
  `.maencof/`.
````
...
Run it:     /run-<slug>                       (generated vault skill — builds,
                                                binds a free port, opens browser)
            cd <target> && make serve         (same thing, run manually)

Dev mode:   cd <target> && make dev-backend   (terminal 1 — Fastify, first free
                                                port at/after 5174)
            cd <target> && make dev-frontend  (terminal 2 — Vite opens the
                                                browser; proxy follows the
                                                backend's bound port)

Ports are auto-selected: a busy 5174 steps to 5175, …, and the browser opens to
whatever port the backend actually bound. The bound port + URL are written to
...
Stack-defining majors are caret-pinned (`^N`) in the templates so the API surface stays compatible across `npm install`. **Framework-coupled plugins are NOT leaf libraries** — co-pin each to the highest plugin major that still supports the framework's pinned major:

- `@fastify/static` (`^7`), `@fastify/cors` (`^8`), and `fastify-sse-v2` (`^4`) target the Fastify-4-compatible majors. Each enforces its Fastify range at `app.register()` time, so leaving them `"latest"` pulls Fastify-5-only majors and the backend crashes at boot with `FST_ERR_PLUGIN_VERSION_MISMATCH` (install + `tsc` still pass, so the failure only shows when the server runs).
- `@vitejs/plugin-react` (`^5`) targets the Vite-6-compatible majors. `@vitejs/plugin-react@latest` (6.x) declares `vite ^8` as its peer, so against the pinned `vite ^6` the user's `npm install` fails with an `ERESOLVE` peer conflict — never resolve it with `--force`/`--legacy-peer-deps`; v5 is the highest major whose peer range still includes `vite ^6`.

Leaf libraries (chokidar, gray-matter, markdown-it, recharts, zustand, dayjs, fuse.js, fast-glob) ship as `"latest"` and the user's `npm install` writes a fresh lockfile. `--pin <version>` rewrites the `"latest"` entries to the given concrete version; it MAY also override the caret-pinned majors when the user opts in explicitly.
...
[ ] scripts/read-api-port.mjs present at <target>/scripts/
[ ] (unless --no-run-skill) <vault>/.claude/skills/run-<slug>/SKILL.md exists
    with every {{placeholder}} substituted (no literal "{{" remains)
