...
1. **Q3 (Boundary)**: User selects "나중에 정할게"
   - AI responds with T2-SKIP: "이 질문은 건너뛰겠습니다. 나중에 /maencof:maencof-setup --step 2 로 다시 설정할 수 있습니다."
...
3. **Result**: Setup completes successfully with 6 of 7 stages.
   - User can later run `/maencof:maencof-setup --step 2` to set the boundary
   - User can later run `/maencof:maencof-setup --step 3` to create a companion
