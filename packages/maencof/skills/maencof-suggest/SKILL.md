---
name: maencof-suggest
user_invocable: true
description: "[maencof:maencof-suggest] Identifies missing knowledge graph connections using Spreading Activation and Jaccard similarity, recommending new edges to fill structural gaps rather than general graph traversal."
...
# maencof-suggest — Link Suggestion
...
> **Difference from `/maencof:maencof-explore`**: explore performs SA-based traversal from a seed node. suggest focuses on recommending new connections that don't yet exist.
...
- Resolve orphan nodes detected by `/maencof:maencof-checkup`
...
```
/maencof:maencof-suggest [path|tags|text] [--max <1-20>] [--min-score <0.0-1.0>]
```
...
```
/maencof:maencof-suggest 02_Derived/typescript-patterns.md
/maencof:maencof-suggest react,hooks,state-management
/maencof:maencof-suggest knowledge graph traversal algorithms --max 10
```
