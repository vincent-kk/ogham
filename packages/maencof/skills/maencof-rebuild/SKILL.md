...
complexity: simple
context_layers: []
orchestrator: maencof-rebuild skill
plugin: maencof
---

# maencof-rebuild — Force Full Index Rebuild

Ignores the existing `.maencof/` index cache and rebuilds the entire knowledge graph from scratch.
This is the force mode of `/maencof:maencof-build`.
...
```
/maencof:maencof-rebuild [options]
```
...
/maencof:maencof-rebuild --force
```

## Difference from `/maencof:maencof-build`

| | `/maencof:maencof-build` | `/maencof:maencof-rebuild` |
|---|---|---|
