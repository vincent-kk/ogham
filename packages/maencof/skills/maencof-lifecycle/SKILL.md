...
# maencof-lifecycle — Dynamic Hook Action Management
...
- maencof vault initialized (`.maencof-meta/` exists) — run `/maencof:maencof-setup` if not
...
```
/maencof:maencof-lifecycle [mode] [id]
```
