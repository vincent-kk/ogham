...
#
# Priority:
#   0. Cached path in ~/.claude/plugins/maencof/node-path-cache (skip full search)
...
# ---------------------------------------------------------------------------
CACHE_FILE="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/plugins/maencof/node-path-cache"
...
if [ -n "$NODE_BIN" ]; then
  ...
  if [ "$_node_major" -lt 20 ]; then
    printf '[maencof] Warning: Found node %s (%s) but require >= 20. Skipping.\n' \
...
# ---------------------------------------------------------------------------
if [ -z "$NODE_BIN" ]; then
  printf '[maencof] Error: Could not find node binary. Ensure Node.js >= 20 is installed.\n' >&2
