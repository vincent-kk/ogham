...
`;function Vt(e){let t=e.cwd??process.cwd(),n=[];if(!nt(t))return{continue:!0,hookSpecificOutput:{hookEventName:"SessionStart",additionalContext:"[maencof] Vault is not initialized. Run `/maencof:setup` to get started."}};let r=Ir(t,n);r&&n.push(`[maencof:${r.name}] ${r.greeting}`),Cr(t,r?.name,n);try{let o=Ft(t);o.created.length>0&&n.push(`[maencof] Config files provisioned: ${o.created.join(", ")}`),o.migrated.length>0&&n.push(`[maencof] Config schemas updated: ${o.migrated.join(", ")}`)}catch(o){m(t,{hook:"session-start",error:String(o),timestamp:new Date().toISOString()})}kr(t,n),Or(t,n);let i=y(t,"wal.json");A(i)&&n.push("[maencof] Incomplete transaction (WAL) detected from a previous session. Run `/maencof:checkup` to diagnose.");let s=y(t,"schedule-log.json");if(A(s))try{let o=JSON.parse(w(s,"utf-8"));o.pending&&o.pending.length>0&&n.push(`[maencof] ${o.pending.length} pending task(s) found. Run \`/maencof:organize\` to process.`)}catch(o){m(t,{hook:"session-start",error:String(o),timestamp:new Date().toISOString()})}let a=y(t,"sessions");if(A(a)){let o=Ar(a);o&&n.push(`[maencof] Previous session summary:
${o}`)}let c=y(t,"data-sources.json");try{let o=w(c,"utf-8"),u=JSON.parse(o);(!u.sources||u.sources.length===0)&&n.push("[maencof] No external data sources connected. Run `/maencof:connect` to set up.")}catch(o){m(t,{hook:"session-start",error:String(o),timestamp:new Date().toISOString()}),n.push("[maencof] No external data sources connected. Run `/maencof:connect` to set up.")}try{let o=kt(t);o.message&&n.push(`[maencof] ${o.message}`)}catch(o){m(t,{hook:"session-start",error:String(o),timestamp:new Date().toISOString()})}try{let o=se(t);o.enabled&&n.push(Ot(o));let u=Ct(t);if(u&&u.captures.length>0){let f=u.captures.filter(p=>p.layer===2).length,d=u.captures.filter(p=>p.layer===5).length,g=u.captures.map(p=>`  - L${p.layer}: "${p.title}"`).join(`
`);n.push(`\u{1F4A1} \uC9C0\uB09C \uC138\uC158\uC5D0\uC11C ${u.captures.length}\uAC1C \uC778\uC0AC\uC774\uD2B8\uB97C \uC790\uB3D9 \uCEA1\uCC98\uD588\uC2B5\uB2C8\uB2E4 (L2: ${f}, L5: ${d}):
${g}
/maencof:insight --recent \uB85C \uD655\uC778\uD558\uC138\uC694.`),wt(t)}}catch(o){m(t,{hook:"session-start",error:String(o),timestamp:new Date().toISOString()})}try{let o=e.session_id??"unknown";ft(t,{time:dt(new Date),category:"session",description:`Session started (session_id: ${o})`})}catch(o){m(t,{hook:"session-start",error:String(o),timestamp:new Date().toISOString()})}let l={continue:!0};try{let o=vr(t),u=n.length>0?n.join(`
...
${t}
</${Q}>`}function Ir(e,t){let n=y(e,"companion-identity.json");if(!A(n))return null;try{let r=JSON.parse(w(n,"utf-8"));return At(r)?{name:r.name,greeting:r.greeting}:(t.push("[maencof] companion-identity.json present but invalid \u2014 missing `name` or `greeting`. Run `/maencof:setup --reset --companion` to repair."),m(e,{hook:"session-start",error:"companion-identity.json failed validation (missing name/greeting)",timestamp:new Date().toISOString()}),null)}catch(r){return m(e,{hook:"session-start",error:String(r),timestamp:new Date().toISOString()}),null}}function Cr(e,t,n){let r=!1;try{let i=rt(e),s=ut(i),a=Ut(e);if(s===null){let c=Z(e,t);ee(i,c,{createIfMissing:!0}),jt(e,v),n.push("[maencof] maencof directives initialized in CLAUDE.md."),r=!0}else if(a===null)jt(e,v),r=!0;else if(a!==v){let c=Z(e,t);ee(i,c,{createIfMissing:!1}),wr(e,a),n.push(`[maencof] CLAUDE.md directives updated (${a} \u2192 ${v}).`),r=!0}}catch(i){m(e,{hook:"session-start",error:String(i),timestamp:new Date().toISOString()})}return r}function Ut(e){let t=y(e,"version.json");if(!A(t))return null;try{return JSON.parse(w(t,"utf-8")).version??null}catch(n){return m(e,{hook:"session-start",error:String(n),timestamp:new Date().toISOString()}),null}}function jt(e,t){let n=y(e,"version.json"),r={version:t,installedAt:new Date().toISOString(),migrationHistory:[]};$t(n,JSON.stringify(r),"utf-8")}function wr(e,t){let n=y(e,"version.json"),r;try{r=JSON.parse(w(n,"utf-8"))}catch(i){m(e,{hook:"session-start",error:String(i),timestamp:new Date().toISOString()}),r={version:t,installedAt:new Date().toISOString(),migrationHistory:[]}}r.migrationHistory.push(`${t} -> ${v}`),r.version=v,r.lastMigratedAt=new Date().toISOString(),$t(n,JSON.stringify(r),"utf-8")}function kr(e,t){try{let n=y(e,"version.json"),r="1.0.0";A(n)&&(r=JSON.parse(w(n,"utf-8")).architecture_version??"1.0.0"),r!==X&&t.push(`[maencof] Architecture update available (${r} \u2192 ${X}).
L3 sub-layers (relational/structural/topical) and L5 sub-layers (buffer/boundary) are now supported.
Run \`/maencof:migrate\` to upgrade your vault structure.`)}catch(n){m(e,{hook:"session-start",error:String(n),timestamp:new Date().toISOString()})}}function Or(e,t){try{let n=Ut(e);n!==null&&n!==v&&t.push(`[maencof] Plugin updated (${n} \u2192 ${v}).
Run \`/maencof:setup\` to complete the migration.`)}catch(n){m(e,{hook:"session-start",error:String(n),timestamp:new Date().toISOString()})}}function Ar(e){try{let t=Er(e).filter(s=>s.endsWith(".md")).sort().reverse();if(t.length===0)return null;let n=_r(e,t[0]);return w(n,"utf-8").split(`
`).slice(0,10).join(`
