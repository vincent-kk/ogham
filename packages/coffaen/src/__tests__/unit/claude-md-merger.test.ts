...
beforeEach(() => {
  testDir = join(tmpdir(), `coffaen-test-${Date.now()}`);
...
/** 테스트용 coffaen 섹션 내용 */
const SAMPLE_COFFAEN_CONTENT = `# coffaen Knowledge Space

## Active Configuration
- Knowledge Path: ./knowledge/
- Autonomy Level: 0`;

describe('mergeCoffaenSection — 파일 없음', () => {
  it('파일이 없으면 새로 생성해야 한다', () => {
    ...
    expect(content).toContain(COFFAEN_START_MARKER);
    expect(content).toContain(COFFAEN_END_MARKER);
  ...
  it('dryRun=true이면 파일을 실제로 생성하지 않아야 한다', () => {
    ...
    expect(result.changed).toBe(true);
    expect(existsSync(claudeMdPath)).toBe(false);
    expect(result.content).toContain(COFFAEN_START_MARKER);
  });
});

describe('mergeCoffaenSection — 마커 없는 기존 파일', () => {
  ...
  it('기존 내용을 보존하고 coffaen 섹션을 끝에 추가해야 한다', () => {
    const result = mergeCoffaenSection(claudeMdPath, SAMPLE_COFFAEN_CONTENT);
  ...
  it('coffaen 섹션이 기존 내용 뒤에 위치해야 한다', () => {
    mergeCoffaenSection(claudeMdPath, SAMPLE_COFFAEN_CONTENT);

    const content = readFileSync(claudeMdPath, 'utf-8');
    const existingIdx = content.indexOf('# My Project');
    const markerIdx = content.indexOf(COFFAEN_START_MARKER);
    expect(existingIdx).toBeLessThan(markerIdx);
  });
});

describe('mergeCoffaenSection — 기존 coffaen 섹션 업데이트', () => {
  const oldCoffaenContent = `# coffaen Knowledge Space\n\n- Knowledge Path: ./old-knowledge/\n- Autonomy Level: 0`;
  const existingContent = `# My Project\n\nSome content.\n\n${COFFAEN_START_MARKER}\n${oldCoffaenContent}\n${COFFAEN_END_MARKER}\n`;
  ...
  it('기존 coffaen 섹션을 새 내용으로 교체해야 한다', () => {
    const result = mergeCoffaenSection(claudeMdPath, SAMPLE_COFFAEN_CONTENT);
...
describe('readCoffaenSection', () => {
  it('파일이 없으면 null을 반환해야 한다', () => {
    expect(readCoffaenSection(claudeMdPath)).toBeNull();
  });
  ...
  it('coffaen 섹션 내용을 반환해야 한다', () => {
    const content = `# Project\n\n${COFFAEN_START_MARKER}\n${SAMPLE_COFFAEN_CONTENT}\n${COFFAEN_END_MARKER}\n`;
    writeFileSync(claudeMdPath, content, 'utf-8');

    const section = readCoffaenSection(claudeMdPath);
    expect(section).not.toBeNull();
    expect(section).toContain('Knowledge Path: ./knowledge/');
  });
});

describe('removeCoffaenSection', () => {
  it('파일이 없으면 false를 반환해야 한다', () => {
    expect(removeCoffaenSection(claudeMdPath)).toBe(false);
  });
  ...
  it('coffaen 섹션을 제거하고 나머지 내용을 보존해야 한다', () => {
    const content = `# My Project\n\nSome content.\n\n${COFFAEN_START_MARKER}\n${SAMPLE_COFFAEN_CONTENT}\n${COFFAEN_END_MARKER}\n`;
    writeFileSync(claudeMdPath, content, 'utf-8');

    const result = removeCoffaenSection(claudeMdPath);
    ...
    expect(newContent).not.toContain(COFFAEN_START_MARKER);
    expect(newContent).not.toContain(COFFAEN_END_MARKER);
  });
});
...
describe('ClaudeMdMerger 클래스', () => {
  it('merge/read/remove가 순서대로 동작해야 한다', () => {
    ...
    // 삽입
    const mergeResult = merger.merge(SAMPLE_COFFAEN_CONTENT);
