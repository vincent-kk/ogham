...
async function loadGraphIfNeeded(vaultPath: string): Promise<KnowledgeGraph | null> {
...
export function createServer(): McpServer {
  ...
  server.registerTool(
    'coffaen_create',
    {
      ...
      inputSchema: z.object({
        layer: z.number().int().min(1).max(4).describe('문서 Layer (1=Core, 2=Derived, 3=External, 4=Action)'),
        ...
        filename: z.string().optional().describe('파일명 힌트 (선택, 미지정 시 자동 생성)'),
        source: z.string().optional().describe('외부 출처 (Layer 3용)'),
        expires: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional().describe('만료일 YYYY-MM-DD (Layer 4용)'),
      }),
    },
    async (args) => {
      try {
  ...
  server.registerTool(
    'coffaen_read',
    {
      ...
      inputSchema: z.object({
        ...
        depth: z.number().int().min(1).max(10).optional().describe('SA 홉 수 (기본 2)'),
        include_related: z.boolean().optional().describe('관련 문서 포함 여부 (기본 true)'),
      }),
    },
    async (args) => {
      try {
  ...
  server.registerTool(
    'coffaen_delete',
    {
      ...
      inputSchema: z.object({
        path: z.string().describe('문서 경로'),
        force: z.boolean().optional().describe('backlink 경고 무시 여부 (기본 false)'),
      }),
    },
    async (args) => {
      try {
  ...
  server.registerTool(
    'coffaen_move',
    {
      ...
      inputSchema: z.object({
        path: z.string().describe('문서 경로'),
        target_layer: z.number().int().min(1).max(4).describe('목표 Layer (1-4)'),
        reason: z.string().optional().describe('전이 사유'),
        confidence: z.number().min(0).max(1).optional().describe('신뢰도 (Layer 3→2 전이 시)'),
      }),
    },
    async (args) => {
      try {
  ...
  server.registerTool(
    'kg_search',
    {
      ...
      inputSchema: z.object({
        seed: z.array(z.string()).min(1).describe('시드 노드 (경로 또는 키워드)'),
        max_results: z.number().int().min(1).max(100).optional().describe('최대 반환 수 (기본 10)'),
        decay: z.number().min(0).max(1).optional().describe('감쇠 인자 (기본 0.7)'),
        threshold: z.number().min(0).max(1).optional().describe('발화 임계값 (기본 0.1)'),
        max_hops: z.number().int().min(1).max(20).optional().describe('최대 홉 수 (기본 5)'),
        layer_filter: z.array(z.number().int().min(1).max(4)).optional().describe('Layer 필터'),
      }),
    },
    async (args) => {
      try {
  ...
  server.registerTool(
    'kg_navigate',
    {
      ...
      inputSchema: z.object({
        ...
        include_inbound: z.boolean().optional().describe('인바운드 링크 포함 (기본 true)'),
        include_outbound: z.boolean().optional().describe('아웃바운드 링크 포함 (기본 true)'),
        include_hierarchy: z.boolean().optional().describe('부모/자식/형제 포함 (기본 true)'),
      }),
    },
    async (args) => {
      try {
  ...
  server.registerTool(
    'kg_context',
    {
      ...
      inputSchema: z.object({
        ...
        token_budget: z.number().int().min(100).max(10000).optional().describe('토큰 예산 (기본 2000)'),
        include_full: z.boolean().optional().describe('상위 N개 전문 포함 (기본 false)'),
      }),
    },
    async (args) => {
      try {
