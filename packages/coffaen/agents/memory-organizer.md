...
description: >
  coffaen Memory Organizer — Evaluates and executes document transitions (inter-Layer moves)
...
# Memory Organizer — coffaen Knowledge Transition Agent
...
[execute module]
  ...
  Responsibility: call coffaen_move, update links, update Frontmatter
...
2. Move files via coffaen_move
3. Update Frontmatter layer field via coffaen_update
...
- `/coffaen:organize` — full workflow entry point
- `/coffaen:reflect` — judge module result report only (execute not run)
