...
This skill can run the MCP setup step of `/coffaen:setup` independently.
...
> This file contains personal user settings that coffaen will never modify.
...
Next steps:
  - Verify after setting token: `/coffaen:mcp-setup --verify`
  - Start data ingestion: `/coffaen:ingest`
```
...
- `packages/coffaen/.mcp.json` — Plugin Area (coffaen's own MCP server configuration)
- All files under `packages/coffaen/` — Plugin Area

## Options

```
/coffaen:mcp-setup [options]
```
