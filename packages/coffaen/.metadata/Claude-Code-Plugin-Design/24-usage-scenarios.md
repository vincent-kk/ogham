...
두 경로 모두 동일한 core/ 알고리즘과 .coffaen/ 인덱스를 사용한다.
...
### H1: 명시적 검색
`/coffaen:explore "키워드"` → SA 실행 → 마크다운 테이블 결과.

### H2: 네비게이션 맵
`/coffaen:explore /path/to/doc.md` → 연결 맵 (인/아웃 링크, PageRank, WP 유사도).

### H3: 인덱스 건강도 진단
`/coffaen:diagnose` → 고아 문서, 깨진 링크, 순환 참조 보고서 + 권장 조치.
