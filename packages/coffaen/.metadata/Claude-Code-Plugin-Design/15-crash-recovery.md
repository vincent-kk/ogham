...
```
1. WAL 파일 생성: .coffaen-meta/wal.json
...
### 인덱스 빌드
`.coffaen/` 빌드 중 크래시 → "인덱스는 파생물" 원칙으로 재빌드. WAL 불필요.
...
### .coffaen/ 캐시 보호
- `.coffaen/` 전체가 파생물. 삭제 후 `/coffaen:build`로 완전 복구
- `.coffaen/.lock` 파일로 동시 빌드 방지
