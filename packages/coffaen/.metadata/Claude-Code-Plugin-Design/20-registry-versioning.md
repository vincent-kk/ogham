...
```
packages/coffaen/
├── skills/{skill-name}.md      → /coffaen:{skill-name}
...
| 우선순위 | 위치 | 유형 |
|---------|------|------|
| 1 | `.coffaen-meta/custom-skills/` | 사용자 정의 스킬 |
| 2 | `.coffaen-meta/custom-agents/` | 사용자 정의 에이전트 |
...
```
/coffaen:create-skill 호출 → 인터뷰 (이름, 목적, context_layers)
  → .coffaen-meta/custom-skills/{이름}.md 생성
  → 즉시 /coffaen:{이름} 사용 가능
```
