...
사용자가 `/coffaen:명령` 형태로 호출하는 확장 기능. 지식 트리 + 검색 엔진 컨텍스트를
...
| 스킬 | 호출 | 역할 | Phase |
|------|------|------|-------|
| build | `/coffaen:build` | 전체/증분 인덱스 구축 | 1 |
| explore | `/coffaen:explore` | 대화형 SA 기반 지식 탐색 | 1 |
| diagnose | `/coffaen:diagnose` | 인덱스 건강도 진단 (고아, 깨진 링크) | 1 |
| rebuild | `/coffaen:rebuild` | 인덱스 강제 전체 재구축 | 1 |
