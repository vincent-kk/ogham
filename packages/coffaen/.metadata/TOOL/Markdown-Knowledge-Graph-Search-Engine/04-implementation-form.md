...
사용자가 `/coffaen:명령어` 형태로 명시적으로 호출하는 워크플로우이다.
...
|                    사용자 / AI 에이전트                     |
|                                                           |
|  Skill (/coffaen:build)    Hook (UserPromptSubmit)        |
|  Skill (/coffaen:explore)  Hook (PostToolUse:Write)       |
|         |                         |                       |
|         v                         v                       |
|  +-------------+          +----------------+              |
|  | 오프라인     |          | 온라인          |              |
...
**인덱스 위치**: `.coffaen/` (저장소 루트 또는 지정 경로)

**동시 접근 제어**:
- 읽기(MCP, Hook): 항상 허용. 메모리에 로드된 스냅샷 사용.
- 쓰기(Skill/CLI): `.coffaen/.lock` 파일로 배타적 접근. 빌드 중 다른 빌드 차단.
- 무효화(Hook): `.coffaen/stale-nodes.json`에 무효화된 노드 목록 추가 (append-only, 락 불필요).
...
| 스킬 | 설명 | Phase |
|------|------|-------|
| `/coffaen:build` | 전체/증분 인덱스 구축 | 1 |
| `/coffaen:explore` | 대화형 지식 탐색 세션 | 1 |
| `/coffaen:diagnose` | 인덱스 건강도 진단 (고아 문서, 깨진 링크) | 1 |
| `/coffaen:rebuild` | 인덱스 강제 전체 재구축 | 1 |
