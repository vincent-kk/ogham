...
/prawf:review --workdir ~/reviews  # 산출물 루트 고정 (또는 PRAWF_WORKDIR)
...
**건전성 단독 verdict.** verdict 는 _미해결 건전성_ finding 만의 순수 함수입니다.
...
`/prawf:review` 는 `<workdir>/review/<paper-slug>/` 아래에 `review-report.md`(finding
