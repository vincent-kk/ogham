...
```
Phase 1 → validate  (imbas-analyst)
Phase 2 → split     (imbas-planner + imbas-analyst reverse-inference)
Phase 2.5 → manifest-stories (batch Jira/GitHub/local create)
Phase 3 → devplan   (imbas-engineer, EARS Subtask generation)
...
- **Agent separation**: `imbas-analyst` (sonnet, 검증), `imbas-planner` (sonnet, 분할), `imbas-engineer` (opus, maxTurns:80, 코드 탐색)
...
| `src/mcp/tools/` | 12개 MCP 도구 (run_create, manifest_save 등) |
| `src/hooks/` | pre-tool-use, context-injector, session-cleanup |

### Skills (12)

`imbas-pipeline`, `imbas-validate`, `imbas-split`, `imbas-devplan`, `imbas-manifest`, `imbas-digest`, `imbas-fetch-media`, `imbas-read-issue`, `imbas-scaffold-pr`, `imbas-setup`, `imbas-status`, `imbas-cache`

### Agents (4)

`imbas-analyst` (sonnet), `imbas-planner` (sonnet), `imbas-engineer` (opus, maxTurns:80), `imbas-media` (멀티모달)
...
**알려진 한계**: `imbas-engineer` (opus, maxTurns:80) subagent-internal 컨텍스트 소진은 preamble로 해결 불가. 체크포인트 파일 계약 follow-up 이슈 참조. 신규 skill 추가 시 `packages/filid/skills/filid-pipeline/SKILL.md:11-24` 3-layer 패턴을 참조.
