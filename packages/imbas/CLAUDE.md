...
| `src/mcp/tools/` | 15개 MCP 도구 (run_create, manifest_save 등) |
| `src/hooks/` | pre-tool-use, context-injector, session-cleanup |

### Skills (11)

`imbas-pipeline`, `imbas-validate`, `imbas-split`, `imbas-devplan`, `imbas-manifest`, `imbas-digest`, `imbas-read-issue`, `imbas-scaffold-pr`, `imbas-setup`, `imbas-status`, `imbas-cache`
