...
| `hooks/`                     | pre-tool-use / context-injector / session-cleanup |
| `skills/`                    | 사용자 스킬 디렉토리                              |
...
- 4-Phase 파이프라인: validate → split → manifest-stories → devplan → manifest-devplan
- provider 추상화 (`jira` / `github` / `local`); 실행 state 는 `.imbas/runs/<id>/`
