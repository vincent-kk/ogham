...
// Errors append to ~/.claude/plugins/imbas/error-log.json so silent hook
// failures (e.g. Windows PATH lacks node) become observable.
const probe = await selfProbe({ writeLog: true, pkg: 'imbas' });
...
if (probe.errors.length > 0) {
  const warning =
    '[imbas] hook bootstrap diagnostic — some hooks may not work:\n' +
    probe.errors.map((e) => `  - ${e}`).join('\n') +
    '\nSee ~/.claude/plugins/imbas/error-log.json for details.';
