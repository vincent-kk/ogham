...
- `validate` → 항상 시작 가능
- `split` → validate가 통과(PASS 또는 PASS_WITH_WARNINGS)한 후에만
- `devplan` → split이 완료되고 Story 리뷰가 끝난 후에만
...
| 세션 종료 시 | 임시 파일 정리 | `.imbas/.temp/` 작업 디렉토리 제거 |
...
| `/imbas:digest` | Yes | Jira 이슈를 구조화된 요약으로 압축 |
| `/imbas:fetch-media` | Yes | 미디어 첨부파일 다운로드 및 분석 |
| `/imbas:cache` | No | 내부: Jira 메타데이터 캐시 관리 (24시간 TTL) |
| `/imbas:read-issue` | No | 내부: Jira 이슈 컨텍스트 읽기 및 구조화 |
...
분할(Split) 단계에서:
- **INVEST 위반** — 너무 크거나, 독립적으로 전달할 수 없거나, 테스트 가능성이 없는 Story
