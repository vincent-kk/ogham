...
`mcp_tools_ast_search`/`mcp_tools_ast_analyze`에서 `sgLoadError` 발생 시:
...
| `mcp_tools_run_transition` | imbas MCP | 단계 시작/완료 전이 |
| `mcp_tools_manifest_get` | imbas MCP | stories-manifest 로드 |
| `mcp_tools_manifest_save` | imbas MCP | devplan-manifest 저장 |
| `mcp_tools_manifest_validate` | imbas MCP | 매니페스트 유효성 검증 |
| `mcp_tools_ast_search` | imbas MCP | AST 패턴 검색 |
| `mcp_tools_ast_analyze` | imbas MCP | 의존성 그래프/복잡도 분석 |
