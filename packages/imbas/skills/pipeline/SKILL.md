...
/imbas:pipeline requirements.md --codebase /path/to/repo
...
/imbas:pipeline PROJ-42 --codebase /path/to/repo
/imbas:pipeline PROJ-42,PROJ-43,PROJ-44 --codebase /path/to/repo
```
  ...
  >>> CODEBASE CHECK: --codebase resolved?
      NO  → STOP (planning-only complete). Emit planning report with resume guidance:
            "Planning pipeline complete. N Stories created.
             To generate Subtasks: /imbas:pipeline <source> --codebase /path/to/repo
             Or individually: /imbas:devplan --run <run-id> --codebase /path/to/repo"
