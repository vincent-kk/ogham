...
`ast_search`/`ast_analyze`에서 `sgLoadError` 발생 시:
...
| `run_transition` | imbas MCP | 단계 시작/완료 전이 |
| `manifest_get` | imbas MCP | stories-manifest 로드 |
| `manifest_save` | imbas MCP | devplan-manifest 저장 |
| `manifest_validate` | imbas MCP | 매니페스트 유효성 검증 |
| `ast_search` | imbas MCP | AST 패턴 검색 |
| `ast_analyze` | imbas MCP | 의존성 그래프/복잡도 분석 |
