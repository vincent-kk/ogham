...
Replaces `imbas:imbas-split` skill Step 7 (interactive user review). This is the key pipeline innovation.
...
[ ] Manifest validation passes
    - mcp_tools_manifest_validate(project_ref, run_id, type: "stories") returns 0 errors
...
- Call mcp_tools_run_transition(complete_phase, split, pending_review: false, stories_created: N)
...
- Call mcp_tools_run_transition(escape_phase, split, escape_code: "E2-3")
...
Escape conditions (except E2-3):
  ...
  EC-2 (source defect)
    ...
    → Resume: fix document, re-validate (/imbas:imbas-validate), then re-run pipeline
...
Manifest validation errors:
  → List all validation errors from mcp_tools_manifest_validate
```
...
Replaces `imbas:imbas-devplan` skill Step 4 (interactive user review).
...
```
[ ] Manifest validation passes
    - mcp_tools_manifest_validate(project_ref, run_id, type: "devplan") returns 0 errors
...
When all criteria pass:
- Call mcp_tools_run_transition(complete_phase, devplan, pending_review: false)
...
PARTIAL:
  ...
  → Suggest: "/imbas:imbas-manifest <type> --run <run-id>" to retry (idempotent — skips created items)
...
EXCEPTION — Phase 2.5 (manifest-stories) failure:
  ...
  → Resume: "/imbas:imbas-manifest stories --run <run-id>" then manual devplan
```
