...
1. **이슈 읽기** — `imbas:imbas-read-issue`를 사용하여 이슈 메타데이터(키, 요약, 타입)를 조회한다.
...
| 도구 | 출처 | 용도 |
|------|------|------|
| `imbas:imbas-read-issue` | imbas 내부 스킬 | 이슈 메타데이터 및 서브태스크 조회 |
| `mcp_tools_config_get` | imbas MCP | provider 설정 확인 |
