...
기획 문서(로컬 마크다운 또는 Confluence 페이지)를 `imbas-analyst` 에이전트가 4가지 관점에서 검증하여
...
3. **imbas-analyst 에이전트 실행** — 4가지 검증 수행, `validation-report.md` 작성
...
| `imbas-analyst` | 에이전트(sonnet) | 4가지 검증 수행 |
