...
기획 문서(로컬 마크다운 또는 Confluence 페이지)를 `imbas-analyst` 에이전트가 5가지 관점에서 검증하여
...
3. **imbas-analyst 에이전트 실행** — 5가지 검증 수행, `validation-report.md` 작성
...
| 도구 | 출처 | 용도 |
|------|------|------|
| `imbas_run_create` | imbas MCP | 런 디렉토리 생성, 소스 복사, state.json 초기화 |
| `imbas_run_get` | imbas MCP | 런 상태 읽기 |
| `imbas_run_transition` | imbas MCP | 단계 전이 (시작/완료) |
| `getConfluencePage` | Atlassian MCP | Confluence 페이지 조회 |
| `searchConfluenceUsingCql` | Atlassian MCP | 참조 문서 검색 |
| `imbas-analyst` | 에이전트(sonnet) | 5가지 검증 수행 |
