...
Phase 1(validate)에서 통과한 기획 문서를 `imbas-planner` 에이전트가 INVEST 원칙에 맞는 Story로 분할하고,
...
3. **imbas-planner 에이전트 실행** — INVEST 준수 Story 목록 생성 (User Story + AC 형식)
4. **3→1→2 검증** (Story별)
   - [3] 앵커 링크 확인 — 원본 문서 섹션 참조 존재 여부
   - [1] 일관성 확인 — 문서 목표와의 정합성
   - [2] 역추론 검증 — `imbas-analyst`가 Story를 재조합하여 원본과 비교
...
| `imbas_run_transition` | imbas MCP | 단계 전이 (시작/완료/이스케이프) |
| `imbas_manifest_save` | imbas MCP | stories-manifest 저장 |
| `imbas_manifest_validate` | imbas MCP | 매니페스트 유효성 검증 |
| `getJiraIssue` | Atlassian MCP | Epic 존재 확인 |
| `imbas-planner` | 에이전트(sonnet) | INVEST Story 분할 |
| `imbas-analyst` | 에이전트(sonnet) | 역추론 검증 |
