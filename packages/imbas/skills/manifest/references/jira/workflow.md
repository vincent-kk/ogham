...
2. For each `issue_ref`, call `mcp__plugin_imbas_atlassian__getJiraIssue(issue_ref)`.
...
updated `status` / `issue_ref` via `manifest_save`. This is the crash-recovery
...
- If manifest has `epic_ref == null` and an Epic entry exists:
  1. Call `createJiraIssue(project: KEY, type: "Epic", summary, description)`.
...
For each story in `manifest.stories` where `status == "pending"`:
  1. Call `createJiraIssue(project: KEY, type: "Story", summary: story.title, description: story.description, parent: epic_ref)`.
...
For each link in `manifest.links` where `status == "pending"`:
  ...
  - For EACH target in `link.to`:
    ...
    2. Call `createIssueLink(type: link.type, inwardIssue: resolve(link.from), outwardIssue: resolve(target))`.
  - Update link `status`:
...
For each task in `manifest.tasks` where `status == "pending"`:
  1. `createJiraIssue(project: KEY, type: "Task", summary: task.title, description: task.description)`.
...
For each task, for each subtask in `task.subtasks` where `status == "pending"`:
  1. `createJiraIssue(project: KEY, type: "Sub-task", summary, description, parent: task.issue_ref)`.
...
For each task, for each `blocked_story_id` in `task.blocks`:
  ...
  2. `createIssueLink(type: "Blocks", inwardIssue: task.issue_ref, outwardIssue: story_issue_ref)`.
...
where `status == "pending"`:
  1. `createJiraIssue(project: KEY, type: "Sub-task", summary, description, parent: entry.story_key)`.
  2. Update subtask: `status = "created"`, `issue_ref = <returned key>`.
  3. Save manifest immediately.

#### Step 5 — add_feedback_comments
For each comment in `manifest.feedback_comments` where `status == "pending"`:
  1. `addCommentToJiraIssue(issueIdOrKey: comment.target_ref, body: comment.comment)`.
