...
1. **이슈 읽기** — `imbas:imbas-read-issue` 스킬로 전체 이슈 로드, 첨부 미디어는 `atlassian:atlassian-media-analysis`로 분석
...
- `imbas:imbas-manifest`를 통한 Done 전이 발생
- 댓글 3개 이상
- 작성자 2명 이상
...
| 도구 | 출처 | 용도 |
|------|------|------|
| `[OP: get_issue]` | Jira ([OP:]) | 이슈 읽기 (`imbas:imbas-read-issue` 경유) |
| `[OP: add_comment]` | Jira ([OP:]) | digest 댓글 게시 |
| `[OP: fetch_attachment]` | Jira ([OP:]) | 첨부 미디어 다운로드 |
| `imbas:imbas-read-issue` | 내부 스킬 | 구조화된 이슈 컨텍스트 |
| `atlassian:atlassian-media-analysis` | 외부 스킬 (atlassian) | 첨부 미디어 분석 |
