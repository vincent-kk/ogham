...
`~/.claude/plugins/cogair/config.json` 로드·저장 담당. 파일 누락·Zod 검증 실패 시 `DEFAULT_CONFIG` 로 fallback. legacy 정수 비율 → enabled flag 마이그레이션, gemini⊕antigravity 상호 배제(`normalizeMutualExclusion`), `model_map` deep merge(`mergeModelMap`) 포함.
