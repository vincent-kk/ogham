...
`runtime/counter.json` 으로 provider(gemini / codex) 호출 횟수 추적. 파일의 `parent_pid` 가 현재 세션 PID 와 다르면 카운트를 0/0 으로 간주하고 다음 write 시 갱신 — 세션마다 자동 격리.
...
- 디스크 JSON 키는 snake_case (`gemini`, `codex`, `parent_pid`)
...
- `parent_pid` 불일치 시 `{ gemini: 0, codex: 0 }` 반환 후 다음 write 에서 pid 갱신
