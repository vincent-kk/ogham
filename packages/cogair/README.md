...
Where `atlassian` or `filid` encapsulate domain knowledge, cogair is a **delegation surface**: Claude decides when another model family fits better (heavy code → codex; live web search → gemini or antigravity) and the plugin handles session bookkeeping, ratio tracking, and per-session call counters.
...
- `gemini` (Google Gemini CLI) — run `gemini auth login`. **The Gemini CLI service ends 2026-06-18**; cogair is migrating to the Antigravity CLI below.
...
Env overrides (codex/gemini): `COGAIR_CODEX_{HIGH,MID,LOW}`, `COGAIR_GEMINI_{HIGH,MID,LOW}`. Antigravity tiers are mapped in `/setup`, not via env vars.
