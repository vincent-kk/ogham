...
init 스킬이 프로젝트 초기화 시 복사하는 템플릿 파일을 포함한다. 훅 설정, 규칙 가이드, 산출물 매니페스트 등 정적 자산을 관리한다.

## Structure

| 경로 | 역할 |
|------|------|
| `deliverables.json` | init 산출물 매니페스트 (생성 파일 목록) |
...
- `deliverables.json`은 init이 참조하는 산출물 목록
...
- 템플릿 내용 변경 시 `filid:init` 스킬 문서 동기화

### Ask first

- 기존 템플릿 파일 경로 변경 (init 로직 수정 필요)
