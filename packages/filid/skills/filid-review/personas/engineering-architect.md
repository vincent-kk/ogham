...
1. **LCOM4 >= 2**: Recommend module split (fix type: `filid-restructure`). Specify target sub-modules.
2. **CC > 15**: Recommend function decomposition or strategy pattern extraction (fix type: `code-fix`).
3. **3+12 violation**: Recommend test file promotion/splitting (fix type: `filid-promote`).
