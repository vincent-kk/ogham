...
# filid-migrate — CLAUDE.md/SPEC.md to INTENT.md/DETAIL.md Migration
...
```bash
bash "${CLAUDE_PLUGIN_ROOT}/skills/filid-migrate/migrate.sh" <target-path>
```

> **Script resolution**: Use `${CLAUDE_PLUGIN_ROOT}` to resolve the absolute path.
> If `CLAUDE_PLUGIN_ROOT` is not set, use `Glob(**/skills/filid-migrate/migrate.sh)`
...
```
/filid:filid-migrate [path] [--execute] [--auto-commit]
...
/filid:filid-migrate src/payments

# Execute migration
/filid:filid-migrate --execute

# Execute and auto-commit
/filid:filid-migrate --execute --auto-commit
```
