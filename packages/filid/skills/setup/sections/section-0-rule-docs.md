# setup — Rule Docs Management (Phase 0a~0d)

> Detail reference for Phase 0a~0d of /filid:setup.
...
- Line 2 (action): `"→ Accept template update: re-run /filid:setup --rules and re-check [UPDATE] items. Keep local edits: ignore."`
...
`/filid:setup` is safe to run repeatedly. On a second or later
