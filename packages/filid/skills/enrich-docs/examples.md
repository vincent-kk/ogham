...
- Best fit: first-time audit after initial `setup`.

### Module-scoped dry run

```bash
/filid:enrich-docs packages/filid/src/core --dry-run
```
...
```bash
/filid:enrich-docs packages/filid/src --min-quality 60 --auto-approve
```
...
```bash
/filid:enrich-docs packages/maencof/src --depth 1 --dry-run
```
