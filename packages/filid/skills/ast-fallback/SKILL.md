...
```
/filid:ast-fallback <pattern> [--language <lang>] [--path <dir>] [--replace <replacement>]
```
...
/filid:ast-fallback "console.log($MSG)"

# Search in specific directory for Python
/filid:ast-fallback "def $NAME($$$ARGS)" --language python --path src/

# Replace var with const
/filid:ast-fallback "var $NAME = $VALUE" --replace "const $NAME = $VALUE"
