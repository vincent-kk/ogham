...
`@ast-grep/napi`(tree-sitter) 기반 정적 분석 모듈을 모은 fractal. 파싱 진입점과 언어 매핑 organ, 그리고 순환 복잡도·응집도·의존성·시맨틱 diff의 4개 분석 엔진을 제공한다. `core/`·`mcp/tools/`·`hooks/changeTracker`가 이 결과를 규칙 평가·리포트 생성·변경 추적에 사용한다.
...
- `@ast-grep/napi`는 `astGrepShared.getSgModule`의 lazy 로더만 경유
- 언어/확장자·결정점 상수는 `constants/astLanguages.ts`, `constants/decisionPoints.ts`에서만 관리
...
- `@ast-grep/napi` (optional peer), `../types/`, `../constants/{astLanguages,decisionPoints}.js`
