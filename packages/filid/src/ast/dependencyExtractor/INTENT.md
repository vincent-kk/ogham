# dependencyExtractor -- import·export·call 정적 추출

## Purpose

TypeScript/JavaScript 소스에서 import 구문, export 선언, 호출식(`CallExpression`)을 AST 기반으로 추출해 `DependencyInfo`(imports/exports/calls)를 반환한다. `fractalValidator`의 순환 의존성 검사와 `mcp/astAnalyze` 도구가 이 결과를 소비한다.

## Structure

- `dependencyExtractor.ts` — `extractDependencies` (public), `getCallee` / `stripQuotes` (internal)
