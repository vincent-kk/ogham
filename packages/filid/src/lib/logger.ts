/**
 * Lightweight debug logger for filid.
 *
 * All output is gated by FILID_DEBUG=1. When enabled, writes to:
 *   1. stderr (console.error) — visible in direct execution
 *   2. debug.log file (appendFileSync) — visible via `tail -f` in hook context
 *
 * Modeled after oh-my-claudecode's component-tagged, never-crash logging pattern.
 *
 * Usage:
