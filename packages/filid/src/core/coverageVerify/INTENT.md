# coverageVerify -- 서브트리 테스트 커버리지 검증 파이프라인
...
| `testCoverageChecker` | 3-Strategy(co-located / mirror / integration)로 테스트 파일 탐지 → `UsageCoverage[]` + 경고 |
...
### Never do
- 테스트 파일 직접 실행
- `importResolver` 외부 노출 (서브모듈 내부에서만 사용)
- node_modules 패키지·tsconfig paths·barrel re-export 해석 시도 (v1 scope 이탈)

## Dependencies
- `../tree/fractalTree/` — `scanProject`, `getDescendants`, `getFractalsUnderOrgans`
- `../../ast/dependencyExtractor/` — `extractDependencies`
- `../../metrics/testCounter/` — `countTestCases`
- `../../constants/scanDefaults.js` — `SKIP_PATTERNS`
