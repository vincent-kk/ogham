...
- `resolve` guard: before `unlink`, the handler MUST call `assertUnder(debtDir, filePath)` (`src/mcp/tools/utils/fs-guard.ts`). Traversal throws and propagates — no silent `{ deleted: false }`.
