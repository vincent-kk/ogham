...
- 각 파일은 단일 `handle*` 함수 export; 입력 타입은 `server.ts` zod 스키마에서 파생
- 파일시스템 I/O는 `review-manage` · `debt-manage`에만 허용; Git subprocess는 `review-manage`의 `content-hash` · `check-cache` 액션에만 허용
...
- 반환 타입을 `toolResult()`가 직렬화 가능한 형태로 유지
...
**Dependencies**: `../../core/`, `../../ast/`, `../../metrics/`, `../../compress/`
