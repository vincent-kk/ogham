# sessionCleanup -- SessionEnd 캐시 정리

## Purpose

SessionEnd 이벤트에서 해당 세션이 남긴 `session-context-{hash}` 마커와 `cached-context-{hash}` 캐시 파일을 삭제해 세션 간 상태 누출을 방지한다. 엔트리 포인트는 매우 얇으며 실제 삭제는 `core/infra/cacheManager.removeSessionFiles`에 위임한다.

## Structure

- `sessionCleanup.ts` — `cleanupSession` (thin wrapper)
- `sessionCleanup.entry.ts` — esbuild 번들 진입점
...
- `../../core/infra/cacheManager/` (`removeSessionFiles`)
