...
Write/Edit 도구가 `INTENT.md` 또는 `DETAIL.md`를 수정하려 할 때 50줄 제한(INTENT.md)과 append-only 방지(DETAIL.md)를 검사해 위반 시 해당 도구 호출만 `deny` (턴 비중단). `pre-tool-use` 오케스트레이터가 호출한다.
...
- `../../types/hooks.js`
- `../../constants/hook-defaults.js` (`DENY_RETRY_GUIDANCE`)
- `node:fs`, `node:path`
