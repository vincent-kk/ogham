...
- `threePlusTwelve.ts` — `check312Rule`

## Conventions

- 검사 대상: `fileType === 'spec'`인 `TestCaseCount` 항목만 (`test.ts`는 규칙 면제)
- 임계값: `THREE_PLUS_TWELVE_THRESHOLD`(=15) — `constants/qualityThresholds.ts` 단일 소스
...
- `../../constants/qualityThresholds.js` (`THREE_PLUS_TWELVE_THRESHOLD`)
