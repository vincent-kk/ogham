...
- `getApiVersion`은 `service` + `is_cloud` (+ 선택 `override`)로 Jira는 `"3"|"2"`, Confluence는 `"v2"|"v1"`을 반환한다 (Confluence는 `override` 무시).
- `EnvironmentInfo` 타입을 index.ts에서 함께 재내보내어 소비자가 직접 import하도록 한다.

### Ask first

- `CLOUD_HOSTNAME_PATTERN` 패턴 변경 (새 클라우드 도메인 추가 등).
- API 버전 매핑(Jira: `is_cloud→"3"|!is_cloud→"2"`, Confluence: `is_cloud→"v2"|!is_cloud→"v1"`) 변경.
