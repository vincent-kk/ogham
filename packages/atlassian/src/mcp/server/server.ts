...
async function buildClientConfig(service: 'jira' | 'confluence'): Promise<HttpClientConfig | null> {
  const config = await loadConfig();
  const svc = config[service];
  if (!svc) return null;

  const authHeader = await getAuthHeader(service, svc.auth_type, svc.username);

  return {
    base_url: svc.base_url,
    auth_header: authHeader ?? undefined,
    ssl_verify: svc.ssl_verify,
    timeout: svc.timeout,
  };
}
...
export function createServer(): McpServer {
  ...
  server.registerTool(
    ...
    wrapHandler(async (args: { endpoint: string; query_params?: Record<string, string>; expand?: string[]; headers?: Record<string, string>; accept_format?: 'json' | 'raw' }) => {
      const service = detectService(args.endpoint);
      const config = await buildClientConfig(service);
      if (!config) throw new Error(`No ${service} configuration found. Run setup first.`);
  ...
  server.registerTool(
    ...
    wrapHandler(async (args: { endpoint: string; body?: unknown; headers?: Record<string, string>; content_type?: string; content_format?: 'json' | 'markdown' }) => {
      const service = detectService(args.endpoint);
      const config = await buildClientConfig(service);
      if (!config) throw new Error(`No ${service} configuration found. Run setup first.`);
  ...
  server.registerTool(
    ...
    wrapHandler(async (args: { endpoint: string; body?: unknown; method?: 'PUT' | 'PATCH'; headers?: Record<string, string>; content_format?: 'json' | 'markdown' }) => {
      const service = detectService(args.endpoint);
      const config = await buildClientConfig(service);
      if (!config) throw new Error(`No ${service} configuration found. Run setup first.`);
  ...
  server.registerTool(
    ...
    wrapHandler(async (args: { endpoint: string; query_params?: Record<string, string>; headers?: Record<string, string> }) => {
      const service = detectService(args.endpoint);
      const config = await buildClientConfig(service);
      if (!config) throw new Error(`No ${service} configuration found. Run setup first.`);
