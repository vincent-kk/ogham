...
vi.mock("../../../../core/authManager/index.js", () => ({
...
import { handleAuthCheck } from "../authCheck.js";
import { loadConfig } from "../../../../core/configManager/index.js";
import { loadCredentials } from "../../../../core/authManager/index.js";
...
describe("handleAuthCheck", () => {
  ...
  it("Jira + Confluence 설정, connection_test:false — 두 서비스 모두 반환", async () => {
    ...
    expect(result.authenticated).toBe(true);
    expect(result.services.jira![0]).toMatchObject({
      configured: true,
      base_url: "https://jira.atlassian.net",
    });
    expect(result.services.confluence![0]).toMatchObject({
      configured: true,
      base_url: "https://confluence.atlassian.net",
    });
  });
  ...
  it("Confluence 연결 테스트 성공 — user는 null", async () => {
    ...
    mockLoadCredentials.mockResolvedValue({
      confluence: { basic: { api_token: "secret" } },
    });
    mockTestConnection.mockResolvedValue({
      service: "confluence",
      success: true,
  ...
  it("자격 증명 안전성 — 응답에 api_token/password 값 노출 없음", async () => {
    ...
    mockLoadCredentials.mockResolvedValue({
      jira: {
        basic: { api_token: "super-secret-token", password: "my-password" },
      },
    });
    mockTestConnection.mockResolvedValue({
      service: "jira",
  ...
  it("다중 사이트 — 여러 사이트 각각에 대한 엔트리 반환", async () => {
    ...
    expect(result.services.jira![0]?.base_url).toBe(
      "https://site1.atlassian.net",
    );
    expect(result.services.jira![1]?.base_url).toBe(
