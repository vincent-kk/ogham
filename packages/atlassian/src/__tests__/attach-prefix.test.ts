...
describe("attachPrefix", () => {
  ...
  describe("edge", () => {
    it("attaches /rest/api prefix for Confluence DC (v1)", () => {
      expect(attachPrefix("/content/12345", "confluence", "v1")).toBe(
    ...
    it.each([
      [
        "/pages/1?body-format=storage",
        "confluence",
        "v2",
        "/wiki/api/v2/pages/1?body-format=storage",
      ],
      [
        "/search?jql=project=DEV",
        "jira",
        "2",
        "/rest/api/2/search?jql=project=DEV",
      ],
    ] as const)(
      "preserves query string after attaching prefix: %s",
      (path, service, version, expected) => {
        expect(attachPrefix(path, service, version)).toBe(expected);
      },
    );

    it("preserves trailing slash in endpoint", () => {
      expect(attachPrefix("/pages/1/", "confluence", "v2")).toBe(
        "/wiki/api/v2/pages/1/",
      );
    });
    ...
    it("attaches /wiki/api/v2 prefix to confluence Cloud children endpoint", () => {
      expect(attachPrefix("/pages/1/children", "confluence", "v2")).toBe(
        "/wiki/api/v2/pages/1/children",
      );
    });

    it("attaches /rest/api prefix to confluence DC child endpoint", () => {
      expect(attachPrefix("/content/1/child/page", "confluence", "v1")).toBe(
        "/rest/api/content/1/child/page",
      );
    });
