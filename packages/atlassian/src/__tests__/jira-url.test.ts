...
describe("parseJiraUrl", () => {
  ...
  it("extracts focusedCommentId from query param", () => {
    const result = parseJiraUrl(
      "https://foo.atlassian.net/browse/KAN-27?focusedCommentId=10110",
    );
    expect(result).toEqual({
      issueKey: "KAN-27",
      projectKey: "KAN",
      focusedCommentId: "10110",
    });
  });

  it("handles focusedId alias", () => {
    const result = parseJiraUrl(
      "https://foo.atlassian.net/browse/KAN-27?focusedId=10110",
    );
    expect(result).toEqual({
      issueKey: "KAN-27",
      projectKey: "KAN",
  ...
  it("prefers focusedCommentId over focusedId when both present", () => {
    const result = parseJiraUrl(
      "https://foo.atlassian.net/browse/KAN-1?focusedCommentId=111&focusedId=222",
    );
