...
3. **Skill 계층**: 컨텍스트 창을 작게 유지하기 위해 **지연 참조 로딩(Lazy Reference Loading)**을 사용하는 API 스펙 캡슐(`jira`, `confluence`)입니다. 도구 스키마는 필요할 때만 로드됩니다.
...
| 스킬 | 범위 | 설명 |
| --- | --- | --- |
| `/setup` | 공통 | 인증 및 연결 관리 (Basic, PAT, OAuth 2.0) |
| `/download` | 공통 | Jira/Confluence 통합 첨부파일 다운로드 |
| `/jira` | Jira | Jira API 도메인 라우터 (이슈, 검색, 애자일 등 15개 도메인) |
| `/confluence` | Confluence | Confluence API 도메인 라우터 (페이지, 검색, 스페이스 등 8개 도메인) |
