...
PR-H 부터 시작. 각 패키지 적용 일관 절차 (HANDOFF "🔁 적용 절차" 섹션) 준수. PR 단위로 끝나면 멈추고 결과 보고.
```

> 다른 PR 부터 시작하려면 "V1 부터 진행" 같이 명시. PR-H 는 atlassian docs-only.
...
3. 사용자가 어떤 PR (PR-H) 또는 검증 (V1/V2/V3) 부터 진행할지 명시했는지 확인. 미명시 시 권장 = PR-H.
