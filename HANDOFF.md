...
ogham 모노레포 6개 플러그인의 Windows-Unix 호환성을 한 곳에 모으기 위해 **`shared/cross-platform` 모노레포 내부 워크스페이스** 신설 + 9 도구별 sub-entry (`env`/`eol`/`paths`/`spawn`/`binaries`/`shim`/`error-log`/`self-probe`/`bootstrap`) 구현 + tree-shaking 으로 hook bundle 최소화 + 단위 테스트 57 + `spawnCliSync` (sync caller cascade 회피용). **PR-C / PR-D / PR-E / PR-F / PR-G / PR-H 모두 완료** — 6개 패키지 마이그레이션 종료. 다음 단계: **V1** (CI 매트릭스) → **V3** (사용자 직접 Windows dogfood) → **PR-I** lint 가드. **V2 (hook bootstrap 실 연결) 는 폐기** — `.cmd` shim 자체가 `bridge/node.exe` 부재 가정이라 PATH-결손 케이스를 해결하지 못함. selfProbe + error-log 가시화로 책임 분리 완료. 자세한 사유는 § "🔍 프로덕션 ship 전 추가 검증" V2 섹션.
...
V1 (CI 매트릭스 push & 검증) 부터 진행. HANDOFF § "🔍 프로덕션 ship 전 추가 검증" V1/V2/V3 절차 준수. 단계 종료 시 결과 보고.
```

> 모든 마이그레이션 PR 완료. 다음은 V1/V2/V3 검증 → PR-I lint 가드.
...
| 8    | **PR-G**   | imbas (`setup.ts` HOME/basename + hook bootstrap; npm fallback deferred)              | 0.5d | PR-B   | ✅ 완료    | `cb7b539` |
| 9    | **PR-H**   | atlassian — docs only (chmod Windows 무시 + NTFS ACL 명기)                            | 0.5d | none   | ✅ 완료    | `aafda8c` |
| 10   | **V1**     | 추가 검증 — CI 매트릭스 push & 결과 확인                                              | 0.5d | PR-F/G | ⏳ 진행 중 | —         |
| 11   | ~~V2~~     | ~~hook bootstrap 실 연결~~ — ❌ 폐기 (`.cmd` shim 자체가 무효, 자세한 사유는 V2 섹션) | —    | —      | ❌ 폐기    | —         |
| 12   | **V3**     | 추가 검증 — Windows native dogfood (사용자 직접 진행)                                 | 0.5d | V1     | ⏳ 대기    | —         |
...
| ✅ PR-G | imbas        | `setup.ts:21` `process.env.HOME` → `env.home()`. `setup.ts:23` `cwd.split('/').pop()` → `path.basename(cwd)`. 5 hook entry 에 `logHookFailure`, setup entry 에 `selfProbe`. build-hooks 에 banner + `generateWindowsCmd` + cap (LIGHT 10K / HEAVY 17K / SESSION_START 40K). **`build-mcp-server.mjs:44` `execSync('npm root -g')` fallback 은 deferred** — CJS banner 안에서 동기 실행 + Windows shell PATHEXT 해석으로 작동, ESM cross-platform 헬퍼를 CJS banner 에 inline 불가. 결과: 269 tests pass.                                                                                                                   |
| ✅ PR-H | atlassian    | docs only — chmod 0o600 이 Windows NTFS 에서 no-op 임을 `auth-manager` / `config-manager` INTENT.md (Always do) + README/README-ko_kr Security 섹션에 명기. 보호는 `~/.claude/plugins/atlassian/` 부모 디렉토리 NTFS ACL 상속 (기본값 현재 사용자 전용). 회피 디렉토리 예시 (`C:\ProgramData\`, `C:\Users\Public\`) 포함.                                                                                                                                                                                                                                                                                                  |

---

## 🔍 프로덕션 ship 전 추가 검증 (V1 / V3)

PR-D/E 후 자체 분석에서 미보장으로 노출된 영역. PR-I lint 가드 전에 반드시 통과. V2 는 PR-F/G/H 작업 중 폐기 결정 (아래 V2 섹션 참조).
...
### V2 — hook bootstrap 실 연결 — ❌ **폐기 (2026-05-26)**

- **폐기 사유**: `bridge/run-hook.cmd` 의 본문 (`"%~dp0node.exe" "%~dp0../libs/run.cjs" %*`) 은 cmd 파일과 같은 디렉토리에 `node.exe` 가 존재한다고 가정함. 우리는 `bridge/` 에 node.exe 를 배포하지 않음 → shim 자체가 PATH-결손 케이스를 해결하지 못함.
- 실 동작 매트릭스:
  | 상황 | hooks.json 의 `node "..."` | `.cmd` shim |
  | ----------------------------------- | -------------------------- | --------------- |
  | Windows + node PATH ✓ (정상) | ✅ 작동 | (호출되지 않음) |
  | Windows + node PATH ✗ | ❌ | ❌ (bridge/node.exe 부재) |
  | nvm-windows / fnm + 비대화 셸 | 케바케 | ❌ (동일) |
- 옵션 A/B/C 도 동일한 한계 (PATHEXT 해석은 하지만 shim 본문이 여전히 node.exe 를 못 찾음). 진짜 해결은 OMC 패턴 (설치 시점 `process.execPath` 절대경로 patch) 이 필요한데 Claude Code 의 plugin install hook 지원에 종속 + 별도 설계 필요.
- 대안 (이미 반영됨):
  1. `selfProbe` 가 node/git/PATH/CLAUDE_PLUGIN_ROOT 부재를 detect → `~/.claude/plugins/<pkg>/error-log.json` + SessionStart `additionalContext` 경고로 가시화. 사용자가 직접 PATH 손볼 수 있도록 책임 분리.
  2. `.cmd` shim 생성 자체는 build-hooks 가 계속 emit (해롭지 않음, 50B). 향후 Claude Code 가 install hook 지원하면 OMC 패턴으로 재활성화 가능.
- **후속 조치**: 6개 패키지의 README 에 "Windows 실행 시 node ≥ 20 이 PATH 에 있어야 함" 명기는 V3 사용자 dogfood 후 필요 시 추가.
...
| PR-G imbas 전환                                   | ✅ 완료 (`cb7b539`) |
| PR-H atlassian docs                               | ✅ 완료 (`aafda8c`) |
| V1 CI 매트릭스 push & 검증                        | ⏳ 진행 중          |
| ~~V2 hook bootstrap 실 연결~~                     | ❌ 폐기 (shim 무효) |
| V3 Windows native dogfood                         | ⏳ 사용자 진행      |
...
3. 마이그레이션 6개 패키지 전부 완료. 사용자가 어떤 검증 (V1/V2/V3) 부터 진행할지 명시했는지 확인. 미명시 시 권장 = V1 (CI 매트릭스 push).
