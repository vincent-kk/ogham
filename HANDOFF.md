...
| 7    | **PR-F**   | filid (ast-grep 화이트리스트 OS-aware + git spawnCli{,Sync} + hook bootstrap) | 1d   | PR-B   | ✅ 완료   | (new)     |
| 7.5  | **PR-F.1** | cross-platform `spawnCliSync` 추가 (sync 호출자 cascade 회피)   | —    | PR-F   | ✅ 완료   | (new)     |
...
| PR-F filid 전환                                   | ✅ 완료 (pending)   |
| PR-F.1 cross-platform spawnCliSync                | ✅ 완료 (pending)   |
| PR-G imbas 전환                                   | ⏳ 다음             |
