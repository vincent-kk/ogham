...
- 단편적 수정 대신 **`packages/cross-platform` 공유 어댑터 + `cross-spawn` 의존성 + Windows `.cmd` hook bootstrap** 3축으로 표준화하면 같은 문제가 다른 패키지에서 재발하지 않음.
...
### 4.2 신규 공유 모듈: `packages/cross-platform`

> 모듈 위치는 **신규 워크스페이스 `packages/cross-platform`** 으로 확정됨 (Part 8 결정 #1). drift 방지와 단일 책임 원칙을 우선.
...
### Phase 1 — `packages/cross-platform` 워크스페이스 신설 (3d, PR-A + PR-B)

**PR-A (1d) — 워크스페이스 + 빌드 파이프라인 골격**

| 변경 파일                                         | 변경 내용                                                                                                                                                                                                       |
| ------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `packages/cross-platform/package.json`            | `private: true`, `name: "@ogham/cross-platform"`, `type: "module"`, `main: "./dist/index.js"`, `types: "./dist/index.d.ts"`, `devDependencies` 에 `cross-spawn ^7`, `which ^4`, `env-paths ^3` (또는 자체 구현) |
| `packages/cross-platform/tsconfig.build.json`     | ESM declaration true, outDir `./dist`                                                                                                                                                                           |
| `packages/cross-platform/INTENT.md` + `DETAIL.md` | FCA-AI 규약대로 — 50 line 이내, 3-tier 경계                                                                                                                                                                     |
| `packages/cross-platform/src/index.ts`            | barrel only (서브 모듈 도착 시점에 채워짐)                                                                                                                                                                      |
...
- `import { spawn|exec|execSync|execFile|fork } from 'node:child_process'` — allow-list: `packages/cross-platform/src/spawn/**` 내부만
- `process.env.HOME`, `process.env.USERPROFILE`, `process.env.TMPDIR`, `process.env.TEMP` 직접 사용 — allow-list: `packages/cross-platform/src/env/**`, `packages/cross-platform/src/paths/**` 내부만
...
1. **공유 모듈 위치** — **결정됨: (a) 신규 워크스페이스 `packages/cross-platform`**
