...
export function geminiEnv(
  ...
  const env: Record<string, string> = { COGAIR_FAKE_GEMINI_MODE: mode };
  if (opts.uuid) env.COGAIR_FAKE_GEMINI_UUID = opts.uuid;
  if (opts.index) env.COGAIR_FAKE_GEMINI_INDEX = opts.index;
  if (opts.listMode) env.COGAIR_FAKE_GEMINI_LIST_MODE = opts.listMode;
  return env;
}
...
export function codexEnv(
  ...
  const env: Record<string, string> = { COGAIR_FAKE_CODEX_MODE: mode };
  if (opts.threadId) env.COGAIR_FAKE_CODEX_THREAD = opts.threadId;
  return env;
}
