...
const mode = process.env.COGAIR_FAKE_GEMINI_MODE || 'success';
const uuid = process.env.COGAIR_FAKE_GEMINI_UUID || '11111111-2222-3333-4444-555566667777';
const index = process.env.COGAIR_FAKE_GEMINI_INDEX || '1';

if (args.includes('--list-sessions')) {
  const listMode = process.env.COGAIR_FAKE_GEMINI_LIST_MODE || 'present';
...
beforeEach(async () => {
  ...
  delete process.env.COGAIR_FAKE_GEMINI_MODE;
  delete process.env.COGAIR_FAKE_GEMINI_UUID;
  delete process.env.COGAIR_FAKE_GEMINI_INDEX;
  delete process.env.COGAIR_FAKE_GEMINI_LIST_MODE;
});
...
describe('geminiDispatcher.start', () => {
  it('returns success, captures stdout as response, and reads UUID from --list-sessions', async () => {
    ...
    process.env.COGAIR_FAKE_GEMINI_UUID =
  ...
  it('maps HTTP 401 stderr to an auth failure', async () => {
    process.env.COGAIR_FAKE_GEMINI_MODE = 'auth-stderr';
    const result = await geminiDispatcher.start(baseOptions());
  ...
  it('maps HTTP 429 stderr to a rate_limit failure', async () => {
    process.env.COGAIR_FAKE_GEMINI_MODE = 'rate-limit-stderr';
    const result = await geminiDispatcher.start(baseOptions());
    expect(result.error?.code).toBe('rate_limit');
  });

  it('maps ECONNRESET stderr to a network failure', async () => {
    process.env.COGAIR_FAKE_GEMINI_MODE = 'network-stderr';
    const result = await geminiDispatcher.start(baseOptions());
    expect(result.error?.code).toBe('network');
  });

  it('maps exit 55 to auth (untrusted workspace)', async () => {
    process.env.COGAIR_FAKE_GEMINI_MODE = 'exit-55';
    const result = await geminiDispatcher.start(baseOptions());
    expect(result.error?.code).toBe('auth');
  });
  ...
  it('fails when --list-sessions returns no entries (cannot capture UUID)', async () => {
    process.env.COGAIR_FAKE_GEMINI_MODE = 'success';
    process.env.COGAIR_FAKE_GEMINI_LIST_MODE = 'empty';
    const result = await geminiDispatcher.start(baseOptions());
    expect(result.status).toBe('failure');
...
describe('geminiDispatcher.resume', () => {
  it('resolves the stored UUID to the current index and preserves externalSessionRef', async () => {
    ...
    process.env.COGAIR_FAKE_GEMINI_UUID =
      'aaaaaaaa-bbbb-cccc-dddd-eeeeeeffffff';
    process.env.COGAIR_FAKE_GEMINI_INDEX = '7';
  ...
  it('passes through budget_exhausted (exit 53) from the prompt call', async () => {
    process.env.COGAIR_FAKE_GEMINI_MODE = 'exit-53';
    process.env.COGAIR_FAKE_GEMINI_UUID =
      'aaaaaaaa-bbbb-cccc-dddd-eeeeeeffffff';
    const result = await geminiDispatcher.resume({
      ...baseOptions(),
