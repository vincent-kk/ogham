...
- 독립 "MCP Addons" 섹션 (provider 라우팅과 분리) — `addons.youtube` = enabled + language(en/ko) + targets(codex·antigravity). `/save` 가 체크된 대상 CLI 의 MCP 설정(agy mcp_config.json / codex config.toml)에 yt-dlp-mcp 서버를 등록/해제 (cogair 네임스페이스 밖 부수효과). gemini 는 내장 YouTube 로 제외
