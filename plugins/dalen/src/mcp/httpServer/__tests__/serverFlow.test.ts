...
const REPORT_HTML = `<!doctype html><html data-theme="auto"><head><title>t</title></head><body><script>window.__DALEN_STATE__="__DALEN_STATE__";</script><div id="report"></div></body></html>`;
...
beforeAll(async () => {
  const pluginRoot = mkdtempSync(join(tmpdir(), "dalen-bridge-"));
...
describe("report server flow", () => {
  it("serves a rendered report with injected, escaped state", async () => {
    ...
    expect(html).not.toContain('"__DALEN_STATE__"');
