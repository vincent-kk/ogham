...
export async function renderAll(): Promise<void> {
  const nodes = Array.from(
    document.querySelectorAll<HTMLElement>("#report .dalen-mermaid"),
  );
  for (let i = 0; i < nodes.length; i += 1) {
    const node = nodes[i];
    const src = node.querySelector(".dalen-mermaid-src")?.textContent ?? "";
    try {
      const { svg } = await mermaid.render(`dalen-mermaid-${i}`, src);
      node.innerHTML = svg;
    } catch {
