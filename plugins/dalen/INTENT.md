...
`@ogham/dalen` 패키지 루트. Claude 가 생성한 markdown 보고서를 로컬 HTTP 서버가 브라우저 페이지로 렌더링하고, 라인 단위 피드백(파일·클립보드 이미지 포함)을 수집해 Claude 에 되돌려주는 Claude Code 플러그인. 설계 명세는 [`.metadata/dalen/`](../../.metadata/dalen/).
...
- 디스크 경로는 `~/.claude/plugins/dalen/` 하위
- 개발 착수 전 [HANDOFF.md](./HANDOFF.md) + `.metadata/dalen/` 확인
