...
- `rare_event`는 희귀 사건이며 전체 발생률이 낮습니다.
- `factor_b`, `region`, `ordinal_band` 분포가 균형적이지 않아 교란과 층화 필요성을 테스트할 수 있습니다.
- `ordinal_band`가 높을수록 사건 가능성이 커지고, `factor_b`의 한 수준은 평균적으로 사건 가능성을 낮추도록 설계했습니다.
