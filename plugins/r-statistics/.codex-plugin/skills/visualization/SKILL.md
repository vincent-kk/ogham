...
name: visualization
user-invocable: true
description: 'Produce publication-ready statistical plots with ggplot2 — distributions, boxplots, scatter/regression, survival curves, forest plots — saved as image artifacts. Use when asked to "plot this", "make a boxplot", "박스플롯 그려줘".'
...
Generate a statistical figure with `ggplot2` and save it as an image artifact. Execution runs through `mcp__r_statistics__run_r`.
...
2. **Build the plot** in R from `read_data("<id>")`, with clear axis labels, a readable theme (`theme_minimal()` or similar), and the grouping/colour the question implies.
3. **Save** with `save_plot_artifact("plot.<kind>", p, "<kind>.png", "<desc>")` (PNG by default; SVG/PDF on request). The helper registers it in the manifest.
