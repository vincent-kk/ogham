...
```bash
yarn r-statistics typecheck     # tsc --noEmit
yarn r-statistics test:run      # vitest run (single)
yarn r-statistics test          # vitest watch
yarn r-statistics build         # clean → version:sync → tsc → buildMcpServer
yarn r-statistics version:sync  # package.json → src/version.ts + plugin.json
```

(Run from the repo root; the `r-statistics` alias maps to `yarn workspace @ogham/r-statistics`.)
...
- MCP server name is `tools` — skills/agents reference it as `mcp_tools_<name>`.
