...
- **Skills** — 6 exposed (analyze, data-preparation, assumption-check, visualization, reporting, setup) + lazy `skills/analyze/references/methods/{technique}/`.
...
- **R packages**: `setup` always installs `REQUIRED_PACKAGES` and offers the rest by use case via `PACKAGE_USE_CASES` (`src/constants/defaults.ts`); `PACKAGE_WHITELIST` is the run_r baseline. Keep the required/use-case lists in `skills/setup/references/packages.md` and the baseline list in `agents/r-expert.md` in sync with those constants.
