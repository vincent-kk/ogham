 ...
 *
 * Hook isolation guards: hooks must remain thin scripts (Node builtins +
 * @ogham/cross-platform light helpers only). Heavy externals belong in the
 * MCP server bundle.
 */
...
// Routes through libs/run.cjs (process.execPath via spawnSync).
generateWindowsCmd({
...
if (violations.length > 0) {
  ...
  console.error(
    '\nHooks must stay thin (Node builtins + @ogham/cross-platform light helpers).\n' +
      ...
      'External modules and cross-platform heavy helpers belong in MCP / Skill paths.',
  );
  process.exit(1);
}
