...
Provider-agnostic errors are in `../errors.md`. This file lists GitHub-specific error cases triggered inside the read-issue `workflow.md` (this directory).
...
When multiple `<!-- imbas:digest -->`-marked comments exist on the same issue, this is NOT an error. Apply the **last-wins policy**:
...
When `depth == "shallow"` is requested and the issue is found, no errors are generated for missing comments or an absent digest. Shallow mode explicitly skips comment processing — this is expected behavior, not an error.
