...
2. Call mcp__plugin_imbas_tools__cache_get(project_ref, cache_type: "all")
...
2. Fetch issue types:
   ...
   c. Call mcp__plugin_imbas_tools__cache_set(project_ref, "issue-types", <aggregated data>)

3. Fetch link types:
   a. [OP: get_link_types]
   b. Call mcp__plugin_imbas_tools__cache_set(project_ref, "link-types", <data>)

4. Fetch project metadata:
   a. [OP: get_projects]
   b. Find matching project by key
   c. Call mcp__plugin_imbas_tools__cache_set(project_ref, "project-meta", <data>)
...
5. Fetch workflow data (optional — requires existing issue):
   ...
   c. If result has an issue:
      ...
      → Call mcp__plugin_imbas_tools__cache_set(project_ref, "workflows", <data>)

6. cached_at.json is automatically updated by mcp__plugin_imbas_tools__cache_set:
