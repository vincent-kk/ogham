...
Loaded when `config.provider === 'jira'`. Provider-agnostic imbas MCP tools (`mcp__imbas__run_get`, `mcp__imbas__manifest_save`, `mcp__imbas__manifest_validate`) are documented in `../tools.md` and are used by all providers.
