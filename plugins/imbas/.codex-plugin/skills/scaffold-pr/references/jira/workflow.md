...
1. Read `config.jira.base_url` via `mcp__imbas__config_get` to build issue URLs. If `base_url` is null or unset, derive the host from the `self` field of the `[OP: get_issue]` response (scheme + host, e.g. `https://site.atlassian.net`) and build `<host>/browse/<key>`. If neither is available, render issue keys without links (do not guess a host).
