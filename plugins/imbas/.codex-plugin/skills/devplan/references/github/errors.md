...
Provider-agnostic errors are in `../errors.md`. This file lists GitHub-specific error cases for the devplan skill's GitHub branch.
...
All GitHub tool calls in devplan Step 2 are **optional**. Failures do NOT abort the devplan skill. The skill degrades gracefully to code-only exploration (mcp__plugin_imbas_tools__ast_search, mcp__plugin_imbas_tools__ast_analyze, Read, Grep, Glob) when GitHub access is unavailable.

## Manifest output errors

The devplan skill emits `feedback_comments` with `target_ref: "owner/repo#N"`. If a `target_ref` cannot be resolved (story has no `issue_ref` in the manifest), the feedback_comment is still recorded with a null `target_ref` and a warning:

```
Warning: Story "<ID>" has no issue_ref. Feedback comment recorded but cannot be
posted until the story is created in GitHub. Run /imbas:manifest to create it first.
```

This is a planning-time warning, not a runtime execution error. The manifest skill handles the actual posting.
