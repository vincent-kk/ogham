...
`imbas:split` 단계에서 생성된 `stories-manifest.json` 또는 `imbas:devplan` 단계에서 생성된 `devplan-manifest.json`을 읽어 Epic, Story, Task, Sub-task, 링크, 댓글을 Jira에 일괄 생성한다.
...
| 도구                                     | 출처         | 용도                          |
| ---------------------------------------- | ------------ | ----------------------------- |
| `mcp__plugin_imbas_tools__manifest_get`  | imbas MCP    | 매니페스트 로드               |
| `mcp__plugin_imbas_tools__manifest_save` | imbas MCP    | 아이템별 저장 (크래시 복구)   |
| `mcp__plugin_imbas_tools__manifest_plan` | imbas MCP    | dry-run 실행 계획 생성        |
