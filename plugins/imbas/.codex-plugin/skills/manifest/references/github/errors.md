...
|----------|------------------|-----------|-------------|
| `LABEL_BOOTSTRAP_FORBIDDEN` | `gh label create` exits non-zero with HTTP 403 or "resource not accessible" | Check exit code + stderr contains "403" or "resource not accessible" | Emit human-readable error (see below), call `mcp__plugin_imbas_tools__run_transition → blocked`, STOP. |
...
1. Call `mcp__plugin_imbas_tools__run_transition` → `blocked` state.
