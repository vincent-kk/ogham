...
Loaded when `config.provider === 'jira'`. Provider-agnostic imbas MCP tools (`mcp__plugin_imbas_tools__run_get`, `mcp__plugin_imbas_tools__manifest_get`, `mcp__plugin_imbas_tools__manifest_save`, `mcp__plugin_imbas_tools__manifest_plan`) are documented in `../tools.md` and are used by all providers.
