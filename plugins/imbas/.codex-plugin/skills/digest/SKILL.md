...
```
/imbas:digest <issue-ref> [--preview] [--no-media] [--update]
...
--update     : (GitHub only) Edit the latest imbas digest comment in place (last-wins) instead of posting a new one. Ignored for jira/local, which always keep old digests and post new ones.
```
