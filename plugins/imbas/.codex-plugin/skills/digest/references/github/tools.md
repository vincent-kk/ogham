...
(`mcp__plugin_imbas_tools__run_get`, `mcp__plugin_imbas_tools__config_get`) are documented in `../tools.md` and are used by all
providers. Issue reading is delegated to the `read-issue` skill (see Step 1 of
`../workflow.md`).
...
The comment id is obtained from the `read-issue` response (`comments[*].databaseId`
