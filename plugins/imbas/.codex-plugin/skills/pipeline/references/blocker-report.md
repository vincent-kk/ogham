...
Emit when pipeline completes manifest-stories but `--codebase` was not provided. Not an error — intentional scope boundary. The document has been refined into Stories; Subtask generation is deferred until a codebase is available.
...
`/imbas:pipeline <source> --codebase /path/to/repo --project <KEY>`
