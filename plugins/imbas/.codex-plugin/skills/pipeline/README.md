...
validate → split → manifest-stories → devplan → manifest-devplan 전체 흐름을 하나의 `/imbas:pipeline` 명령으로 자동 실행한다. 각 단계 사이에 품질 게이트(auto-approval)를 두어, 통과 시 자동 진행하고 실패 시 구조화된 블로커 리포트를 출력한다.
