...
2. `Edit` the file's `## Digest` section, appending the comment body (do NOT replace the section). Format:
   ```
   ### <timestamp>
   <comment body>
   ```
...
IDEMPOTENCY: for every item, check `status` and `issue_ref` before writing. If `issue_ref` already exists, re-verify the file exists (Glob) and skip. Re-execution is safe after partial failure.
