...
Step 0.6 — Route
  ...
  DEVPLAN PIPELINE (codebase null) → STOP: "Devplan pipeline requires --codebase.
    ...
    Usage: /imbas:pipeline PROJ-42 --codebase /path/to/repo"
```
...
Step 1.1 — Run Initialization
  ...
  2. Call mcp__plugin_imbas_tools__run_create with:
  ...
  3. Call mcp__plugin_imbas_tools__run_transition:
     - project_ref, run_id, action: "start_phase", phase: "validate"

Step 1.2 — Document Source Resolution
  - Local file (*.md, *.txt): Already copied to source.md by mcp__plugin_imbas_tools__run_create. Read directly.
  - Confluence URL:
...
Step 1.5 — State Update
  Call mcp__plugin_imbas_tools__run_transition:
...
Step 2.5 — Escape Condition Detection
  ...
  - Call mcp__plugin_imbas_tools__run_transition: action "escape_phase", phase "split", escape_code
  ...
  - Call mcp__plugin_imbas_tools__manifest_save to persist it
  - Call mcp__plugin_imbas_tools__run_transition: action "escape_phase", phase "split", escape_code "E2-3"
...
Step 2.7 — Manifest Generation
  ...
  2. Call mcp__plugin_imbas_tools__manifest_save(project_ref, run_id, type: "stories", manifest)
  3. Call mcp__plugin_imbas_tools__manifest_validate(project_ref, run_id, type: "stories")
...
Step 2.5.2 — Batch Execution
  CRITICAL: After EACH item creation, immediately save manifest via mcp__plugin_imbas_tools__manifest_save.
  ...
  Phase D — Source Issue Transitions (runs after Phase C):
    See manifest/references/<provider>/workflow.md Phase 4d for provider-specific execution.
...
Step 2.5.4 — Post-Execution Label Transitions (stories type)
  ...
  Provider commands: see manifest/references/<provider>/workflow.md Step 6.
    ...
    → Emit planning-only report:
      "Planning pipeline complete. N Stories created in <PROJECT>.
       ...
       Resume with codebase:
         /imbas:pipeline <source> --codebase /path/to/repo
       Or run devplan individually:
         /imbas:devplan --run <run-id> --codebase /path/to/repo"
    → EXIT
...
Step 3.0 — DEVPLAN PIPELINE Mode Setup (only when input is Story keys)
  ...
  1. Call mcp__plugin_imbas_tools__run_create(project_ref, source_file: "devplan-pipeline", supplements: [])
     (sentinel value — not a file path; mcp__plugin_imbas_tools__run_create skips file copy when source_file does not point to an existing file)
  ...
  4. Call mcp__plugin_imbas_tools__manifest_save(type: "stories", manifest)
  5. Call mcp__plugin_imbas_tools__run_transition: action "skip_phases", phases ["validate", "split"]
...
Step 3.1 — Start Phase
  1. Call mcp__plugin_imbas_tools__run_transition: action "start_phase", phase "devplan"
...
Step 3.2 — engineer Agent Spawn
  ...
  AST FALLBACK:
  If mcp__plugin_imbas_tools__ast_search or mcp__plugin_imbas_tools__ast_analyze returns sgLoadError:
...
Step 3.4 — Manifest Validation
  1. Call mcp__plugin_imbas_tools__manifest_save(project_ref, run_id, type: "devplan", manifest)
  2. Call mcp__plugin_imbas_tools__manifest_validate(project_ref, run_id, type: "devplan")

  IF agent returns devplan-blocked-report.md (all Stories blocked):
  - Save blocked report to run directory
  - Call mcp__plugin_imbas_tools__run_transition(complete_phase, devplan, result: "BLOCKED")
...
>>> GATE 3: Devplan Quality (see auto-approval-gates.md)
  - All criteria pass → call mcp__plugin_imbas_tools__run_transition(complete_phase, devplan, pending_review: false)
...
Step 3.5.1 — Dry-Run Check
  If --dry-run flag is set:
  - Call mcp__plugin_imbas_tools__manifest_plan(project_ref, run_id) for execution plan preview
