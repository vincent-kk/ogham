...
| Error                                   | Action                                                                                        |
| --------------------------------------- | --------------------------------------------------------------------------------------------- |
| No config.json found                    | STOP: "imbas not initialized. Run /imbas:setup first."                                        |
| No project key (argument or config)     | STOP: "No project key. Run /imbas:setup or pass --project KEY."                               |
...
| Error                                     | Action                                                                        |
| ----------------------------------------- | ----------------------------------------------------------------------------- |
| mcp__plugin_imbas_tools__run_create fails | STOP: display error. Common: "Run directory already exists."                  |
...
| mcp__plugin_imbas_tools__run_transition precondition fail | STOP: "Cannot transition: \<error from tool\>."                                |
...
| Error                               | Action                                                                                           |
| ----------------------------------- | ------------------------------------------------------------------------------------------------ |
| Stories not in Jira (any pending)   | STOP: "Stories must exist in Jira. Run /imbas:manifest stories --run \<run-id\> first."          |
...
This creates a NEW run (new run_id). Previous run state is preserved and can be inspected with `/imbas:status <old-run-id>`.
...
/imbas:manifest devplan --run <run-id>
```
...
/imbas:split --run <run-id>           # manual review + approve
/imbas:manifest stories --run <run-id>
/imbas:devplan --run <run-id>
/imbas:manifest devplan --run <run-id>

# After Phase 2 (split) stop:
/imbas:split --run <run-id>           # review flagged Stories
/imbas:manifest stories --run <run-id>
/imbas:devplan --run <run-id>
/imbas:manifest devplan --run <run-id>

# After Phase 2.5 (manifest-stories) partial failure:
/imbas:manifest stories --run <run-id>  # retry failed items (idempotent)
/imbas:devplan --run <run-id>
/imbas:manifest devplan --run <run-id>

# After Phase 3 (devplan) stop:
/imbas:devplan --run <run-id>           # review flagged items
/imbas:manifest devplan --run <run-id>

# After Phase 3.5 (manifest-devplan) partial failure:
/imbas:manifest devplan --run <run-id>  # retry failed items (idempotent)
```

### Idempotency

Manifest execution is idempotent: items with existing issue_ref are skipped on re-run. This makes retry-after-failure safe — already-created items are not duplicated.
