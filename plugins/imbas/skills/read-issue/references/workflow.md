...
Each provider returns a result object with the same shape so Step 5 can build the structured output identically.
...
- Jira: all fields populated; `participants`, `decisions`, `open_questions` fully analyzed from the comment thread.
- GitHub: all fields populated from `gh issue view --json` output; comment thread analyzed the same way as Jira.
- Local: `participants: []`; `decisions` / `open_questions` are scanned from description + digest bodies only; `comment_count` = digest entry count.
