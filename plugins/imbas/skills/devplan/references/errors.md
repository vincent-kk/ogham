...
| Error | Action |
|-------|--------|
| No codebase path (argument or config) | STOP: "Devplan requires --codebase. Subtask generation needs a codebase to explore. Usage: /imbas:devplan --run \<run-id\> --codebase /path/to/repo" |
...
| `engineer` fails | Save partial manifest if available. Display: "Engineer agent error. Partial results saved. Re-run with --stories to complete." |
...
| `engineer` returns blocked report | Set devplan output to "devplan-blocked-report.md". Display blocked dependencies and suggested resolutions. If partial manifest exists, note: "Partial devplan available for unblocked Stories." |
