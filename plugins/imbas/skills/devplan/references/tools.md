...
Provider-specific tool surfaces are in `jira/tools.md` and `local/tools.md`. Both are optional during Step 2 exploration — the core `engineer` agent tool set (`mcp__plugin_imbas_tools__ast_search`, `mcp__plugin_imbas_tools__ast_analyze`, `Read`, `Grep`, `Glob`) is sufficient to produce a devplan manifest regardless of provider.
