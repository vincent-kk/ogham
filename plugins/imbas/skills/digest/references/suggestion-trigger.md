...
> **Provider scope**: Suggestion auto-trigger is Jira-only in v1 because it depends on `[OP: transition_issue]` events from `imbas:split`. Local mode has no equivalent transition signal — users invoke `imbas:digest` manually. See `local/workflow.md` Suggestion Trigger section.

## Trigger Conditions

The `imbas:digest` skill is suggested (not auto-executed) when all conditions are met:

1. **Done transition**: imbas:split calls `[OP: transition_issue]` to move an issue to Done status
