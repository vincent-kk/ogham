...
긴 논의 이력이 있는 Jira 이슈에서 핵심 의사결정, 거부된 대안, 기술적 제약, 미해결 이슈를 추출하여 3계층 압축 구조(경영진 요약 → 카테고리별 추출 → 원문 근거)로 정리한다. State Tracking + QA-Prompting 하이브리드 방식을 사용한다.

## 워크플로우

1. **이슈 읽기** — `imbas:read-issue` 스킬로 전체 이슈 로드, 첨부 미디어는 `atlassian:media-analysis`로 분석
