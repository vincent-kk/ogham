...
1. **Resolve run**:
   ...
   - Otherwise call `mcp__plugin_imbas_tools__run_list` to find the most recent run satisfying the source precondition.
2. **Call the planner tool**:
   ...
   - **On precondition failure** (`E-IP-3` missing/wrong source manifest, `E-IP-4` schema validation failure, `E-IP-5` invalid `--max-parallel`): emit terminal marker `Implement plan BLOCKED: <error_code> — <reason>` (see `errors.md` for the full table) and end execution. Do NOT continue to Step 3.
...
4. **Optional validation**:
   - `mcp__plugin_imbas_tools__manifest_validate {type: "implement-plan"}` to cross-check reference integrity.
