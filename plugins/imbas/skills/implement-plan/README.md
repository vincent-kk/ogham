...
Story와 cross-story Task를 병렬 배치로 묶어 명시적 실행 순서를 산출하는 DAG 기반 구현 일정 생성 스킬. 예: `[S1,S2,S5] -> [T1,S3] -> [S4,S6,S7]`
...
- `issue_ref`가 없는 티켓도 스케줄에 포함된다; 이슈 생성 후 재실행하면 리포트에 전체 참조가 채워진다.
