...
대신 주입하는 것은 파일이 스스로 말할 수 없는 것뿐입니다. 지금 어떤 규칙이 활성인지, 개입 강도가 어디 있는지, 배포된 규칙 중 원본에서 편집된 것이 있는지 — 그리고 **선출 계약**: 순간이 seiri 워크플로우와 맞으면(실패 발생, 다단계 작업 시작, 계획 완성, 완료 선언 직전, 리뷰 송수신) 행동 전에 그 워크플로우를 로드하고, 읽은 뒤에 판단합니다. 선출 라인은 규칙 배포와 무관하게 다이얼만으로 렌더됩니다. `standard`는 절차를 짚고 완료 선언의 소유자 하나만 명시하며 (`/seiri:verify`), `strict`는 모든 순간의 소유 스킬을 직접 명시합니다.
