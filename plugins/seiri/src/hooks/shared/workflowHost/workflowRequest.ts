 ...
 * @param tool Native tool name from the hook payload; must equal `adapter.runtimeTool` or the call yields `undefined`.
 ...
 * @param adapter Host adapter fixed at build time; supplies the compiled runtime tool address.
...
export function workflowRequest(
  ...
  if (tool !== adapter.runtimeTool) return undefined;
