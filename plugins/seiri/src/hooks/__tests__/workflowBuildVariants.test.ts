...
function run(host: string, hook: string, input: Record<string, unknown>) {
  ...
  const result = spawnSync(
    process.execPath,
    [portableJoin(pluginRoot, 'libs/run.cjs'), bridge],
    {
...
function fixture() {
  const cwd = mkdtempSync(portableJoin(tmpdir(), 'seiri-built-'));
  roots.push(cwd);
...
it.each(['claude', 'codex'])(
  '%s bundle contains only its workflow ABI',
  (host) => {
...
it.each(['claude', 'codex'])(
  '%s manifest runtime acknowledges only its native paired lifecycle',
  (host) => {
    const cwd = fixture();
    const native = {
      cwd,
    ...
    expect(
      run(host, 'pre-tool-use', { ...input, hook_event_name: 'PreToolUse' }),
    ).toBe('');
    const post = {
      ...input,
      hook_event_name: 'PostToolUse',
      tool_response: host === 'codex' ? { content } : content,
    };
    expect(run(host, 'post-tool-use', post)).toContain('started');
    expect(run(host, 'post-tool-use', post)).toBe('');
    expect(
      run(host, 'user-prompt-submit', {
        ...native,
        ...(host === 'codex' ? { turn_id: 'next' } : { prompt_id: 'next' }),
        hook_event_name: 'UserPromptSubmit',
      }),
    ).toContain('test-task');
    expect(run(host, 'post-tool-use', post)).toBe('');
