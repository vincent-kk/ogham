...
it('a same-task non-entry step is silent, while resume and pause on the same task acknowledge', () => {
  const base = fixture();
  expect(
  ...
  expect(
    call(
      base,
      'step-verify',
      { action: 'step', project_root: base.cwd, task: 'task-a', step: 'verify' },
      { status: 'accepted', action: 'step', task: 'task-a', step: 'verify', intent: 'change' },
    ).hookSpecificOutput,
  ).toBeUndefined();
  expect(
    call(
      base,
      'resume-same-task',
      { action: 'resume', project_root: base.cwd, task: 'task-a', intent: 'change' },
      { status: 'accepted', action: 'resume', task: 'task-a', intent: 'change' },
    ).hookSpecificOutput,
  ).toBeDefined();
  expect(
    call(
