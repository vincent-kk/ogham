...
- `utils/` 는 하위 모듈들의 공통 조상에 놓인 공유 헬퍼다. 한 곳만 쓰는 것이 생기면 그쪽으로 내린다.
- `.seiri/` 안의 비추적 파일은 전부 `ensureSeiriDir` 를 거쳐 만들어진다 — `.gitignore` 가 같이 생겨야 커밋으로 새지 않는다.
- 저장소 루트는 `findRepoRoot` (fs walk-up, spawn 없음) — 훅 경로에서도 쓰이며 세션마다 git 프로세스를 띄우지 않기 위한 선택이다.
- 파일 쓰기는 전부 `writeAtomically` 경유.
- 해시는 raw 바이트 기준 — 그래서 `.gitattributes` 가 규칙 템플릿을 LF 로 고정하고, 루트 `.prettierignore` 가 포매터를 막는다.
