...
- start/resume/pause/finish는 정확한 호출 ID·입력·actor·turn·generation에 대응하는 성공한 Post에서만 적용된다. 바인딩 변경은 generation을 올리고 진행 중 호출을 폐기한다.
...
- 새 turn, 다른 actor, 교체된 task, 중복·역순·늦은 결과가 이전 작업 효과를 만들지 않는다.
