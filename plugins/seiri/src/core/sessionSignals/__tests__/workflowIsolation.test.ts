...
import { observeBoundary } from '../workflow/transitions.js';
...
it('does not mutate or run effects without the actor lock', () => {
  ...
  let effects = 0;
  expect(withWorkflowState(id, false, () => effects++)).toBeUndefined();
...
it('revokes subsequent automatic effects when a boundary cannot acquire its lock', () => {
  ...
  expect(withWorkflowState(id, true, () => 'effect')).toBeUndefined();
});
...
it('rejects corrupted state until a trusted fresh boundary', () => {
  ...
  expect(withWorkflowState(id, false, () => 'effect')).toBeUndefined();
  observeBoundary({ ...id, turn: 'next' }, true);
  expect(withWorkflowState(id, false, (s) => s.turn)).toBe('next');
});
...
it('prunes stale invocations while retaining the active actor', () => {
  ...
  expect(
    withWorkflowState(id, false, (s) => Object.keys(s.invocations)),
  ).toEqual([]);
});
