...
- **3계층**: 개인 기본값 user `config.json`(호스트 상태 루트) + 프로젝트 기준선 `.seiri/config.json`(커밋, setup 표면 전용) + 런타임 밸브 `runtime.json`(비추적, `config` action 전용). 유효값 `runtime ?? project ?? user ?? off` 를 매 훅 재계산한다.
