...
- **2계층**: 기준선 `config.json`(커밋, setup 표면 전용) + 런타임 밸브 `runtime.json`(비추적, `config` action 전용). 유효값 `runtime ?? baseline ?? standard` 를 매 훅 재계산한다.
...
- 경로는 저장소 루트 기준(`findRepoRoot`)이라 워크트리끼리 밸브가 안 샌다. 조합·비교는 `cross-platform/compat` 경유 (네이티브 `node:path` 금지).
