...
- **3계층**: 개인 기본값 user `config.json`(호스트 상태 루트) + 프로젝트 기준선 `.seiri/config.json`(커밋, setup 표면 전용) + 런타임 밸브 `runtime.json`(비추적, `runtime` 도구의 `dial` action 전용). 유효값 `runtime ?? project ?? user ?? off` 를 매 훅 재계산한다.
...
- 세 계층은 같은 한 키짜리 객체라 `readDialFile` 하나로 파싱한다. `off`도 저장 가능한 정상 다이얼 값이다.
