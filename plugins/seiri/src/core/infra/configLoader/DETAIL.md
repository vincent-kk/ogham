...
- 다이얼은 3계층이다: user `config.json`(개인 기본값) · 프로젝트 `CONFIG_DIR` 아래 `CONFIG_FILE`(커밋되는 프로젝트 기준선) · `RUNTIME_FILE`(비추적 런타임 밸브 — 소비 프로젝트에서 실행 중에만 생기는 파일이라 저장소에는 없다). 세 이름은 `src/constants/files.ts`가 정본이다. 유효값은 `runtime ?? project ?? user ?? off` 이며 매 훅 실행마다 다시 계산한다. 기존 `advisory`·`standard`·`strict` 파일은 그대로 유효하다.
...
2026-09-26 — 프로젝트 기준선 경로를 `src/constants/files.ts` 상수 이름으로 적었다.
