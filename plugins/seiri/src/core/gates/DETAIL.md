...
- `CHECK` 가 결과 조건까지 검사하고 충족된 경우에만 고정 성공 문자열을 출력한다. 단순 조회 명령의 정상 종료는 결과 조건의 검사를 대신하지 못한다.
- `EXPECT` 는 양끝 공백을 정리한 대소문자 구분 리터럴 부분문자열이며, 하나의 관측 출력 줄 안에서만 일치한다. 슬래시·정규식 메타문자·플래그는 일반 문자다. 정규식 실행이나 구형 원장 이관은 하지 않는다.
- EXPECT가 없으면 `unjudgeable`, 출력이 비면 `unmet — no output`, EXPECT가 불일치하면 고정 사유의 `unmet`이다. 불일치 사유에는 저장소가 쓴 EXPECT 원문을 넣지 않는다. 이미 충족한 게이트의 non-met 재실행은 종류와 무관하게 체크를 풀고 `pending (regressed)`를 기록한다.
...
- MCP 서버 시작 시 `sweepStaleTasks`가 `.seiri/tasks`에서 디렉터리 자신과 모든 하위 항목이 72시간 넘게 수정되지 않은 항목을 정리한다.
- 작업 디렉터리는 `gates.lock`을 잡고 lock과 디렉터리 자신을 제외한 하위 항목을 다시 확인한 뒤 통째로 삭제한다. 사라진 항목은 건너뛰고 심볼릭 링크는 따라가지 않는다.
- git 추적·ignore 여부는 보지 않는다. gates는 프로세스를 띄우지 않는다.
...
| `sweepStaleTasks(projectRoot, now)`                | MCP 시작 시 유휴 작업을 정리하며 항목별 실패를 삼키고 다음 항목을 처리한다.              |
...
- EXPECT 불일치 판정은 저장소가 쓴 EXPECT 원문을 훅 결과에 반사하지 않는다.
...
### AC-gates-literal-expect — EXPECT는 리터럴 성공 문자열이다

- 정규식처럼 보이는 EXPECT도 전체 문자열 그대로 비교한다. 그 문자열이 없는 출력은 정규식으로는 일치하더라도 미충족이다.
- 일반 문자열의 대소문자 구분, 양끝 공백 정리, 줄 단위 부분문자열 매칭과 첫 일치 줄의 반환을 유지한다.
- CHECK가 검사하는 값이 달라져도 실제 조건이 충족되면 같은 성공 문자열로 증명한다. 조건이 틀리면 성공 문자열을 출력하지 않아 exit 정보 없는 관측 결과도 미충족이다.
...
### AC-gates-idle-retention — 유휴 작업 정리

- 오래된 `gates.md`와 새 `plan.md`가 함께 있거나 하위 디렉터리에 새 파일이 있으면 작업은 남는다.
- 빈 디렉터리는 디렉터리 자신의 mtime으로 판정하며 정확히 72시간이면 남는다.
- lock이 잡혀 있으면 남는다.
...
- 2026-09-27 — 원장의 영구 보존 대신 72시간 유휴 정리를 도입했다. `.seiri`가 끝없이 커지지 않도록 하기 위해서다.
- 2026-09-05 — 결과 조건 검사를 CHECK에 맡기고 EXPECT의 정규식 해석을 제거했다. CHECK/EXPECT 쌍이 고정 성공 문자열을 계약하므로 저장소 패턴을 훅에서 실행할 필요가 없고, 관측 출력 기반 증명은 유지된다.
- 2026-09-05 — 게이트 불일치 사유에서 EXPECT 원문을 제거했다. 저장소가 작성한 원문을 훅 `additionalContext`로 다시 내보내지 않으면서 met/unmet 판정은 그대로 유지하기 위해서다.
