...
- 서버 키는 `.mcp.json` 의 `tools` — 소비처는 full-form `mcp__plugin_seiri_tools__<name>` 로 참조한다. short-form 은 서브에이전트에서 해석되지 않는다.
- 프로젝트 루트는 `@ogham/cross-platform/host-paths` 의 `projectRoot(path?)`, 플러그인 루트는 `pluginRoot()`. 미해석 시 `process.cwd()` 폴백 금지.
- 로컬 서버 가드는 `@ogham/http-kit` 재사용 — 토큰·Origin 검증을 재구현하지 않는다.
- 도구 응답은 `wrapHandler` 를 통해 compact JSON 으로 직렬화하고, throw 는 모델이 읽을 수 있는 오류 결과로 바꾼다.
