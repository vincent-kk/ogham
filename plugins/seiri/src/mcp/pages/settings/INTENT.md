...
규칙 선택·훅 workflow mode·**저장 전 diff** 를 한 폼에 담은 로컬 페이지. 빌드가 `index.html` + `styles/` + `scripts/` 를 `public/settings.html` 한 장으로 인라인하고, 서버가 요청마다 상태를 주입해 서빙한다.
...
- workflow mode는 저장 값 `off`를 사용자가 이해할 수 있는 `Skills only`로 표시한다. 새 설정의 기본 선택이며 스킬은 남고 훅 context/state만 없다는 설명을 같은 radio card에 둔다.
...
- 모든 workflow mode는 visible label과 완전한 도움말을 갖고 keyboard radio 탐색을 유지한다.
