...
`127.0.0.1` 전용 설정 서버를 띄워 브라우저 폼(규칙 선택 + 다이얼)을 열고, 사용자의 저장/닫기를 **정해진 시간 안에서 기다렸다가** 결과를 반환한다. 기다림이 있어야 setup 스킬이 URL 만 던지고 멈추는 대신 같은 턴에서 이어간다.
...
- 모듈 레벨 `currentServer` 싱글톤 — `pending` 후 재호출이 같은 세션을 이어 기다린다. 다른 프로젝트 요청이 오면 교체한다.
