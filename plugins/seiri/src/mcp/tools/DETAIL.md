...
- 도구는 settings, gates, workflow다. 코드 읽기·검색·분석은 하니스가 제공한다. workflow는 명시적 참여 요청 검증이며 실제 효과는 짝이 맞는 Post 훅이 적용한다.
...
- `settings/` — 브라우저 폼과 bounded long-poll, 헤드리스 폴백(`status`·`manifest`·`plan`·`sync`), 다이얼 `config`.
- `gates/` — 작업별 게이트 원장의 상태 조회·포기·수동 증거 기록. 명령을 실행하거나 원장을 생성하지 않는다.
- workflow는 start/resume/pause/finish 요청을 검증합니다. 필수 절대 project_root와 task를 받고 start/resume에 intent를 요구하며 native identity는 입력받지 않습니다. off/advisory는 disabled이며 상태 적용은 성공한 paired Post의 책임입니다.
