...
- `dial`(`dial_op`: get/set/clear, `intervention`)은 훅 짝 없이 `.seiri/runtime.json` 밸브를 즉시 조회·설정·해제한다. 참여 액션은 입력만 검증하며, `dial`은 `.seiri/runtime.json`만 쓴다.
...
- `dial`은 `.seiri/runtime.json` 외에는 아무것도 쓰지 않는다.
