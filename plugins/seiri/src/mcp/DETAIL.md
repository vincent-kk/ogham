...
- 등록된 도구는 settings, gates, runtime 세 개다. runtime은 호스트가 제공하지 않는 명시적 조건부 참여·단계·다이얼 밸브 계약이다.
...
- settings는 설정 표면, gates는 작업 증거, runtime은 명시적 참여·단계·다이얼 요청을 소유합니다. accepted 응답은 hook ACK를 대신하지 않습니다.
