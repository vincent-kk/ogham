...
- 스킬 파티션의 정본은 `src/constants/skillPolicy.ts` 다 — 자동 호출 규율 7종, 조건부 질문 스킬 2종(write-plan·review-plan), 모델 목록에 보이는 사용자 시작 스킬 6종, 모델에게 숨기는 사용자 전용 게이트 2종(setup·scaffold-pr). `skillPolicy.test.ts` 가 각 스킬의 frontmatter 와 본문 정본 문장을 검사한다.
...
- `architect` 는 모델-visible 사용자 시작 문서 스킬이며 표준 워크플로우 체인과 `brainstorm`의 필수 전후 단계가 아니다.
...
2026-08-31 — architect 스킬 계약을 추가하고 write-plan·review-plan의 planning method 우선 계약을 보존했다.
