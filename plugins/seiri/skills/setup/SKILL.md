...
**Phase 1 — settings page.** Call `mcp__plugin_seiri_tools__settings({ action: "open", project_root: "<absolute-path>", wait_seconds: 300 })`. It blocks until the user acts. Dispatch on `result.status`, same response:
...
1. `mcp__plugin_seiri_tools__settings({ action: "status", project_root })` — what is deployed now.
2. `mcp__plugin_seiri_tools__settings({ action: "plan", project_root, selections })` — show `result` and get explicit confirmation; retain `result.revision`.
3. `mcp__plugin_seiri_tools__settings({ action: "sync", project_root, selections, resync, revision })` — only after that confirmation, passing the revision from step 2.
