...
Deploys the rules you select to `.claude/rules/seiri_*.md`, where the harness loads them at the start of every session. Nothing is written before you confirm, and the page shows the exact change first.

## Workflow

**Phase 1 — settings page.** Call `mcp__plugin_seiri_tools__open_settings({ path: "<absolute-path>", waitSeconds: 300 })`. It opens the browser form and blocks until the user acts. Dispatch on `result.status` and continue in the same response:

- `saved` — report the outcome list from `result.summary.ruleDocs` in one or two lines. Name any entry whose action is `drift` or `skip`; those are the ones that did not land.
- `pending` — call once more. If still pending, surface `result.url` and stop.
- `closed` — nothing changed. Say so and stop.

**Phase 2 — headless fallback.** Only when Phase 1 cannot open a browser (the call fails, or the host has no display). Then:
...
- Pass a rule id in `resync` only when the user asked for that rule's local edits to be discarded. Report drift; do not resolve it for them.
