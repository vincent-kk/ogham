...
for (const entry of manifest.rules) {
  ...
  // `text eol=lf` for plugins/seiri/templates/rules/*.md so developers
  ...
  if (entry.templateHash !== hash) {
    mismatches.push({
      id: entry.id,
      expected: entry.templateHash ?? '<absent>',
      actual: hash,
    });
    if (!checkMode) {
...
if (checkMode) {
  if (mismatches.length > 0) {
    ...
    console.error(
      'Run `node scripts/sync-rule-hashes.mjs` (or `yarn seiri build`) to refresh.',
    );
    process.exit(1);
  }
