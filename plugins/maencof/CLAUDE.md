...
- **MCP 도구 참조**: skills/agents 는 full-form `mcp__plugin_maencof_tools__<tool>` 로 도구를 참조 (`.mcp.json` 서버 키 `tools`). short-form `mcp_tools_*` 는 서브에이전트에서 해석되지 않아 도구 grant 실패 → 디스크 직접 접근 폴백을 유발하므로 사용 금지.
