...
- **`/seiri:mental-model`** (if installed) — the same discipline specialized to code, with `file:line` tracing. Prefer it when the question is what code does.
...
### Step 2 — Collect shadows

A shadow is an observation with provenance: where it fell, when, who cast it. Sources, cheapest first:
...
### Step 3 — Hypothesize the Form

Write claims that could be wrong: each states what must hold, and what breaks if it does not. One claim, one layer — **Shape** (what it is), **Behaviour** (how it acts), **Purpose** (what it is for and what it protects). Cut every sentence that predicts nothing — that is summary, and summary cannot be checked. Forms and layers: `references/claims.md`.
...
An unattacked claim does not enter the model. Where counterexamples hide, per layer, and how each medium distorts: `references/breaking.md`.

### Step 5 — Report what survived, marked

Each claim carries `seen`, `heard`, `inferred`, or `assumed` (`references/claims.md`). Refuted claims stay in the report with what killed them. There is no mark for knowledge: the strongest claim is seen from independent angles and still standing — which is not proof.
...
- Hand-offs: a model that shapes a decision → `/maencof:think`; shadows missing → `/maencof:explore`; a code-behaviour question → `/seiri:mental-model` if installed.
