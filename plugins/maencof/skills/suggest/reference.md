...
- `rebuildRecommended: true` → warn: "The index is stale. Suggestions may be inaccurate. `/maencof:build --force --reset-cache` is recommended."
...
- If user selects a suggestion → read the document with `mcp__plugin_maencof_tools__read` and show content
