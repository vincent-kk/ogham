...
name: suggest
user-invocable: true
description: 'Recommends missing knowledge-graph links using Spreading Activation and Jaccard similarity. Use when linking a new document, expanding connections, or resolving orphan nodes from checkup.'
...
Call `mcp__plugin_maencof_tools__kg_status()`. Abort if no index. Warn if stale.
...
Call `mcp__plugin_maencof_tools__kg_suggest_links` with the determined target parameters.
...
User selects a suggestion to view details via `mcp__plugin_maencof_tools__read`, or manually adds links.
