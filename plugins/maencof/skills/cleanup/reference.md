...
- Keyword/title → search with `mcp__plugin_maencof_tools__kg_search` and confirm with user
...
```
mcp__plugin_maencof_tools__read(path: target path)
```
...
```
mcp__plugin_maencof_tools__kg_navigate(path: target path, include_inbound: true)
```
...
```
mcp__plugin_maencof_tools__delete(path: target path, force: --force flag)
```
...
Note: Run `/maencof:build --force --reset-cache` to update the index.
```
...
```
mcp__plugin_maencof_tools__claudemd_read()
```
...
```
mcp__plugin_maencof_tools__claudemd_remove(dry_run: true)
```
...
```
mcp__plugin_maencof_tools__claudemd_remove(dry_run: false)
```
