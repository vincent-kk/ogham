...
1. **Identify target** — resolve from path, keyword search (`mcp__plugin_maencof_tools__kg_search`), or ask user
2. **Preview** — read target with `mcp__plugin_maencof_tools__read`, show Frontmatter summary
3. **Safety checks** — L1 (01_Core/) deletion is forbidden; backlink check via `mcp__plugin_maencof_tools__kg_navigate`
4. **Execute** — `mcp__plugin_maencof_tools__delete` after user confirmation (or `--force`)
...
- **read**: `mcp__plugin_maencof_tools__claudemd_read()` — display current maencof section
- **remove**: `mcp__plugin_maencof_tools__claudemd_remove(dry_run: true)` → confirm → `mcp__plugin_maencof_tools__claudemd_remove(dry_run: false)`
...
| `mcp__plugin_maencof_tools__kg_navigate`     | Check inbound links / backlink warnings (document mode)    |
| `mcp__plugin_maencof_tools__move`            | Promote buffer items to target layer (buffer-cleanup mode) |
| `mcp__plugin_maencof_tools__claudemd_read`   | Read CLAUDE.md maencof section (claudemd mode)             |
| `mcp__plugin_maencof_tools__claudemd_remove` | Remove CLAUDE.md maencof section (claudemd mode)           |
