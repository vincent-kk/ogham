...
Delegate to the checkup agent. The agent runs `mcp__plugin_maencof_tools__kg_status`, `mcp__plugin_maencof_tools__kg_navigate`, `mcp__plugin_maencof_tools__kg_suggest_links`, `mcp__plugin_maencof_tools__read` across all vault documents and `.maencof-meta/` to perform 8 diagnostic checks.
...
- Fill missing Frontmatter fields (`mcp__plugin_maencof_tools__update`)
...
| Tool                                          | Purpose                                                                                                              |
| --------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| `mcp__plugin_maencof_tools__kg_status`        | Index status, stale ratio, wikilink-orphan breakdown by layer/archived + paths (D1, D2); sole tool in `--quick` mode |
| `mcp__plugin_maencof_tools__read`             | Frontmatter validation (D4, D6)                                                                                      |
| `mcp__plugin_maencof_tools__update`           | Auto-fix Frontmatter (D4, D6)                                                                                        |
| `mcp__plugin_maencof_tools__kg_navigate`      | Verify link validity, detect broken links (D3)                                                                       |
| `mcp__plugin_maencof_tools__kg_search`        | Tag-based search for duplicate detection (D5)                                                                        |
| `mcp__plugin_maencof_tools__kg_suggest_links` | Query-time link candidates for authored-orphan triage (D1)                                                           |
...
| `--quick`   | false   | Short-circuit to `mcp__plugin_maencof_tools__kg_status` only and render a compact health report (absorbs the former `maencof-diagnose` skill) |
