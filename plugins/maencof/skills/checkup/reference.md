...
- **Wikilink (LINK) isolation** — report `linkOrphanCount` / `linkInboundOrphanCount` / `linkOutboundOrphanCount`. These surface semantic isolation that the total-degree orphan check hides (folder SIBLING edges make almost every node look "connected"). Treat raw L3 external clippings leniently — prose wikilinks may be legitimately absent there:
  ```
  Wikilink Isolation:
    fully isolated (no in/out LINK): N
