...
- Keyword/title → search with `mcp__maencof__kg_search` and confirm with user
...
```
mcp__maencof__read(path: target path)
```
...
```
mcp__maencof__kg_navigate(path: target path, include_inbound: true)
```
...
```
mcp__maencof__delete(path: target path, force: --force flag)
```
...
```
mcp__maencof__claudemd_remove(dry_run: true)
```
...
```
mcp__maencof__claudemd_remove(dry_run: false)
```
