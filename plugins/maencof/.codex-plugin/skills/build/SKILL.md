...
Otherwise, check the current index status with the `mcp__maencof__kg_status` MCP tool.
...
For incremental build: recompute only changed files + 1-hop neighbors. For full/force mode: call `mcp__maencof__kg_build(force=true)` to reprocess every node.

## Available MCP Tools

| Tool                                   | Purpose                                                          |
| -------------------------------------- | ---------------------------------------------------------------- |
| `mcp__maencof__kg_status` | Check current index status (pre- and post-rebuild snapshots)     |
| `mcp__maencof__kg_build`  | Run index build (incremental: `force=false`, full: `force=true`) |

## Error Handling

- **No vault path**: "Vault not initialized. Please run `/maencof:setup` first."
