...
One file per change date: `02_Derived/changelog/YYYY-MM-DD.md`. Read any existing file for that date first to avoid duplicate entries, then write via `mcp__maencof__create` (new) or `mcp__maencof__update` (existing).
...
| Tool                                | Type   | Purpose                                                               |
| ----------------------------------- | ------ | --------------------------------------------------------------------- |
| `mcp__maencof__create` | MCP    | Create new vault document                                             |
| `mcp__maencof__read`   | MCP    | Read existing vault document                                          |
| `mcp__maencof__update` | MCP    | Update existing vault document (when the date's entry already exists) |
