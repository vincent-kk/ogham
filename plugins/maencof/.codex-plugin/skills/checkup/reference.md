...
| --- | ----------------------------------------------------------------------------------------------------------------------- | ------------------- | -------- | --------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **Orphan Node**: nodes with 0 inbound and 0 outbound wikilinks (LINK subgraph), triaged by provenance bucket            | orphan-node         | warning  | partially (`mcp__maencof__kg_suggest_links` for authored docs; archived stubs and raw clippings are expected states) |
...
- `mcp__maencof__kg_status` (`include_orphan_paths: true`) → detect D2 (stale) + D1 orphan buckets (`linkOrphanByLayer` / `linkOrphanArchivedCount` / `linkOrphanPaths`)
- `Glob "**/*.md"` → collect full file list
- `mcp__maencof__read` per file → verify D6 (Frontmatter), D4 (Layer violation), D8 (missing L1 gist)
- `mcp__maencof__kg_navigate` → validate backlink-index.json integrity → detect D3 (broken link; a target under `99_Archive/` never appears as a graph node — check disk existence instead and classify as informational archive-reference)
- `mcp__maencof__kg_suggest_links` on sampled authored orphans → D1 actionability (review candidates from the tool; when none exist, add tags first)
...
- Authored, actionable: {file} — link candidates found → call `mcp__maencof__kg_suggest_links` and review its output
- Authored, no candidates: {file} — add tags first, then call `mcp__maencof__kg_suggest_links` again
...
| ------------------------------------- | --------------------------------------------- | ------------------------ |
| Fill missing Frontmatter fields       | `mcp__maencof__update`           | D6 items                 |
| Rebuild stale index                   | `/maencof:build --force --reset-cache`        | D2 items                 |
| Fix layer field based on path         | `mcp__maencof__update`           | D4 items                 |
| Suggest links for orphan nodes        | `mcp__maencof__kg_suggest_links` | D1 authored-bucket items |
...
**Layer 1 (01_Core/) exception**: Auto-fix via `mcp__maencof__update` is forbidden for L1 files. Report the issue and guide the user to run `/maencof:setup --step 4` or edit manually.

**No-frontmatter exception**: `mcp__maencof__update` requires the target to already have a frontmatter block. A D6 file whose frontmatter block is entirely missing cannot be auto-fixed via `update` — recreate it with `mcp__maencof__create` or repair the file manually. (Files that have a frontmatter block but invalid/missing fields are still auto-fixable.)
...
Always include sub-layer distribution from the `mcp__maencof__kg_status` response (`subLayerDistribution` field) after the status banner:
...
| Structural issue | —             | `/maencof:checkup` (full 8-check diagnosis) |
