...
name: checkup
user-invocable: true
description: 'Runs 8 vault diagnostics — broken links, orphan nodes, frontmatter errors, duplicates, auto-insight health — with prioritized auto-fix suggestions; --quick is a read-only kg_status health report. Use for vault health checks and maintenance.'
...
Checks knowledge vault health across 8 diagnostic items and suggests auto-fixes. Delegates detailed analysis to the checkup agent. Also supports a lightweight `--quick` mode that short-circuits to a single `mcp__maencof__kg_status` call and renders a compact health report (absorbs the former `maencof-diagnose` skill).
...
Delegate to the checkup agent. The agent runs `mcp__maencof__kg_status`, `mcp__maencof__kg_navigate`, `mcp__maencof__kg_suggest_links`, `mcp__maencof__read` across all vault documents and `.maencof-meta/` to perform 8 diagnostic checks.
...
- Fill missing Frontmatter fields (`mcp__maencof__update`)
...
- Call only `mcp__maencof__kg_status`
...
| Tool                                          | Purpose                                                                                                              |
| --------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| `mcp__maencof__kg_status`        | Index status, stale ratio, wikilink-orphan breakdown by layer/archived + paths (D1, D2); sole tool in `--quick` mode |
| `mcp__maencof__read`             | Frontmatter validation (D4, D6)                                                                                      |
| `mcp__maencof__update`           | Auto-fix Frontmatter (D4, D6)                                                                                        |
| `mcp__maencof__kg_navigate`      | Verify link validity, detect broken links (D3)                                                                       |
| `mcp__maencof__kg_search`        | Tag-based search for duplicate detection (D5)                                                                        |
| `mcp__maencof__kg_suggest_links` | Query-time link candidates for authored-orphan triage (D1)                                                           |
...
| `--quick`   | false   | Short-circuit to `mcp__maencof__kg_status` only and render a compact health report (absorbs the former `maencof-diagnose` skill) |
