...
`sub_layer` applies only to L3. For every other layer, omit it entirely — passing it will be rejected by `mcp__maencof__create` because **sub_layer is only valid for Layer 3**.
...
Pass `sub_layer` to `mcp__maencof__create`. If the content doesn't clearly fit a sub-layer, default to **topical** (`03_External/topical/`). This aligns with the L3 Classification Rules used by `/maencof:migrate`.
...
Search for similar documents with `mcp__maencof__kg_search` to prevent duplicates:
...
Create a distinct new document with the `mcp__maencof__create` MCP tool:
...
**L3 Frontmatter** (fields written into the document by `mcp__maencof__create`):
...
`confidence` (internalization 0.0-1.0, initial ~0.3) is not a create parameter — set it after creation via `mcp__maencof__update` (frontmatter). Sub-layer scalar metadata (L3A `person_ref`/`trust_level`/`expertise_domains`, L3B `org_type`/`membership_status`/`ba_context`, L3C `topic_category`/`maturity`) is likewise settable only through post-create `update`. The nested L3A `person` object still has no MCP write path — do not promise it during remember.
...
| Tool                                   | Purpose                                               |
| -------------------------------------- | ----------------------------------------------------- |
| `mcp__maencof__create`    | Create document (primary tool)                        |
| `mcp__maencof__kg_search` | Pre-creation duplicate check                          |
| `mcp__maencof__update`    | Update existing document (when duplicate found)       |
| `mcp__maencof__read`      | Read existing document content (when duplicate found) |
