...
Use complete `mcp__maencof__kg_inventory` pagination and full `mcp__maencof__read` bodies, including unchanged/held/error candidates in the report. Use `mcp__maencof__kg_search` only to find related current accounts. Preview exact source and target writes, preserve originals, verify the target before marking sources, and stop on failure or drift. Only validated knowledge may form an L2 synthesis; hold unvalidated L5 groups. Execute through `mcp__maencof__create` and `mcp__maencof__update`, then read back. Source deletion, archival and implicit layer promotion are outside this mode.
...
- **`organize`** — judge + execute. Mutates the vault via `mcp__maencof__move` after explicit user confirmation. Use when you are ready to apply transitions.
- **`reflect`** — read-only vault assessment. Produces an analysis report and may retain review snapshots outside the vault. Use to preview candidates before committing. Rule of thumb: preview → `reflect`; apply layer changes → `organize`; propose new links → call `mcp__maencof__kg_suggest_links`.
...
> **Note.** Stale-node bookkeeping is handled inside the MCP server middleware layer (around `mcp__maencof__move` / `mcp__maencof__update` calls and at boot-time via vault scanning). It never interacts with the organize workflow directly.
...
Check vault status and stale nodes with the `mcp__maencof__kg_status` MCP tool before delegating to the agent. If no index is found, abort with: "No index found. Please run `/maencof:build` first."
...
- Call `mcp__maencof__move` (with `target_sub_layer` when moving to L3 or L5 sub-directories)
- Update the Frontmatter `layer` and `sub_layer` fields
- Update link paths
- **L5 field auto-strip**: When moving out of Layer 5, `mcp__maencof__move` automatically strips `buffer_type` · `promotion_target` · `source_context`
...
| ---------------------------------------- | --------------------------------------- | ---------------------------------- |
| `mcp__maencof__kg_status`   | skill (Step 1)                          | Check vault status and stale-nodes |
| `mcp__maencof__read`        | active agent or memory-organizer | Read full bodies and metadata; verify changes |
| `mcp__maencof__kg_navigate` | memory-organizer agent                  | Traverse link relationships        |
| `mcp__maencof__move`        | memory-organizer agent (execute module) | Execute file move                  |
| `mcp__maencof__update`      | active agent or memory-organizer within its access matrix | Update content and Frontmatter |
| `mcp__maencof__kg_inventory` | active agent (insight mode) | Enumerate all scoped candidates |
| `mcp__maencof__kg_search` | active agent (insight mode) | Locate related current accounts |
| `mcp__maencof__create` | active agent (insight or maintenance mode) | Create reviewed knowledge documents |
...
4. **Execute** via `mcp__maencof__move` with `target_sub_layer` — buffer metadata is auto-stripped. Pass `target_subdirectory` (e.g. `"projects"`, max 2 levels) to file the promoted document into a subdirectory of the target layer/sub-layer.
