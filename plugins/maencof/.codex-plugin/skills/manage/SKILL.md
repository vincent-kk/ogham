...
>
> Note: `mcp__maencof__create` is intentionally excluded here. It creates vault knowledge documents (requires Layer + tags + Frontmatter) and is not suitable for generating plugin structure files such as SKILL.md or agent definitions. Use the `Write` tool for those.
