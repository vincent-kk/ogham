...
- Check index status first with `mcp__maencof__kg_status`
...
- `rebuildRecommended: true` -> warn before exploration: "The index is stale. Results may be inaccurate. `/maencof:build --force --reset-cache` is recommended."
...
- Keyword -> pass to `mcp__maencof__kg_search` in Step 3; the top result becomes the seed node
...
Run SA via the `mcp__maencof__kg_search` MCP tool:
...
When the user selects a number, look up neighbors with `mcp__maencof__kg_navigate`:
...
```
mcp__maencof__read(path: selected node path)
```

To assemble token-optimized context for multiple activated nodes, use:

```
mcp__maencof__kg_context(query: current seed or keyword, token_budget: 2000)
```
...
| Tool                                     | Purpose                                                        |
| ---------------------------------------- | -------------------------------------------------------------- |
| `mcp__maencof__kg_status`   | Check index status (stale check)                               |
| `mcp__maencof__kg_search`   | SA-based related document search                               |
| `mcp__maencof__kg_navigate` | Look up a node's neighbors (inbound/outbound links, hierarchy) |
| `mcp__maencof__kg_context`  | Assemble token-optimized context blocks                        |
| `mcp__maencof__read`        | Read the full content of a selected document                   |
