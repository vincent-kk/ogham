...
지식 그래프 쿼리 엔진. 시드 노드 해석(쿼리 내 상대 IDF 로 흔한 토큰 시드 강등 — 동형이의어 노이즈 억제, 후보 union 불변), kg_context 자연어 분해(`deriveContextSeeds` — 단어 OR + 인접 2-gram phrase), QGA-SA 확산 실행 (maxHops → 반복 횟수 T 매핑, 쿼리 토큰 게이트 공급).
