...
describe('Setup Wizard 7단계 통합 테스트', () => {
  ...
  it('Step 4: scaffold-tree — 4개 Layer 디렉토리 + Core Identity 문서 생성', async () => {
    ...
    const identityResult = await handleMaencofCreate(vault, {
      layer: 1,
      tags: ['identity', 'core'],
    ...
    // 초기 메모 문서 생성 (Layer 2)
    const memoResult = await handleMaencofCreate(vault, {
      layer: 2,
      tags: ['setup', 'initial'],
  ...
  it('Step 6: guide — CLAUDE.md에 maencof 섹션 통합', async () => {
    const claudeMdPath = join(vault, 'CLAUDE.md');
    const maencofGuide = `## maencof 지식 트리
...
- Layer 4 (04_Action): 액션 아이템`;
    ...
    const mergeResult = mergeMaencofSection(claudeMdPath, maencofGuide, {
      dryRun: false,
    });
    expect(mergeResult.changed).toBe(true);
    expect(mergeResult.hadExistingSection).toBe(false);
    expect(mergeResult.content).toContain(MAENCOF_START_MARKER);
    expect(mergeResult.content).toContain(MAENCOF_END_MARKER);
    expect(mergeResult.content).toContain('maencof 지식 트리');

    // 파일에서 섹션 읽기
    const readSection = readMaencofSection(claudeMdPath);
  ...
  it('전체 7단계 워크플로우 시뮬레이션', async () => {
    ...
    // Step 6: CLAUDE.md 통합
    const claudeMdPath = join(vault, 'CLAUDE.md');
    const mergeResult = mergeMaencofSection(
      claudeMdPath,
      '## maencof 활성화됨\n- vault: ' + vault,
    );
    expect(mergeResult.changed).toBe(true);
