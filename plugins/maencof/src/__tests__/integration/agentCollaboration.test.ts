/**
 * @file agentCollaboration.test.ts
...
async function makeTempVault(): Promise<string> {
  return mkdtemp(join(tmpdir(), 'maencof-agent-'));
}
...
describe('에이전트 협업 시퀀스 통합 테스트', () => {
  ...
  it('Agent 1: identity-guardian — Layer 1 이동 차단', async () => {
    const moveResult = await handleMaencofMove(vault, {
      path: '01_Core/core-identity.md',
      target_layer: 2,
  ...
  it('Agent 2: memory-organizer — 만료된 문서 정리', async () => {
    ...
    const expiredRead = await handleMaencofRead(vault, {
    ...
    const extRead = await handleMaencofRead(vault, {
  ...
  it('Agent 3: knowledge-connector — 검색 후 연결 문서 생성', async () => {
    ...
    if (!('error' in searchResult)) {
      ...
      const connectorDoc = await handleMaencofCreate(vault, {
        layer: 2,
...
검색 시드: knowledge, derived`,
  ...
  it('3개 에이전트 순차 협업 시퀀스', async () => {
    ...
    // Layer 1 보호 검증
    const deleteAttempt = await handleMaencofDelete(vault, {
      path: '01_Core/core-identity.md',
    });
    executionLog.push({
    ...
    // 만료 문서 정리
    const expiredDelete = await handleMaencofDelete(vault, {
      path: '04_Action/expired-task.md',
    });
    executionLog.push({
    ...
    // Layer 3 → 2 전이
    const layerMove = await handleMaencofMove(vault, {
      path: '03_External/high-conf-ext.md',
      target_layer: 2,
      reason: '신뢰도 검증 완료',
    });
    ...
    const newDoc = await handleMaencofCreate(vault, {
      layer: 2,
