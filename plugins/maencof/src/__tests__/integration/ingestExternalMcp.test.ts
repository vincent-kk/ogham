/**
 * @file ingestExternalMcp.test.ts
...
/** 외부 검색 결과를 maencof Layer 3 문서로 변환 */
...
이 문서는 "${query}" 검색으로 발견된 외부 자료입니다.`;
    ...
    const createResult = await handleMaencofCreate(vaultPath, {
      layer: 3,
...
describe('ingest 스킬 외부 MCP 시뮬레이션 통합 테스트', () => {
  ...
  it('시나리오 2: 검색 결과를 Layer 3 문서로 ingest', async () => {
    ...
    // 생성된 문서 읽기 확인
    const readResult = await handleMaencofRead(vault, { path: ingested[0] });
  ...
  it('시나리오 6: 전체 ingest 파이프라인 — 검색 → ingest → 빌드 → 검색 → 연결', async () => {
    ...
    const synthesisDoc = await handleMaencofCreate(vault, {
      layer: 2,
