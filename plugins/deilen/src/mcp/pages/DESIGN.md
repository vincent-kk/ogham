...
- **문서 본문(`.viewer`)**: `--font-doc` 라틴 세리프 + 한글 고딕 폴백(`Apple SD Gothic Neo`…, 라틴 폰트에 없는 글리프만 per-glyph fallback 으로 넘어감), `16.5px / 1.75`. 사용자 설정 `font_family` 는 `--font-doc` 를 덮어쓴다. 헤딩 weight 는 700 (Charter 는 400/700 두 웨이트뿐 — 중간값 금지).
