...
| File                  | Role                                                             |
| --------------------- | ---------------------------------------------------------------- |
| `createServer.ts`     | `McpServer('tools')` + 도구 4개 `registerTool` (zod inputSchema) |
| `startServer.ts`      | 만료 세션 prune 후 stdio transport 로 connect                    |
| `registerShutdown.ts` | 종료 시 `settleAllResolvers` + HTTP 서버 close                   |
| `index.ts`            | barrel — `createServer`, `startServer` re-export                 |

## Conventions

- 모든 `registerTool` 콜백은 `wrapHandler` 로 감쌈 (throw 흡수 + extra 전달)
- 도구 등록명은 snake_case (`render_report` 등), 심볼·파일은 camelCase
