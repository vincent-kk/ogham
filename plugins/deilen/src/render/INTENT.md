...
| `markdownIt/` | markdown-it 인스턴스 + math/source-line 규칙 (organ)    |
| `sanitize/`   | allowlist HTML 정제 (태그·속성·URL 스킴) (organ)        |
| `utils/`      | `lineAttrs`·`extractTitle`·`collectSourceLines` (organ) |
| `index.ts`    | barrel                                                  |

## Conventions

- markdown-it `html:false` (raw HTML 차단 1차) + `linkify` + `typographer`
- 코드펜스→`<pre><code data-lang>`, ` ```mermaid `→`<div class="deilen-mermaid">`, `$…$`/`$$…$$`→`<span class="deilen-math" data-display>`
