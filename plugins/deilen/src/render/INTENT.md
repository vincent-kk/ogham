...
- markdown-it `html:true` + `linkify` + `typographer` — raw HTML(GitHub 스타일 `<details>` 등)은 토큰 단계에서 raw 프로파일 정제 후 첫 여는 태그에 라인 앵커 주입
...
- 모든 출력 HTML 은 `sanitizeHtml` 통과 — `html:true` 이므로 sanitizer 가 유일한 XSS 방어층
...
- raw HTML 유래 속성에 내부 예약 class(`deilen-*`)·`data-source-*` 허용 (렌더러 트리거·앵커 위조 경로)
