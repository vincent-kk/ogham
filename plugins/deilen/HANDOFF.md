...
> Phase 0 스캐폴드 완료 시점의 인수인계 문서. **설계 정본은 [`.metadata/deilen/`](../../.metadata/deilen/)**, 구현은 이 플랜의 단계 순서를 따른다. 본 문서는 "무엇을 어디에 어떤 순서로" 만드는지를 정의한다.
...
- **검증 (착수 전 1회)**: 루트에서 `yarn install` → `yarn deilen typecheck` + `yarn deilen test:run` 통과 확인.
...
| `mcp-tools.md`         | `render_viewer`/`collect_feedback`/`open_settings`/`close_viewer` |
...
- `src/mcp/httpServer/{httpServer,routes,routeContext}.ts` + `handlers/{handleGetViewer,handleGetViewerData,handleGetAsset,handlePing}.ts` + `utils/{verifyToken,sendJson,escapeJsonForHtml}.ts` + `INTENT.md`
- `src/mcp/pages/viewer/{index.html,styles/,scripts/{app,enhance,comments,images,submit,copy}.js,renderers/*.entry.ts}`
- `src/mcp/tools/renderViewer/`
- `scripts/{buildViewerHtml,buildRenderers,buildMcpServer}.mjs`
- **빌드 갱신**: `build` 에 `buildViewerHtml → buildRenderers` (tsc 앞) + `buildMcpServer` (tsc 뒤) 추가. `build:plugin` 신설.
- 수동 확인: 브라우저에서 문서 가독 표시 + lazy 렌더러 동작.

### Phase 3 — 피드백 수집·수거

- `src/mcp/httpServer/handlers/handlePostFeedback.ts` + `utils/parseMultipart.ts` (busboy)
- `src/core/feedbackStore/`
- `sessionStore` resolver(멱등 `settle`) + `src/mcp/tools/{collectFeedback,closeViewer}/`
...
- `close_viewer`·세션 TTL·idle/heartbeat 폴백 종료 검증
...
| Phase 2        | `… → version:sync → buildViewerHtml → buildRenderers → tsc → buildMcpServer`      |
| Phase 5        | `… → buildViewerHtml → buildSettingsHtml → buildRenderers → tsc → buildMcpServer` |
...
- **path**: 신뢰 임의경로 + `max_viewer_mb` 캡(canonicalize·utf8·일반파일 검증).
...
- deilen 은 **hook 없음** → `libs/run.cjs` 불필요.

## 7. 명령

```bash
yarn install                 # 루트 (신규 워크스페이스 인식)
yarn deilen typecheck
yarn deilen test:run
yarn deilen build             # bridge/ 산출 (Phase 2+); 사용자가 커밋
yarn deilen version:sync
```
