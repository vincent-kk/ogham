...
MCP 서버 계층. 결정론·계약 실행 레이어로, core/adapters를 얇게 오케스트레이션해 도구 5종(`paper_search`·`mesh_lookup`·`fetch_fulltext`·`setup`·`auth_check`)을 노출한다.
...
| `pages/`       | 설정 폼 프론트엔드(마크업·스타일·스크립트, `public/settings.html`) |
