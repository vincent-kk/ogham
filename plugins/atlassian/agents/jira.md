...
Send version-agnostic paths (`/issue/{key}`, `/myself` etc. — no `/rest/api/{2|3}` prefix). MCP automatically attaches `/rest/api/3` (Cloud) or `/rest/api/2` (Server/DC), and adds `X-Atlassian-Token: no-check` to DC non-GET requests. Markdown bodies are converted to ADF (Cloud v3) or Wiki Markup (Server/DC v2) automatically when `content_format: "markdown"` is set. The wiki converter escapes literal specials (`[ ] { } | * _ - + ^ ~ !`) outside link/image bodies; after writes where formatting matters, verify via `GET /issue/{key}?expand=renderedFields` — a `class="error"` span means broken markup.
...
4. Compose calls to the MCP tool selected by the schema with the documented parameters
