...
```json
{
  ...
  "key_observations": ["~2 second delay observed during Google OAuth popup"]
}
```

Each frame in analysis.json includes a `path` field so callers can verify specific scenes by reading the actual frame image directly (e.g., "Show me the error toast in Scene 3").

## Frame Output Naming

Output frames follow the pattern `frame_NNNN.jpg` (zero-padded 4 digits). Gaps in numbering are normal — they indicate pruned frames. Example: `frame_0001.jpg`, `frame_0003.jpg`, `frame_0007.jpg` means frames 2, 4-6 were pruned.
...
For long videos (>5 min), reduce workload. The stderr-null suffix is shell-specific: POSIX uses `2>/dev/null`, Windows cmd uses `2>nul`, Windows PowerShell uses `2>$null`.
