...
5. The MCP call completes after a successful save or a terminal close/timeout. Report a successful result's `config_path` verbatim; a failed result has no saved path, so report only its message and never reconstruct a host path.
...
On success, report: `Configuration stored in <config_path>.` The path is the exact user/project scope selected in the browser and returned by the MCP tool.
