...
| Invocation | Action                                                                                                                                                                |
| ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--test`   | `mcp__atlassian__auth_check` with `connection_test: true`; report per-site `connection.success`, `message`, `latency_ms`, and the Jira `user`. No wizard |
| `--reset`  | `mcp__atlassian__setup` with `mode: "new"` — opens the wizard; the configuration is replaced when the user saves                                         |
...
For the no-flag flow, call `mcp__atlassian__auth_check` with `connection_test: true`.
