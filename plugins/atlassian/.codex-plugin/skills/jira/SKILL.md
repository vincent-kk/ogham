...
Resolves a Jira request to an endpoint and an `mcp__atlassian__fetch` call. Parameters: [`mcp-tools.md`](../.shared/mcp-tools.md). Errors: [`error-handling.md`](../.shared/error-handling.md).
...
5. Comments on Server/DC: list with `mcp__atlassian__comment_thread` (applies the site's reply-plugin profile; without one it returns the standard comments plus a `hint` — then run "Thread clues" in `tools/comment/schema.md` once). Cloud: `fetch`.
