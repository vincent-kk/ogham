...
Resolves a Confluence request to an endpoint and an `mcp__atlassian__fetch` call. Parameters: [`mcp-tools.md`](../.shared/mcp-tools.md). Errors: [`error-handling.md`](../.shared/error-handling.md).
...
4. Send page/comment bodies as Markdown with `content_format: "markdown"`; it becomes the storage-format envelope for the deployment. Reading: ask for `body-format=storage` (Cloud) or `expand=body.storage` (DC) and convert with `mcp__atlassian__convert` (`storage` → `markdown`) when the user needs text.
