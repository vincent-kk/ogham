...
| Tool                                          | Purpose                                                                                                               |
| --------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| `mcp__atlassian__fetch`          | HTTP request to a configured site — prefix attachment, deployment rewriting, body/response conversion                 |
| `mcp__atlassian__convert`        | Local conversion: adf↔markdown, storage↔markdown, markdown→wiki, adf↔storage                                          |
| `mcp__atlassian__auth_check`     | Configured sites and optional live connection test — see [`setup`](../setup/SKILL.md#authentication-check)            |
| `mcp__atlassian__setup`          | Browser-based setup wizard — used only by the `setup` skill                                                           |
| `mcp__atlassian__comment_thread` | Jira Server/DC comments with reply-plugin replies merged in — see [`comment schema`](../jira/tools/comment/schema.md) |
