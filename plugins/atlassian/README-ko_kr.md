...
claude --plugin-dir ./plugins/atlassian
```
...
2. **Agent 계층**: 도메인 지식(포맷팅, 워크플로우 규칙, 에러 복구 전략, 멀티모달 장면 분석)을 내장한 도메인 전문가(`jira`, `confluence`, `media`)입니다. 복잡한 다단계 작업을 오케스트레이션합니다.
3. **Skill 계층**: 컨텍스트 창을 작게 유지하기 위해 **지연 참조 로딩(Lazy Reference Loading)**을 사용하는 API 스펙 캡슐(`setup`, `download`, `jira`, `confluence`, `media-analysis`)입니다. 도구 스키마와 참조 문서는 필요할 때만 로드됩니다.
4. **MCP 계층**: 도메인 지식이 없는 유틸리티 계층으로, 범용 HTTP 실행(`fetch`), 양방향 포맷 변환(`convert`), 인증 상태 확인(`auth-check`), 인증 설정(`setup`) 도구를 제공합니다.
...
| [dev/mcp-tools](../../.metadata/atlassian/dev/mcp-tools.md)       | 4개의 핵심 MCP 도구 (`fetch`, `convert`, `auth-check`, `setup`) |
