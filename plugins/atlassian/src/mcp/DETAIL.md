...
- 도메인 어댑터는 입력 검증, 실행 컨텍스트 구성, 도메인 fractal(`src/jira/`)의 단일 entry point 호출, 응답 래핑만 수행한다. 도메인 필드 해석, 원격 응답에 따른 레시피 선택, 병합 규칙, 프로필 수명주기는 포함하지 않는다 — 그것은 `src/jira/` 가 소유한다.
...
- 도메인 어댑터(`tools/jiraCommentThread/`)는 `src/jira/index.js` 의 export 만 호출하며, 병합·판정 규칙을 담은 함수가 어댑터 안에 없다.
...
2026-08-28 — 승인된 도메인 어댑터(`jira_comment_thread`)를 허용하도록 도메인 무지 계약을 재서술했다.
