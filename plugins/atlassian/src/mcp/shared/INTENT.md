...
MCP 툴 응답 포맷 표준화 + fetch 컨텍스트 빌더 등 MCP 계층 공유 유틸리티. 성공/오류 응답 생성, 핸들러 래핑, FetchContext 조립 함수를 제공한다.
...
- `../../core/configManager` — `loadConfig` (buildFetchContext)
- `../../core/authManager` — `getAuthHeader` (buildFetchContext)
- `../../utils` — `resolveSiteConfig` (buildFetchContext)
