...
- 설정은 `core/configManager`, 자격증명은 `core/authManager`에서 로드한다
- 연결 테스트는 `core/connectionTester`의 `testConnection`에 위임한다
...
- `core/configManager` — 서비스 설정 로드
- `core/authManager` — 자격증명 로드
