...
import { renderStorageTable } from "./renderStorageTable.js";
  ...
  switch (tag) {
    ...
    case "ul": {
      const items = node.children
        .filter(
          (child): child is Exclude<HtmlNode, string> =>
    ...
    case "ol": {
      const items = node.children
        ...
        .map(
          (child, index) => `${index + 1}. ${renderStorageNode(child).trim()}`,
        )
        .join("\n");
      return `${items}\n\n`;
    }
    ...
    case "pre": {
      const codeChild = node.children.find(
        (child): child is Exclude<HtmlNode, string> =>
          typeof child !== "string" && child.tag === "code",
      );
      const codeText = codeChild
        ? renderStorageNode(codeChild).replace(/^`|`$/g, "")
        : inner;
      return `\`\`\`\n${codeText}\n\`\`\`\n\n`;
    }
    case "table":
