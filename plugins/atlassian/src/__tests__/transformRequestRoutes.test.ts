...
describe("transformRequest — extended routes", () => {
  describe("basic", () => {
    it("rewrites /pages/{id}/versions → /content/{id}/version", () => {
      expect(
        transformRequest("/pages/1/versions", undefined, "confluence", "v1")
          .endpoint,
    ...
    it("rewrites /pages/{id}/descendants → /content/{id}/descendant/page", () => {
      expect(
        transformRequest("/pages/1/descendants", undefined, "confluence", "v1")
          .endpoint,
    ...
    it("rewrites /users/current → /user/current", () => {
      expect(
        transformRequest("/users/current", undefined, "confluence", "v1")
          .endpoint,
      ).toBe("/user/current");
    });
  });
  ...
  describe("edge", () => {
    it("rewrites /pages/{id}/labels → /content/{id}/label", () => {
      expect(
        transformRequest("/pages/1/labels", undefined, "confluence", "v1")
          .endpoint,
    ...
    it("rewrites /attachments/{id} → /content/{id} (DC delete)", () => {
      expect(
        transformRequest("/attachments/att123", undefined, "confluence", "v1")
          .endpoint,
      ).toBe("/content/att123");
    });
    ...
    it("rewrites /pages/{id}/properties → /content/{id}/property", () => {
      expect(
        transformRequest("/pages/1/properties", undefined, "confluence", "v1")
          .endpoint,
