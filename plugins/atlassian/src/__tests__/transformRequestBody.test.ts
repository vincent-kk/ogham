...
describe("transformRequest — body envelope", () => {
  describe("basic", () => {
    ...
    it("leaves body envelope untouched on Cloud V2", () => {
      ...
      expect(
        transformRequest("/pages", input, "confluence", "v2").body,
      ).toEqual(input);
    });
  });
  ...
  describe("edge", () => {
    it("handles numeric parentId by coercing to string", () => {
      ...
      expect((result.body as Record<string, unknown>).ancestors).toEqual([
        { id: "99" },
      ]);
    });
    ...
    it("does NOT inject container when contentType is page (not comment)", () => {
      ...
      expect((result.body as Record<string, unknown>).pageId).toBe(
        "should-not-convert",
      );
      expect(
        (result.body as Record<string, unknown>).container,
      ).toBeUndefined();
    });
