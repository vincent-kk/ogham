...
import { resolveSiteConfig } from "../utils/siteResolver.js";
...
describe("resolveSiteConfig", () => {
  ...
  it("빈 배열 — 에러", () => {
    expect(() => resolveSiteConfig("jira", [])).toThrow(
      /No jira sites configured/,
    );
  });
  ...
  it("absolute URL endpoint에서 hostname 추출하여 매칭", () => {
    ...
    const result = resolveSiteConfig(
      ...
      sites,
      undefined,
      "https://b.atlassian.net/rest/api/3/issue/PROJ-1",
    );
    expect(result.base_url).toBe("https://b.atlassian.net");
  });
  ...
  it("다중 사이트 + 미지정 — 에러", () => {
    ...
    expect(() => resolveSiteConfig("jira", sites)).toThrow(
      /Multiple jira sites configured/,
    );
  });
  ...
  it("등록되지 않은 baseUrl — 에러", () => {
    const sites = [makeSite("https://a.atlassian.net")];
    expect(() =>
      resolveSiteConfig("jira", sites, "https://unknown.atlassian.net"),
  ...
  it("등록되지 않은 endpoint hostname — 에러", () => {
    ...
    expect(() =>
      resolveSiteConfig(
        ...
        sites,
        undefined,
        "https://unknown.atlassian.net/rest/api/3/issue",
      ),
    ).toThrow(/site not found/);
  });
  ...
  it("Cloud+OnPrem 혼합 — console.warn 출력", () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith(
      expect.stringContaining("mixing Cloud and On-Prem"),
    );
    warnSpy.mockRestore();
  });
  ...
  it("baseUrl 우선순위가 endpoint보다 높음", () => {
    ...
    const result = resolveSiteConfig(
      ...
      "https://b.atlassian.net/rest/api/3/issue",
    );
    expect(result.base_url).toBe("https://a.atlassian.net");
  });
});
