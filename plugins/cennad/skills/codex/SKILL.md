...
Run a Codex CLI conversation off-thread: spawn the `cennad:courier` agent in
the background and relay its report. Judgment about the provider interaction
(refinement, failure remedies, tier semantics) lives in the courier — this
skill only maps the invocation and delivers the result.
