...
describe('resolveArtifactPath', () => {
  it('returns <cwd>/.cennad/artifacts/<sid>-<turn>.md for project mode', () => {
    ...
    expect(out).toBe(join('/tmp/work', '.cennad', 'artifacts', 'sid-123-5.md'));
  });

  it('returns <CENNAD_HOME>/artifacts/<projectHash>/<sid>-<turn>.md for user mode', () => {
    const out = resolveArtifactPath({
      location: 'user',
      cwd: '/tmp/work',
      projectHash: 'deadbeefcafe',
