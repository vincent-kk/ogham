...
- user 모드: `~/.claude/plugins/cennad/artifacts/<projectHash>/` (`CENNAD_CONFIG_PATH` 설정 시 해당 디렉터리 하위)
- 호출자는 `artifacts.enabled` 와 `result.status === 'success'` 를 사전에 확인하고 호출한다 — 작성기는 호출되면 항상 쓰기를 시도한다
