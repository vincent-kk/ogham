...
`open_settings` 도구가 기동하는 로컬 웹 UI 의 프런트엔드 소스. 빌드 시 `scripts/buildSettingsHtml.mjs` 가 CSS·JS 를 inline + minify 해 `public/settings.html` 로 빌드 (런타임에 디스크에서 읽어 서빙, 번들 미포함).

## Structure

| Path                         | Role                                                  |
| ---------------------------- | ----------------------------------------------------- |
| `settings/index.html`        | 단일 페이지 마크업, `__CENNAD_STATE__` 토큰 슬롯 포함 |
...
- 외부 CDN·이미지 의존 금지 — 빌드 시 단일 HTML 파일(`public/settings.html`)로 묶여 디스크에서 서빙
...
- 사용자 입력은 빌드 후 inlined `__CENNAD_STATE__` 만 신뢰 (XSS 방지는 백엔드의 escapeJsonForHtml 책임)
...
- 외부 origin 으로 fetch
- 빌드 산출물 (`public/settings.html`) 을 직접 수정
