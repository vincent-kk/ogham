...
- tier 는 필수 입력 — 호출 측(Claude)이 작업 복잡도로 tier(high/mid/low)를 선택; 코드 fallback 없음
...
- `../../../core/configManager` — `loadConfig` (ratio/preamble/recency_factor/option_flags/artifacts 등 조회; `default_model` 없음 — `tier` 는 필수 입력, `options` 는 항상 `{}`)
