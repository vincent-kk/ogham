...
- 서버 이름은 `tools` 이며 도구 3종(`start_conversation`·`continue_conversation`·`open_settings`)을 stdio 로 노출한다.
...
- 등록 도구가 정확히 3개이며 각 핸들러가 공유 래퍼를 거친다.
