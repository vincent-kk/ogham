...
- **Antigravity sandbox**: `flags.sandbox`(config 기본 false)가 true 면 `--sandbox` 부착, `flags.skip_permissions` 면 `--dangerously-skip-permissions`. 과거 #76 non-TTY 출력 드롭 악화로 미부착했으나 #76 종결·재검증 통과로 복원 — 이력은 [agy-upstream-watch.md](../../.metadata/cennad/agy-upstream-watch.md)
- **Antigravity #76 (구버전 호환 안전망)**: 구버전 `agy -p` 가 non-TTY 긴 응답에서 stdout 을 비결정적으로 드롭(빈 출력, exit 0)하던 결함. agy 1.1.2 에서 종결됐으나 cennad 는 agy 버전을 강제하지 않으므로 구버전 안전망으로 복구 경로를 유지: 빈 stdout 시 `resolveTranscript`→`agyTranscriptStore` 가 `~/.gemini/antigravity-cli/brain/<convId>/.system_generated/logs/transcript.jsonl` 에서 읽기 전용 복구. 비문서화 내부 구조 의존 — agy 업데이트(특히 SQLite 전환) 시 깨질 수 있고, 그때는 명확한 cli_error 로 실패. 업스트림 이슈 상태·재검증 절차: [agy-upstream-watch.md](../../.metadata/cennad/agy-upstream-watch.md)
...
- **model_map 레거시 마이그레이션**: `configManager/utils/mergeModelMap.ts` 의 `mergeTierConfig` 는 구버전 antigravity tier 스키마(`model_map.antigravity.{high,mid,low}` 가 문자열)를 만나면 DEFAULT_CONFIG 로 대체하지 않고 `{model: raw}` 로 마이그레이션한다 (effort 는 undefined).
