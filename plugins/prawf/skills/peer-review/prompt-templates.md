...
`WORKDIR` is resolved by [`[OP: resolve_workdir]`](../.shared/operations/resolve_workdir.md) (`--workdir` > `PRAWF_WORKDIR` > default `./.prawf`). `REVIEW_DIR` = `<WORKDIR>/review/<paper-slug>/`, `PAPER_NORMALIZED` = `<REVIEW_DIR>/paper-normalized.md`.
