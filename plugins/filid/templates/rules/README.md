...
This directory ships rule documentation templates that the `/filid:setup`
...
> is invoked exclusively by the `mcp__plugin_filid_tools__rule_docs_sync` MCP tool from the
> `setup` skill after the user confirms a checkbox selection.
...
4. Run `/filid:setup` on a test project — the new rule should appear
