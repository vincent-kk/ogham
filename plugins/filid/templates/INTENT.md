...
`setup` 스킬이 프로젝트 초기 설정 시 사용하는 템플릿 파일을 포함한다. 훅 설정, 규칙 가이드, 산출물 매니페스트 등 정적 자산을 관리한다.

## Structure

| 경로 | 역할 |
|------|------|
| `deliverables.json` | `setup` 산출물 매니페스트 (생성 파일 목록) |
...
- `deliverables.json`은 `setup`이 참조하는 산출물 목록
...
- 기존 템플릿 파일 경로 변경 (`setup` / `mcp__plugin_filid_t__project_init` 로직 수정 필요)
