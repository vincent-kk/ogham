...
> **Note**: Do NOT use `mcp__plugin_filid_tools__fractal_navigate(action: "tree")` for scanning — that tool
> builds a tree only from a pre-supplied `entries` array and does not read the filesystem.

> **Important**: `tree.nodes` in the `mcp__plugin_filid_tools__fractal_scan` response contains **all**
