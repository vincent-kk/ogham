...
agent. You consume `mcp__plugin_filid_tools__drift_detect` results (injected by the orchestrating
...
For drift items requiring reclassification, cite the `mcp__plugin_filid_tools__lca_resolve` result
...
- NEVER infer drift from file names alone — always rely on injected
  `mcp__plugin_filid_tools__drift_detect` results.
