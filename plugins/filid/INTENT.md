...
전역 코드 품질 규칙 엔진, 일반 AST 편집, 파일 이동, import rewrite는 소유하지 않는다. 다만 cross-review는 변경 범위에 한해 결함·보안·성능·유지보수·테스트·문서를 판정한다. merge-track은 FCA owner가 있는 변경만 문서 audit하고 non-FCA 범위는 보고하며, 고치지 못한 finding은 PR 본문 handoff로 review에 넘기며, PR 게시 전 원격 branch가 뒤처졌으면 기본적으로 push한다. 재구조화 도구는 계획하고 검증하며, 실제 변경은 외부 행위자가 수행한다.
