...
- `resolveGateContext(...)` — owner, delivery key, self-delivery와 gate eligibility를 계산한다.
...
- owner INTENT.md 자체에 대한 Read는 ctx·guide 없이 delivery를 stamp하고, 같은 모듈의 다음 턴 mutation은 deny되지 않는다.
- 같은 턴에 이미 방문한 디렉터리에 새 INTENT.md를 Write해도 delivery가 stamp되어 다음 턴의 첫 code mutation이 deny되지 않는다.
