# preToolValidator -- INTENT.md/DETAIL.md 쓰기 차단

## Purpose

Write/Edit 도구가 `INTENT.md` 또는 `DETAIL.md`를 수정하려 할 때 50줄 제한(INTENT.md)과 append-only 방지(DETAIL.md)를 검사해 위반 시 해당 도구 호출만 `deny` (턴 비중단). `preToolUse` 오케스트레이터가 호출한다.

## Structure

- `preToolValidator.ts` — `validatePreToolUse`, `isDetailMd` re-export
...
- 차단 시 `permissionDecision: 'deny'` + `permissionDecisionReason`에 사유 문자열 설정
