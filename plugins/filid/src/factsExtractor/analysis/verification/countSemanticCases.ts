...
class SemanticCaseCounter {
  ...
  constructor(scanned: ScannedSource) {
    this.source = scanned.source;
    this.tokens = scanned.tokens;
    this.untrusted = scanned.untrusted;
  }
  ...
  private detectUnterminatedLiterals(): void {
    const untrusted = this.untrusted;
...
/**
 * Count the semantic cases one file's text declares.
 *
 * @param source - The file's text.
 * @param scanned - The scan of that text, when the caller already took one;
 * omitted, this reading takes its own.
 * @returns The count, with a reason for every part of the file it could not
 * read as a case.
 */
export function countSemanticCases(
  source: string,
  scanned?: ScannedSource,
): VerificationCaseCount {
  return new SemanticCaseCounter(scanned ?? scanSource(source)).count();
}
