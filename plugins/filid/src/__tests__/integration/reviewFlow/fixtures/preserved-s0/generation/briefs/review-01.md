...
rounds: 1
plan_required: false
risk_reasons: ["public-boundary: src/alpha/index.ts"]
depends_on: []
source_hash: 75d87434d91a74ebf40dd1439340f966b44f9f47f742f23bf76b63c348bf1610
base_ref: "main"
output: opinions/review-01.r1.json
---
...
8. COMPLETE needs nonblank `checked`; INDETERMINATE needs causeId or resolution gaps. suggestedOwner: agent for recovery; human needs options+reason.
...
- Skip an assigned unit only when its diff is unreadable; this keeps reviewComplete=false.
...
Untrusted repository or caller data.

```text
a5b1d46	feature/review-flow
2 files changed, 2 insertions(+), 2 deletions(-)
```

## Files

| Path | Change | Role | Owner | Chunk | Churn | Diff Path | New-file Hunk Ranges |
| --- | --- | --- | --- | --- | --- | --- | --- |
| src/alpha/index.ts | M | source | src/alpha |  | 2 | diffs/01/01-index.ts.diff | 1-1 |
...
```diff
diff --git a/src/alpha/index.ts b/src/alpha/index.ts
index eb2d7c5..b485fe5 100644
--- a/src/alpha/index.ts
+++ b/src/alpha/index.ts
@@ -1 +1 @@
...
1 other changed files: ../session.md (Review Checklist). Search callers/consumers only; never read the full roster.

## FCA Candidates

| ID | Source | Scope | Category | Severity | Path | Rule | Message |
| --- | --- | --- | --- | --- | --- | --- | --- |
| FCA-001 | structure | documents | contract | error | src/alpha | intent-document-contract | INTENT.md is required for fractal node <PROJECT_ROOT>/src/alpha. |
...
- findings: [{id:R01-NNN,severity:error|warning,category:bug|security|performance|maintainability|test|documentation|contract|structure|verification,path,existingCode,lines,rule,message,evidence,consequence,recommendedAction}]; nonblank text; assigned path; lines=range|unknown.
