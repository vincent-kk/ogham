...
import { handleRuleDocsSync } from '../../tools/index.js';

interface RuleDocsSyncToolInput {
  action: (typeof RULE_DOC_ACTIONS)[keyof typeof RULE_DOC_ACTIONS];
  path: string;
  selections?: Record<string, boolean> | string | null;
  resync?: string[] | string | null;
}

type RuleDocsSyncData = ReturnType<typeof handleRuleDocsSync>;
...
export function handleRuleDocsSyncTool(
  input: RuleDocsSyncToolInput,
): ToolPayload<Record<string, string | number>, RuleDocsSyncData> {
  const root = projectRoot(input.path);
  const output = handleRuleDocsSync({ ...input, path: root });
