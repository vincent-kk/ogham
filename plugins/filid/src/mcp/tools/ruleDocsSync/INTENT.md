# ruleDocsSync -- 규칙 문서 동기화 도구

## Purpose

manifest에 등록된 managed rule 문서를 현재 host의 project rule target에 배포·제거하는 MCP 도구. `status`, `manifest`, `sync` 세 action만 가지며 project source와 config는 수정하지 않는다.
