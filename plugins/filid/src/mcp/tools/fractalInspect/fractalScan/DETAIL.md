# fractal_inspect scan action — Filid 1.0 Contract

## Requirements

- `fractal_inspect`의 `scan` action은 등록 adapter로 하나의 `ProjectSnapshot`을 만들고 FCA tree 요약을 반환한다.
...
- `paths` 투영에 `nameFilter`가 있으면 diagnostics도 살아남은 node 범위로 좁히고 제외 건수를 summary에 보고한다. `summary`(detail 생략 포함)와 `full`에서는 nameFilter를 적용하지 않고 전체 diagnostics를 유지한다.
...
- 진입점이 없거나 surface certainty가 하나라도 unsupported/indeterminate인 node는 필드를 생략한다. 모두 exact로 검사되었으나 이름이 없는 node만 빈 배열을 담는다.
...
- `paths` 투영에 필터가 있으면 살아남은 node 밖 경로를 가진 diagnostic을 제외하고 그 수를 `diagnosticsOutOfScope`로 보고한다. 다른 투영 또는 필터가 없는 경우 이 필드가 없고 diagnostics는 그대로다.
...
- 2026-09-05 — 기존 scan payload를 `fractal_inspect`의 `scan` action 뒤 child 계약으로 이동했다.
