...
/** Default bounded wait for one settings-page interaction. */
export const DEFAULT_WAIT_SECONDS = 300;
/** Maximum bounded wait accepted by the settings child. */
export const MAX_WAIT_SECONDS = 600;

/** Input accepted by the settings-session child. */
...
/** Saved, closed, or resumable outcome of one bounded settings wait. */
...
export async function handleOpenSettings(
  ...
  return {
    status: 'pending',
    url: server.url,
    message: `No submission within ${wait}s. The page is still open — call project_setup with action "settings" again to keep waiting, or proceed with the existing config.`,
  };
}
