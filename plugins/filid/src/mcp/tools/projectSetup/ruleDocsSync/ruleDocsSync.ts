 ...
 * interactive path now runs through the `project_setup` settings action.
...
/** Child action values understood by managed rule synchronization. */
export type RuleDocsAction =
  (typeof RULE_DOC_ACTIONS)[keyof typeof RULE_DOC_ACTIONS];

/** Input accepted by the managed rule-document child. */
...
/** Action-specific output returned by the managed rule-document child. */
...
/**
 * Handle the child rule-document actions behind `project_setup`.
