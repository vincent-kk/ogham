...
- `uncertain`인 파일은 **언제나 이유 목록 중 하나에 나타난다**: `rejected`(거부된 claim), `indeterminate`(공급자가 보증하지 못한 참조), `unadjudicated`(열린 항목), `pendingAttestations`(확인 대기), `awaitingComparison`(판정이 손상으로 폐기돼 아직 아무도 다시 도출하지 않은 파일). 이유 없이 `uncertain`으로만 실리는 파일은 다음 행동이 없는 정지이므로, 그 다섯의 합집합이 `uncertain`을 덮는지 테스트가 고정한다.
