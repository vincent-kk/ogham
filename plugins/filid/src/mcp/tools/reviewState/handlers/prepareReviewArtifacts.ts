...
import { stabilizeReviewGroupIds } from '../group/stabilizeReviewGroupIds.js';
...
export async function prepareReviewArtifacts(
  ...
  const paths =
    staging?.paths ??
    resolveReviewStatePaths(input.projectRoot, input.branchName);
  ...
  let state: ReviewStateRecord = {
    ...
    scope: {
      ...
      dirtyPathsHash: collected.dirtyPathsHash,
