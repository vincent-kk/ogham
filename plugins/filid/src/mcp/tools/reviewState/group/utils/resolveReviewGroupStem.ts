/** Locale tag with an ISO language and an optional two-letter region. */
const LOCALE_SUFFIX_PATTERN = /^([a-z]{2})(?:[-_]([a-z]{2}))?$/i;

/** Companion-file suffix removed before grouping related source files. */
const COMPANION_SUFFIX_PATTERN = /\.(?:spec|test|stories)$/i;
...
function isLocaleSuffix(suffix: string): boolean {
  const match = LOCALE_SUFFIX_PATTERN.exec(suffix);
  return Boolean(
...
export function resolveReviewGroupStem(basename: string): string {
  ...
  while (stem !== previousStem) {
    previousStem = stem;
    stem = stem.replace(COMPANION_SUFFIX_PATTERN, '');
