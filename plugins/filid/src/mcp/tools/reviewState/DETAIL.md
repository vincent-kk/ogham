...
- `handoff`는 prepare와 같은 규칙으로 base와 설정을 해석하고 같은 snapshot의 changed-scope finding을 골라 분류·상한을 적용한 `## FCA Handoff` 섹션을 `<reviewDirectory>/handoff.md`에 쓴다. caller `entries`의 `note`·`ruleId`·`path`는 도구가 상한에 맞춰 자른다. `Counts:`와 응답 `data.counts`는 machine block 상한 전에 완전한 handoff를 집계하며, 리뷰 범위 안의 scan-level non-finding 진단은 envelope에 유지하면서 `indeterminate` claim으로도 기록한다. 범위는 prepare와 같은 규칙으로 정한다. state와 `evidence.md`는 쓰지 않고, 렌더 결과를 `parseHandoffBlock`으로 자체 검증한다. validation 실패는 `snapshotHash: null`, `documentSync: failed`, `handoff-validate` 합성 항목으로 기록한다. 그 catch는 **렌더·자체 검증 실패**를 위한 것이고, 공유 관문이 던지는 `ToolDiagnosticError`는 삼키지 않는다: code·message·nextAction을 그대로 응답 diagnostics에 실어 `documentSync: failed`의 이유가 코드로 식별되게 한다. 그러지 않으면 `facts-incomplete`가 `status: ok`와 진단 없는 합성 항목 하나로 바뀌어, 응답을 읽는 쪽이 무엇을 하면 되는지 알 수 없다(P5). handoff 전체를 거부로 바꾸지는 않는다 — 문서 동기화 실패는 막는 것이 아니라 보고하는 것이다.
...
- **사실 관문(스펙 §9).** prepare와 handoff가 공유하는 `computeChangedScopeEvidence`에서, 리뷰 범위의 범위 안 파일이 모두 `exact` 또는 `tool-error`가 아니면 `facts-incomplete`로 거절하고 그 파일 목록과 부트스트랩을 다음 행동으로 돌려준다. 프로젝트 전체가 아니라 **리뷰 범위**를 본다 — 무관한 파일의 `missing` 하나가 리뷰를 막으면 관련성 거름망의 취지가 사라진다. 범위 밖(`unsupported`) 파일은 관문의 대상이 아니다. 관문을 공통 지점에 두는 이유는 새 호출자가 생겨도 빠지지 않게 하기 위해서다.
- **digest 입력의 정렬은 locale에 기대지 않는다.** 동결 항목과 그 안의 참조·판정은 raw UTF-8 byte 순으로
  정렬한다(`lib/compareByBytes.ts`). `localeCompare`는 ICU collation을 읽어 기계마다 다를 수 있고, 같은
  byte를 담은 저장소가 두 기계에서 다른 digest를 내면 `review-inputs-stale`이 이유 없이 뜬다.
- **이 버전으로 올린 뒤의 첫 prepare는 새 generation을 만든다.** 동결 항목의 모양(상태 필드)과 정렬이
  바뀌어 `facts.json`이 `scope.factsDigest`와 어긋나고, `hasCompletePreparedArtifacts`가 그 generation을
  불완전으로 본다. 검증된 의견 1회를 버리는 **의도된 일회성 전환 비용**이다. 진행 중이던 validate·seal의
  안전은 `preserved-s0` fixture가 지킨다 — 동결 이전 generation은 `factsDigest`가 없어 이 판정에
  걸리지 않는다.
- **관문은 사실을 읽는 자리에만 둔다.** `checkpoint`는 참조 기반 증거를 전혀 읽지 않는다 — 비교하는 것은
  커밋된 소스 동일성(`sourceHash`)과 artifact 존재·freshness뿐이다. 그래서 checkpoint에 facts 관문을 두면
  쓰지도 않는 증거를 검사하게 된다. 리뷰 도중의 fix 커밋은 이 사슬로 막힌다: checkpoint가
  `review-source-hash-stale`(INDETERMINATE), seal도 같은 코드로 거부해 평결을 내지 않고, 그 next action인
  prepare가 **facts 관문**에 걸려 `facts-incomplete`로 바뀐 파일의 사실을 요구한다. 부트스트랩 뒤 prepare →
  seal이 통과한다. 조용한 pass가 없다는 것을 end-to-end 테스트가 고정한다.
- **사실 동결(스펙 §9).** prepare는 리뷰 범위(변경 파일 + 그래프 이웃) 파일의 정규화된 유효 참조와 적용된 판정을 generation의 `facts.json`에 쓰고, 그 digest를 `scope.factsDigest`에, 파일별 참조 목록의 digest를 `scope.files[].factsHash`에 적는다. 유효 참조는 레코드의 참조와 `adopt`된 항목의 합이고, 프로젝트 안으로 해석된 것만 싣는다 — `external`·`unresolved` 표기 차이로 리뷰가 무효화되지 않게 한다. **facts 범위 안 파일은 간선이 없어도 빈 목록으로 실린다**: "동결하지 않았다"와 "동결했는데 간선이 0이었다"는 다른 사실이고, 구별하지 못하면 나중에 `adopt`로 간선이 생긴 파일이 seal의 대조를 조용히 지나간다. lockfile처럼 facts 범위 밖인 파일은 참조 사실의 대상이 아니므로 항목도 `factsHash`도 없다 — `facts.json`에 없다는 것은 **리뷰 범위 밖이거나 facts 범위 밖**이고, 뒤쪽은 그 파일의 상태 `unsupported`가 가른다.
  - `facts.json`은 prepare가 소유하는 artifact이고 **generation 디렉터리에 산다** — `evidence.md`와 같은 수명이다. generation 하나는 "한 번의 prepare가 본 모든 것"이고, branch에 하나뿐이면 새 prepare가 이전 동결본을 덮어써 옛 generation의 brief로 들어온 `compare`·validate가 남의 동결본을 읽는다. 경로는 `resolveReviewGenerationPaths`가 정하고 쓰는 쪽(`collectChangedScopeEvidence`)과 읽는 쪽(`readFrozenFacts`·`hasCompletePreparedArtifacts`·seal·`readGenerationFrozenFacts`)이 모두 그 값을 받는다. 디렉터리 규칙은 `resolveGenerationDirectory` 한 곳에 있다 — 경로를 직접 조립하는 자리가 이 결함을 만들었다.
  - 없거나 digest와 어긋나면 `hasCompletePreparedArtifacts`가 거짓이 되어 generation을 다시 만든다. `evidence.md`만 보고 재개하던 지름길도 같은 판정을 거친다 — 그러지 않으면 seal이 거부하는 generation을 prepare가 그대로 돌려주어 "prepare 한 번"이 상태를 바꾸지 못한다(P5).
  - `evidenceHash` 튜플에는 **그 파일에 `factsHash`가 있을 때만** 투영이 들어간다. 동결 이전에 prepare된 generation은 `[candidates, claims]` 그대로여서 stale이 되지 않는다(G10이 지킨다).
  - 두 새 필드는 모두 선택 필드이고, 영속 스키마에서도 optional이다.
- **seal의 fold 앞 거부(스펙 §9).** 동결된 사실이 있는 generation에 한해, phase가 `sealed`가 되기 **전에만** live 부속 표를 읽어 두 가지를 본다: (a) 동결된 파일에 아무도 판정하지 않은(또는 `pending-dismiss`) 항목이 있는가, (b) 그 파일의 **현재 유효 참조 집합이 동결된 집합과 다른가** — 어느 방향이든. 간선이 늘었으면 리뷰가 보지 않은 간선이고, 줄었으면 리뷰가 이제 없는 것 위에서 판단한 것이며 부재를 요구하는 결론이 유령 간선에 가려진다. 한 방향만 보면 확정된 `dismiss`나 재제출로 사라진 간선이 조용히 봉인된다. 하나라도 있으면 `facts-discrepancy` 진단(`owner: agent`, 파일·줄)을 담은 `INDETERMINATE` payload를 돌려주고 sealed state를 쓰지 않으며 phase를 바꾸지 않는다. throw가 아니라 payload인 이유는 throw가 `toolError`로 변해 파일·줄을 잃기 때문이다.
  - 두 다음 행동이 다르다: (a)는 `adjudicate` 또는 재제출이고 확정되면 **재리뷰 없이** 풀린다. (b)는 prepare이고(`facts-discrepancy-frozen-edges-changed`), 유효 참조가 바뀐 파일만 다시 리뷰된다. 새 generation은 지금의 집합을 동결하므로 같은 거부가 반복되지 않는다.
  - **동결이 `exact`가 아니라고 기록한 파일은 대조하지 않는다.** 그 파일의 동결 참조가 빈 것은 "간선이 없다"가 아니라 "아무 도구도 읽지 못했다"여서이고(사실 관문은 `tool-error`를 통과시킨다), 대조하면 새 generation도 같은 빈 목록을 동결해 영원히 거부된다(P5). 그 파일은 이미 `facts-tool-error` 후보로 리뷰에 올라간다. 그래서 동결 항목이 동결 시점의 상태를 함께 싣는다.
  - **지금도 `exact`가 아니면 그대로 통과한다 — 미판정 항목이 있어도 마찬가지다.** 대조가 지키는 것은 "리뷰가 판단에 쓴 간선이 바뀌었는가"이고, 동결 시점에 non-exact였던 파일은 리뷰에 간선을 하나도 주지 않았다(unknown으로 올라갔다). 지금도 non-exact면 여전히 하나도 주지 않으므로 리뷰가 선 증거는 그대로다. 지금 `exact`일 때만 리뷰가 갖지 못한 지식이 생긴 것이고, 그때 거부한다. 반대로 하면 `tool-error`로 동결된 파일에 항목 하나가 열릴 때마다 seal이 막히는데, 그 항목이 만료되어 판정할 수 없는 경우 빠져나갈 길이 없다(P5).
  - `facts.json`이 없거나 `scope.factsDigest`와 다르게 해시되면 대조를 **건너뛰지 않고** `review-facts-frozen-unusable`로 거부한다(phase 불변, sealed state 없음). digest가 있다는 것이 동결이 있었다는 증거이므로, 그 파일이 digest와 어긋나면 이 generation의 동결이 아니다. `hasCompletePreparedArtifacts`가 같은 판정을 써서 다음 prepare가 그 generation을 재사용하지 않게 한다 — 그래야 "prepare 한 번"이 상태를 바꾸는 다음 행동이 된다.
  - 이 읽기는 `evidenceHash`에 들어가지 않으므로 `review-inputs-stale`을 만들지 않는다. checkpoint에는 이 거부를 두지 않는다 — 호출자가 resolve·revalidate뿐이고 둘 다 봉인 뒤의 흐름이다.
  - seal은 자기가 읽은 항목(key·상태·`lineDigest`)과 그 digest를 sealed state의 선택 필드 `factsAdjudications`·`factsAdjudicationsDigest`에 적는다. 봉인 뒤의 흐름은 live 표를 다시 읽지 않는다 — 다른 흐름이 같은 파일에 올린 항목이 끝난 리뷰를 다시 열지 않는다.
- **변경 파일의 `tool-error`는 막지 않고 finding이 된다.** 추출기가 읽지 못한 변경 파일은 리뷰가 다뤄야 할 대상이지 준비를 막을 이유가 아니다. 후보의 `rule`은 `facts-tool-error`이고 rule roster에는 들어가지 않는다 — 규칙 엔진이 평가하는 규칙이 아니라 사실 상태의 보고다.
- **리뷰 범위에 facts 범위 밖 파일이 있으면 그 사실을 싣는다.** 선언된 `facts.covers`가 기본 범위(어댑터 소스 확장자)의 파일을 빼면 참조 기반 규칙은 그 파일을 건너뛰는데, 지금까지 리뷰는 그 사실을 어디에도 적지 않고 APPROVED가 됐다 — `unsupported`가 조용히 pass로 읽히는 자리다(스펙 §3: 건너뛰되 보고한다). 그런 파일이 리뷰 범위에 하나라도 있으면 `review-files-outside-facts-scope`를 **`affects: []`**로 싣는다: 개수와 상한 있는 경로 목록, 다음 행동은 `facts.covers`에 넣고 부트스트랩한 뒤 prepare. 범위는 프로젝트의 선언이므로 **막지 않는다** — verdict도 blocker도 바뀌지 않는다. 0개면 아무것도 싣지 않는다(상시 문장 금지).
- **변경 집합 밖의 verification 파일에 귀속된 진단도 같은 자리로 간다.** 아무도 건드리지 않은 테스트 파일의 풀리지 않는 import 하나가 의존성 축을 indeterminate로 만들면, 그 파일에 책임이 없는 리뷰가 전부 INCONCLUSIVE가 된다 — 고칠 수도 없고 넘어갈 수도 없다. 예외는 **그 참조가 변경된 파일의 이름을 적는 경우**다. 그때는 리뷰 중인 변경이 그것을 깨뜨린 것이므로 진단이 범위 안에 남는다. 이름 판정은 관련성 거름망과 같은 규칙(`collectTargetNames`·`containsPathToken`)이라, 이름을 적지 않는 참조는 거기서와 똑같이 빠져나간다.
  - **선언된 한계 둘.** (i) 참조가 변경 파일의 이름을 **적지 않는** 철자 — 해석 표(`paths`·`baseUrl`)가 펼치는 alias, 이름을 싣지 않는 star 재export — 는 범위 밖으로 간다. 이름 판정은 문자열이 무엇을 적었는지만 보고 해석을 흉내 내지 않기 때문이다(P1). (ii) 변경 파일의 이름이 흔하면(`utils`, `types`, `index`가 아닌 디렉터리 이름) 같은 이름을 적은 무관한 테스트의 진단이 범위 **안**에 남는다. 두 방향 중 실패는 "더 남긴다" 쪽으로만 나게 두었다 — 남는 진단은 리뷰어가 한 번 보고 넘기지만, 빠뜨린 진단은 그 위에 봉인된다.
...
- prepare는 기존 generation을 보존한다. force는 새 generation에서 모든 대상 파일을 다시 검토한다. 같은 입력의 prepared generation은 resumable이고 같은 입력의 sealed generation과 report가 함께 있으면 cached다. 쓸 수 없는 state(파일별 입력 기록이 없는 legacy, schema mismatch, malformed, 구형 validation policy)는 force 없이도 prepare가 교체한다: 현재 bytes를 같은 디렉터리의 `replaced-state-<순번>.json`으로 옮기고, state가 없던 것처럼 새 generation을 시작하며, affects 없는 `review-state-replaced` 진단(사유 code와 보관 경로)을 싣는다. 디렉터리는 지우지 않으며 삭제는 `cleanup`뿐이다.
- resumable에서 `evidence.md`와 완전한 state가 있으면 선별·청킹·그룹화를 되살리고 누락 artifact를 복구한다. effective effort는 최초 prepare부터 고정하며, 누락 artifact 복구에서도 기존 round 상한과 미완료 검토를 보존한다. evidence가 없으면 범위 산출부터 다시 만든다. `recoverReviewGroups`는 병합 opinion 누락·hash 불일치 시 raw round r1…rK가 모두 있으면 round 1부터 K까지 순차 재검증해 병합 opinion을 재구성한다. 같은 바이트로 복원되면 기존 verify의 reviewSha256 결합을 유지하고, 달라지면 verify invalid 복구가 이를 지운다. raw round가 하나라도 없으면 review·verify validation을 지우고 병합 opinion을 삭제하며 r1 skeleton을 다시 쓴다. 복구가 병합 opinion을 지울 때 r1보다 큰 모든 round 파일도 그 자리에서 함께 지운다. validate의 직전 round validation 순서 검사는 유지한다. 미완료 review는 다음 round skeleton을, 완료 review는 새 finding과 이전 미해결 finding이 모두 없으면 auto-verify를, 있으면 누락 verify brief를 만든다. verify 파일·hash·reviewSha256 결합이 깨지면 verify validation을 지우고 같은 복구를 적용한다. 복구와 state 저장을 끝낸 뒤 artifact를 재관측한다.
...
- 모든 action의 projectRoot는 저장소 안의 절대 경로이며 Git toplevel로 정규화한다. branchName은 선택 입력으로, 문자열이면 기존 branch 검증을 적용하고 생략하면 Git 현재 branch를 해석한다. branchName 없이 detached HEAD이면 action 범위에서 캐시되는 `rev-parse HEAD`의 앞 12자로 `detached-<sha12>`를 branch key로 쓴다(CI의 기본 상태). 같은 HEAD면 같은 key다. directory key는 원래 branch 문자열을 정규화하고 응답 branchName은 원래 문자열을 보존한다.
...
- validate input은 `{ action: "validate", projectRoot, branchName?, kind, group, round?, generationId? }`이다. `generationId`를 주고 활성 generation과 다르면 opinion을 병합하지 않고 `review-generation-superseded`로 거부한다. review kind는 범위 안의 round가 필수이고 verify kind는 round를 금지한다. group은 `^\d{2,}$`이며 state에 존재해야 한다.
...
- state 파일이 없으면 `missing`, schema version이 2가 아니면 schema mismatch, v2 구조가 malformed이거나 group ID에서 유도한 canonical artifact path와 다르면 `STATE_INVALID` error다. prepare는 이 판정들을 error로 올리지 않고 state를 보관 후 교체하며, 경로 해석도 state를 읽기 전에 branch 경로로 시작해 malformed state에서도 진행한다. 교체 사실은 새 레코드의 `replacedFrom`(사유, 보관 경로, 이전 generation·verdict, 폐기한 그룹, 사슬)에 남고 sealed 보고서·PR comment·seal 응답이 그것을 싣는다. state 파일이 없는데 generation이나 봉인 보고서가 남아 있으면 사유 `review-state-missing`으로 같은 기록을 남긴다. **선언된 한계**: 이 선택 필드가 든 state를 이전 버전 filid가 읽으면 schema가 `.strict()`라 `review-state-invalid`가 된다(다운그레이드는 지원하지 않는다).
...
- group diff는 `diffs/<group>/<ordinal>-<basename>[.<k>-of-<n>].diff`로 materialize한다. review brief frontmatter는 group·rounds·plan-required·dependency·source hash·base ref·output을, 본문은 reviewer method verbatim(호스트별 도구 이름을 담지 않는다), Change Context, FCA Handoff(유효한 블록이 있을 때만), Files, Diffs, Prior Opinions, Other Changed Files(group 밖 파일 개수와 session checklist 참조만, 없으면 none), FCA Candidates, Repository Rules, Rules, Output Contract 순서다.
- review brief의 Prior Opinions는 dependsOn group만 열거한다. 현재 group의 이전 병합 opinion 경로는 handoff의 priorOpinionPath가 전달한다.
- reviewer Output Contract는 handoff output path에 미리 쓴 JSON skeleton의 key·배정 unit을 정본으로 사용하고 완전한 예시를 반복하지 않되 `checked`의 nonempty string array와 `gaps[].resolution`의 optional 중첩 shape·스키마 길이·enum 제약은 문자 그대로 싣는다. verifier는 미리 쓴 skeleton이 없으므로 brief 안의 최소 JSON shape와 배정 finding ID를 사용한다. method·rule·diff·source 추적과 JSON validation은 축소하지 않는다.
...
- verify brief는 group·source hash·output frontmatter, verifier method의 Deliverable부터 끝까지(Codex 호스트에서 실행된 서버는 그 안의 Filid 도구 이름만 Codex 이름으로 바꾸고, method 해시는 정본 바이트로 계산한다), Files, Diffs, assigned finding만의 Decisions Required, Output Contract를 가진다. Re-verification Mode와 Prior Verifier Guidance 절은 포함하지 않으며 FCA candidate ID 행도 넣지 않는다. FCA-1 rule을 인용한 reviewer finding은 assigned에 남는다.
...
- handoff action은 동일 snapshot의 changed-scope finding과 caller 항목을 정렬·상한 처리한 뒤 파서 왕복 검증을 통과한 섹션만 `handoff.md`에 기록한다. 계산 중에도 state와 `evidence.md`는 생성하거나 변경하지 않는다.
...
- fresh state는 `validationPolicyVersion: 2`를 기록한다. 같은 identity의 구형·미지원 정책을 checkpoint, validate, seal에서 재사용하면 `review-validation-policy-outdated` error만 반환하고 verdict·handoff를 노출하거나 artifact를 변경하지 않는다. prepare는 그 state를 보관 후 교체한다. checkpoint·validate·seal의 next action은 "in-flight actor가 끝난 뒤 force 없이 prepare를 한 번 부르고 그 응답에서 이어 간다"이며, 같은 run에서 두 번째면 종료한다. 자동 force하지 않으며 cleanup은 유지한다.
...
- **Consumers**: `**/plugins/filid/src/__tests__/unit/mcp/reviewState/helpers/buildReviewHandoffFixture.ts`, `**/plugins/filid/src/__tests__/unit/mcp/reviewState/helpers/buildReviewOpinion.ts`, `**/plugins/filid/src/__tests__/unit/mcp/reviewState/helpers/buildVerifyOpinion.ts`, `**/plugins/filid/src/__tests__/unit/mcp/reviewState/helpers/completeIncrementalReview.ts`, `**/plugins/filid/src/__tests__/unit/mcp/reviewState/helpers/loadBaselineFoldInput.ts`, `**/plugins/filid/src/__tests__/unit/mcp/reviewState/helpers/prepareCandidateOnlyReviewState.ts`, `**/plugins/filid/src/__tests__/unit/mcp/reviewState/helpers/prepareReviewBlockerFixture.ts`, `**/plugins/filid/src/__tests__/unit/mcp/reviewState/helpers/prepareReviewStateFixture.ts`, `**/plugins/filid/src/__tests__/unit/mcp/reviewState/helpers/prepareReviewStateSealFixture.ts`, `**/plugins/filid/src/__tests__/unit/mcp/reviewState/helpers/prepareWithFacts.ts`, `**/plugins/filid/src/__tests__/unit/mcp/reviewState/helpers/readPersistedReviewState.ts`, `**/plugins/filid/src/__tests__/unit/mcp/reviewState/helpers/readPreparedReviewState.ts`, `**/plugins/filid/src/__tests__/unit/mcp/reviewState/helpers/sealReviewStateFixtureAndAssert.ts`, `**/plugins/filid/src/__tests__/unit/mcp/reviewState/helpers/validateAutoReviewStateSealGroup.ts`, `**/plugins/filid/src/__tests__/unit/mcp/reviewState/helpers/validatePreparedReviewState.ts`, `**/plugins/filid/src/__tests__/unit/mcp/reviewState/helpers/validateReviewStateSealGroup.ts`, `**/plugins/filid/src/__tests__/unit/mcp/reviewState/helpers/writeReviewStateFixtureJson.ts`, `**/plugins/filid/src/__tests__/unit/mcp/scope/helpers/createReviewScopeFileFixture.ts`, `**/plugins/filid/src/__tests__/integration/reviewFlow/helpers/editPersistedReviewState.ts`, `**/plugins/filid/src/__tests__/integration/reviewFlow/helpers/normalizeReviewStateText.ts`
- **Direct import**: `allowed`
- **Reason**: These exact test fixture builders construct and inspect internal review artifacts across integration and unit suites. The runtime entry point cannot expose these implementation types, parsers and state-path helpers without enlarging its public contract solely for verification. Keeping the fixtures with their verification consumers avoids moving production review internals into the source root; only the listed consumers may import this organ directly.
