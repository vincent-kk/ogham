...
- **postcondition은 더 조인다(스펙 §5).** "새 순환 없음"과 "새 경계 위반 없음"은 부재를 요구하는 결론이므로 **프로젝트 범위에 `unknownFiles`가 하나도 없을 때만** 단정한다. 관련성 거름망은 여전히 "어떤 unknown이 이 계획을 막는가"를 가르는 데 쓰지만(`relevant`/`other` 구분은 그대로 응답에 실린다), 부재의 단정에는 쓰지 않는다 — 이름을 담지 않는 alias·star import가 거름망을 빠져나가므로 무관 판정은 증명이 아니다. unknown이 하나라도 있으면 status는 `indeterminate`이고 다음 행동은 **그 파일들의 사실 제출**이다. S3c 전에는 사실을 에이전트가 채울 수 없어 "알려진 간선 위에서 검증됨"으로 통과시켰지만, 이제는 막아도 에이전트가 스스로 풀 수 있다.
