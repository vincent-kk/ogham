...
- **`config_scope` 토글이 페이지 전역을 정한다** — config 저장 레이어와 rule 배포 채널을 같은 선택이 정해 `/save` 의 `scope` 하나로 간다. 서버가 실은 `configByScope` 에서 그 레이어의 문서를 골라 폼을 다시 채우고 저장 문서도 거기서 출발한다. `data-config-path`·`data-scope-state` 계약 정본은 `@ogham/cross-platform` 의 `DETAIL.md` "설정 페이지 계약"
- 서버가 두 레이어의 rule 스냅샷과 채널 경로를 함께 싣고, 토글이 움직이면 목록과 `data-rules-target` 을 다시 그린다 — 페이지는 채널 경로를 조립하지 않으며(Codex 채널은 `AGENTS.md` 의 소유 섹션), 체크박스가 없어도 섹션을 감추지 않는다
