 ...
 * so an edge the new record stops claiming outright is still reported. Without
 ...
 * `tool-error` instead of manufacturing items.
 *
...
export function detectShrunkReferences(input: ShrinkInput): ShrunkReference[] {
  const { previous, accepted, sameEpoch } = input;
  ...
  const reResolved = !sameEpoch && !toolChanged;
  const now = new Map(
  ...
  return previous.references.flatMap((reference): ShrunkReference[] => {
