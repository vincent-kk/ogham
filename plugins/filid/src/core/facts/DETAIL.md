...
- 간선을 **더하는** 판정은 한 actor로 확정하고, **버리는** 판정은 다른 actor의 확인을 받아야 확정한다. 의견이 갈리면 간선을 남긴다. 틀린 판정은 언제든 다시 판정할 수 있어 고칠 수 없는 요구로 굳지 않는다(P5).
- 유효 참조 = 제출된 레코드 ∪ adopt된 참조. adopt는 레코드의 참조를 가리지 않고 더하기만 한다.

- **contract group id도 §4.2의 문자열 존재 검사를 지난다.** 레코드가 주장하는 것은 "이 파일에 `filid:contract <id>`가 있다"이므로, **id 하나가 아니라 marker를 센다** — 산문이나 문자열 안에 id만 있는 것은 본 적 없는 것이다. 주석 안인지는 확인하지 않는다(그것이 파싱이고, 문자열 리터럴 안의 marker는 §4.5와 같은 선언된 한계다). 대조는 제출된 id로 정규식을 만들지 않고 `indexOf`로 훑는다 — 제출 문자열이 matcher 자체가 되는 길을 열지 않는다. 없는 id는 그 항목만 `facts-contract-group-absent`로 거부되고 레코드는 남는다. 추출기의 marker 문법(`[A-Za-z][A-Za-z0-9._-]*`)·길이·개수 상한은 스키마가 먼저 거르므로, 어떤 줄도 담을 수 없는 id는 줄을 읽기 전에 `facts-record-schema-invalid`가 된다.
...
- 패턴은 `FACTS_REFERENCE_LINE_PATTERNS`(ECMAScript 기본값) **∪** `FACTS_REFERENCE_FALLBACK_PATTERN`(범용)이다. conventions pack(§7)이 언어별 패턴의 정본이 될 자리이고, 그때까지는 둘의 합집합을 쓴다 — pack이 붙으면 놓치는 줄이 줄어드는 방향으로만 바뀐다. 기본값의 정본은 추출 프로그램의 `HIDDEN_REFERENCE_PATTERNS`이고, 서버의 이 복사본은 그 패턴이 보는 줄을 **빠짐없이 덮어야** 한다 — 덮지 못하면 추출기가 보고한 참조가 설명 없는 줄로 남는다. 두 벌이 갈라지지 않게 하는 패리티 테스트가 그것을 지킨다. 서버가 복사본을 갖는 이유는 줄 계정이 추출기 없는 자리에서 돌기 때문이다.
...
- 같은 파일에 **tool 등급 레코드가 저장되면 pending을 폐기한다.** 남겨 두면 나중의 두 번째 attested 제출이 아무도 다시 읽지 않은 주장을 되살려 tool 레코드를 덮는다.
