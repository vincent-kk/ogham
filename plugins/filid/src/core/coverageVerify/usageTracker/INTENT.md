# usageTracker -- 프랙탈 서브트리 내 타겟 모듈 사용처 추적

## Purpose

`FractalTree`를 순회해 지정된 서브트리(`subtreeRoot`) 내에서 타겟 모듈(`targetPath`)을 런타임 import하는 모든 peer 파일을 수집하여 `UsageSite[]`로 반환한다.
...
- import 경로 해석은 `../importResolver`에만 위임
