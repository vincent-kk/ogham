# importResolver -- import 소스를 절대 파일 경로로 해석

## Purpose

`extractDependencies`가 추출한 raw import source 문자열을 현재 파일 기준 절대 경로로 해석한다. `usageTracker` 내부 전용 유틸.
...
- 파일 내용 파싱 또는 AST 분석
- 네트워크·외부 프로세스 호출
- 상위 모듈(`coverageVerify`, `usageTracker`)에서 직접 import되는 공개 API로 재분류
