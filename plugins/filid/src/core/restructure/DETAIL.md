...
- specifier가 resolved file을 가리키는지는 **마지막 세그먼트의 확장자를 제거한 형태**로 판정한다. 소스 확장자를 그대로 적을 수 없는 생태계 관례(TypeScript ESM이 `.ts` 파일을 `.js`로 참조, 확장자 생략)를 exact evidence로 인정하기 위해서다.
...
- **import 수정은 edge마다 정확히 한 move가 싣는다.** 소유권은 실행 가능한 move만으로 정한다.
  ...
  - `alreadyPlaced`와 `unresolved`의 `affectedImports`, `delegatedImports`, `preservedImports`는 비어 있다.
  - 판정은 `sourcePath ≠ targetPath`인 instruction에만 한다. 이미 제자리인 unit은 import가 깨지지 않는다.
  - 나가는 쪽에서는 파일 참조와 디렉터리 참조(대상의 엄밀한 상위 디렉터리를 가리키는 specifier)를 rewrite한다. 대상이 움직이지 않으므로 디렉터리도 움직이지 않는다.
  - 절대 경로 specifier는 소비자만 움직일 때 깨지지 않으므로 나가는 쪽에서 판정하지 않는다.
- **filid가 다시 쓸 수 없는 import는 `delegatedImports`로 호출자에게 넘긴다.** 대상은 stem이 다른 들어오는 참조(디렉터리 index, bare `.`/`..`)와 파일·디렉터리 참조가 아닌 나가는 참조다. import는 계획의 분할에 영향을 주지 않고, 소유권·정렬 규칙은 rewrite와 같다. `requiredResolvedPath`는 모든 move 뒤 그 import가 불러야 할 파일이다.
  - 소비자에서 대상까지의 상대 경로가 이동 뒤에도 같으면 위임하지 않고 `preservedImports`에 싣는다(디렉터리 move 안쪽끼리의 참조).
...
- validation은 post-execution snapshot만으로 판정한다.
  ...
  - 위임·보존 import는 해석 결과로 검사한다: 소비자의 해당 참조가 `requiredResolvedPath`로 해석되고, 그 specifier의 `unresolved-local-dependency` 진단이 없어야 한다. 보존 import의 specifier가 바뀌었으면 위임 import와 같이 소비자의 어떤 참조가 `requiredResolvedPath`로 해석되는지로 판정한다. 어기면 `delegated-import-missing` 또는 `preserved-import-broken`이다. 검증 파일은 그래프 certainty에서 면제되므로 진단 검사가 깨진 옛 import를 드러낸다.
...
- `planMoveInstruction(snapshot, request, orderedMoves?): MoveInstruction` — 한 request의 normalized source/target, basis, LCA, decision 상태와 `decisions`를 계산한다. `orderedMoves`는 rewrite·위임의 최종 경로와 소유권에만 쓰고 target 계산에는 쓰지 않는다.
...
- `buildImportRewrites(snapshot, unit, orderedMoves?): ImportRewriteBuildResult` — `unit`은 `{ sourcePath, targetPath, rewriteTargetPath }`다. 이 unit이 소유한 `rewrites`, `delegated`, `preserved`를 반환한다. `orderedMoves`가 비면 unit 혼자 실행되는 것으로 본다.
...
- `validatePlanPreconditions(snapshot, plan): PlanValidationResult` — project root, snapshot hash 불일치와 unresolved 요청을 finding으로 반환.
- `validatePlanPostconditions(snapshot, plan): PlanValidationResult` — `moves`와 `alreadyPlaced` 양쪽의 최종 경로, 위임·보존 import의 해석 결과, 그리고 post snapshot의 boundary·DAG 불일치를 finding으로 반환.
- `validateDelegatedImports(snapshot, move): PlanValidationFinding[]` — 한 move의 위임·보존 import를 post snapshot의 해석 결과로 검사한다.
...
- `.js`로 참조된 `.ts` 소스와 확장자를 생략한 참조는 exact evidence이며 `affectedImports`를 만든다.
- rewrite된 specifier는 소비자가 쓰던 확장자 표기를 유지한다.
- stem이 일치하지 않는 디렉터리 index 참조는 `delegatedImports`에 실리고 move는 실행 가능하다. 디렉터리 move 안쪽끼리의 참조는 `preservedImports`에 실린다.
- 위임 import를 고친 post snapshot은 finding이 없다. 고치지 않으면 `delegated-import-missing`, 보존 import가 가로채이면 `preserved-import-broken`이 난다.
...
- 옮기는 파일이 옮기지 않는 파일을 가져오는 import(파일 참조, 디렉터리 참조)도 수정 목록에 실린다. 절대 경로 specifier는 수정도 위임도 만들지 않는다.
...
- 남은 source, 누락/다른 target, node type, artifact, entry point, import rewrite/위임/보존/boundary와 cycle 또는 non-exact DAG는 postcondition FAIL이다.
