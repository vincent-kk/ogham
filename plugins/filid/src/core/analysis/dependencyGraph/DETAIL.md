...
- DAG 대상인 production dependency가 미해소이거나, adapter가 `certainty: 'indeterminate'`로 표시했거나, source/target owner를 찾지 못하면 edge로 쓰지 않고 certainty를 indeterminate로 보존한다. adapter가 확정한 verification 참조는 DAG 대상이 아니므로 미해소와 owner 부재로는 graph certainty를 낮추지 않는다. adapter가 indeterminate로 표시한 참조는 verification 파일의 것이어도 graph를 indeterminate로 만든다. 그 참조가 실제 import라면 edge 증거가 없어 이동 계획의 수정 목록에서 빠지기 때문이다. 수집기의 미해소 진단은 그대로 남으며, 시작 certainty의 분석 실패·ownership 불확정·unsupported도 그대로 보존한다.
...
- adapter가 indeterminate로 표시한 참조는 production이든 verification이든 edge가 되지 않고 graph를 indeterminate로 만든다.
