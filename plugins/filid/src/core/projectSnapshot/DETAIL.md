...
- snapshot hash가 byte를 읽는 파일 목록은 **adapter가 발견한 소스 파일**(`discoverSourceFiles`)과 문서·entry point·verification 파일이다. 이것은 facts의 **스캔 집합과 다를 수 있다**: 스캔은 `**/scripts/**` 같은 기본 제외 패턴을 적용하고 adapter discovery는 자기 제외 목록(`node_modules`, `dist`, `bridge` …)을 쓴다. 발견은 확장자와 경로로 하는 일이라 내용 판독이 아니며(P1), 기준을 스캔 집합으로 옮기면 문서 한 줄이 snapshot hash와 리뷰의 `sourceHash`를 움직인다. 스캔이 빼는 소스 파일은 facts 상태가 없어 참조 규칙의 대상이 아니고 `filesOutsideFactsScope`에도 들지 않는다 — 그 개수는 **스캔된** 파일 중 선언된 범위가 뺀 것만 센다.
