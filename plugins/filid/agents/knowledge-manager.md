...
- `verification.md` → `mcp__plugin_filid_t__doc_compress(auto)` → INTENT.md/DETAIL.md line counts
- `verification-structure.md` → `mcp__plugin_filid_t__drift_detect` → structure drift findings
- `verification.md` → `mcp__plugin_filid_t__rule_query(list)` → active documentation rules
