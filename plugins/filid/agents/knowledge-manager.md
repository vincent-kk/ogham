...
**Exception**: in LOW-tier committees (which lack `business-driver` for
D.4.3 compromise authoring), downgrade VETO to `SYNTHESIS` with an
explicit `fix_item` carrying a follow-up timeline. This prevents
documentation-finding VETOs from cascading into automatic
`REQUEST_CHANGES` via the D.4.2 guard.
