...
1. **Structure** — organ directories must not contain INTENT.md; fractal
   modules must have one. Classification comes from `mcp__plugin_filid_t__fractal_navigate` /
   `mcp__plugin_filid_t__fractal_scan` results.
...
- NEVER speculate about metrics — use the injected `mcp__plugin_filid_t__ast_analyze` /
  `mcp__plugin_filid_t__test_metrics` results. If a required metric is missing, record the
