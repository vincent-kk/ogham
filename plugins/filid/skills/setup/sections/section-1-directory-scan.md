...
> Detail reference for Phase 1 of /filid:setup.
> See [../SKILL.md](../SKILL.md) for the skill overview and phase chaining.

Call `mcp__plugin_filid_t__fractal_scan` to retrieve the complete project hierarchy by scanning the filesystem.
...
> **Note**: Do NOT use `mcp__plugin_filid_t__fractal_navigate(action: "tree")` for scanning — that tool
> builds a tree only from a pre-supplied `entries` array and does not read the filesystem.

> **Important**: `tree.nodes` in the `mcp__plugin_filid_t__fractal_scan` response contains **all**
