...
5. A **module index** reported by a registered adapter makes a non-organ
   directory a fractal. An executable or framework entry point does not, and
   neither does a path injected through the config `entryPointOverrides` —
   those feed the entry-point surface, never classification.
6. A leaf without fractal children is an organ.
7. A non-leaf directory proven stateless and side-effect-free is a
   pure-function node.
8. The remaining directories are organs. A directory that declares neither a
   document nor a module index has never claimed an independent contract.
