...
> `mcp__plugin_filid_tools__test_metrics` MCP results.
>
...
> **Capability Model**: Per the policy in `restructure/SKILL.md` Stage
...
discovery, then invokes `mcp__plugin_filid_tools__test_metrics(action: "count", files: [...])`
...
The **skill** calls `mcp__plugin_filid_tools__test_metrics(action: "check-gate", files: [...])`
...
> skills (`filid:resolve` Step 4b when a fix item has `type:
> promote`). Not intended for direct user invocation via
> `/filid:promote`. The examples below document orchestrator-level
> argument shapes.
