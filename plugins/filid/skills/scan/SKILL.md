...
> NEVER yield after `mcp__plugin_filid_t__fractal_scan` result, parallel rule evaluations, or
...
Build the project hierarchy using `mcp__plugin_filid_t__fractal_scan` and partition into fractal
...
Verify no organ directory contains a INTENT.md file using `mcp__plugin_filid_t__fractal_scan` results from Phase 1.
See [reference.md Section 3](./reference.md#section-3--organ-directory-validation).

### Phase 4 — Test File Validation (3+12 Rule)

Validate all `*.spec.ts` files against the 15-case limit using `mcp__plugin_filid_t__test_metrics`.
