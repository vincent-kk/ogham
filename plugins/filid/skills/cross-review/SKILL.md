...
Call `review_state({ action: "seal", projectRoot: PROJECT_ROOT, branchName: BRANCH, baseRef: BASE_REF })`. Continue when `status: ok` and `summary.disposition: sealed`. An `indeterminate` seal wrote nothing and changed no phase: it is a designed refusal taken before the verdict is folded, not an abnormal seal. The rule is one rule — read each diagnostic by its `code` field, never by message text, and carry out its `nextAction` whenever the agent can, then repeat the call it names once; `review-facts-frozen-unusable` and its kin resolve that way with a single `prepare`. `facts-discrepancy` is the standing case and its two answers differ in what they cost. An item nobody has settled: adjudicate it through the [facts bootstrap](../.shared/facts-bootstrap.md), where a `dismiss` gets its second actor, or submit that file again with a tool that reads it correctly; then `seal` again, because settling an open item needs no new review. A reviewed file whose valid references are no longer the ones this generation froze — an edge added, or one it was judged on gone — is the other, and it does need one: call `prepare` once with the same arguments and without force, which opens a new generation, so an outstanding handoff validates as `review-generation-superseded` and is discarded by Step 3's rule, then review the files whose valid references changed and seal. On a non-`ok` status with no `nextAction` the agent can perform, and on the same refusal returning a second time in this run, end the run without a terminal verdict and record the diagnostics in the report as a filid defect.
