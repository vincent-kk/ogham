...
- **Artifact completeness sentinel.** An A/B/C artifact is consumable only when its phase has FINISHED, not merely when the file exists — the streaming-write skeleton (frontmatter + empty tables, every unresolved value `PENDING`) exists long before the analysis fills it. The terminal frontmatter field is the done-marker: `overall` (`structure-check.md`), `metrics_passed` (`verification-metrics.md`), `structure_passed` (`verification-structure.md`); for `session.md` it is `committee` + `deliberation_mode`. The producer writes it LAST and it stays `PENDING` (or absent) until the phase completes. Every consumer — the subagent's Step 3.9 self-check, the pipeline A/B/C cross-check, and the chairperson coverage-fallback — MUST treat a `PENDING` sentinel (or any residual `PENDING` cell) as **incomplete**, never as a done or empty result, and MUST NOT overwrite it while the producing worker may still be running.
