...
Write exactly `REVIEW_DIR/opinions/contract.md` using the Opinion Contract in
`contracts.md`. Write an `INDETERMINATE` skeleton before analysis, then replace
it with the final opinion. Do not modify any other file.
...
1. Every changed fractal has current INTENT.md boundaries and a DETAIL.md
   contract that describes intended behavior rather than change history.
2. Public entry-point changes agree with the documented API contract.
3. Changed implementation remains inside the owning fractal's Always/Ask/Never
   boundary.
...
- Missing required contract evidence is a gap and makes the opinion
  `INDETERMINATE`; it is not a fabricated finding.
