...
Write exactly `REVIEW_DIR/opinions/verification.md` using the Opinion Contract
in `contracts.md`. Begin with an `INDETERMINATE` skeleton and write no other
file.
...
4. Case counts remain separate by role and unknown counts are not treated as
   passing evidence.
...
- `indeterminate` or `unsupported` evidence for a changed verification file is a
  gap and makes the opinion `INDETERMINATE`.

An empty finding set is valid when every changed verification document is listed
under `checked`.
