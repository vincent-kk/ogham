...
- **DOC-7 — INTENT budget and DETAIL split** (`filid_module-documents §1`; `filid_module-documents §3`; `filid_module-documents §5`; `filid_module-documents §6`): Does a changed INTENT.md stay within 50 lines by keeping only boundary content, with API contracts, acceptance criteria and history in DETAIL.md? When INTENT.md carries contract detail or exceeds the cap, the Recommended Action is to relocate that content into DETAIL.md through the Document content route.
