...
Effective effort is frozen from preparation for the same source identity, including before the first validation. A changed effective effort returns `review-effort-locked`; resume with the saved effort or explicitly use `--force` after all prior actors finish. Matching-effort metadata changes preserve briefs, caller context, and opinions. Fresh state records `validationPolicyVersion: 2`; older or unsupported policies return `review-validation-policy-outdated` without reusing old approvals or automatically restarting reviewers. Sealing rechecks the current worktree; changes after a seal return `review-worktree-stale` and require prepare while preserving the earlier artifacts. Prepare reports the mode, effective effort, reason, reviewable group count, and `maxReviewerHandoffs` (the sum of configured reviewer rounds, excluding verification and retries). It is a call bound, not a token budget.
