...
| `hook/`           | 훅 전용 경량 inspect/apply 독립 진입점 |
...
- 훅 진입점은 범용 plan/apply/revision/locking 그래프와 분리한다.
...
- hook entry에서 범용 manager, transaction, lock 모듈 import.

## Dependencies

- 범용: `targets`, `transactions`, `@ogham/cross-platform/instructions`.
- 훅: 목적별 target과 `@ogham/cross-platform/filesystem/hook-io`,
  `@ogham/cross-platform/instructions/write`.
