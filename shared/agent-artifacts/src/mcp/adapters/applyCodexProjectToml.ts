...
export async function applyCodexProjectToml(
  ...
  const validationError = validateMcpRequest(plan.request, "codex-project");
  ...
  const write = action === "copy" || action === "update" || action === "remove";
  const candidate = write
    ? buildCodexProjectContent({
        context,
        request: plan.request,
        source,
      })
    : { ok: true as const, content: source };
  if (!candidate.ok)
    return {
      ok: false,
      outcomes: plan.outcomes,
