...
- **4 user-invocable skills** — `/setup` (opens the local settings UI), `/codex` (delegate to Codex CLI), `/gemini` (delegate to Gemini CLI), `/crosscheck` (dispatch the same prompt to both codex and gemini in parallel, then synthesize the two answers). Skills are thin tool-call mappers; model IDs and CLI option details live in the dispatcher, never the skill body.
